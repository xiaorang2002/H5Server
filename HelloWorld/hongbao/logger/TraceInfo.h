/*
	...
	CTraceInfo::Instance().start(lpszFile);
	CTraceInfo::Instance().Term();

*/

#ifndef __TRACEINFOLOG_HEADER__
#define __TRACEINFOLOG_HEADER__
...
#define TRACE_LOG_END
    ...
    #define TRACE_HEX		CTraceInfo::Instance()->TraceBin
...
public:
	...
	static CTraceInfo& Instance()
	{
        static CTraceInfo* pInst=NULL;
		if (!pInst) {
			 pInst  = new CTraceInfo();
		}
	//Cleanup:
		return (*pInst);
	}
...
public:
	static void Trace(LPCTSTR lpszFormat,...)
	{
		do 
		{
			...
			CTraceInfo::Instance().mytrace(szResult);
		}   while (0);
	//Cleanup:
		return;
	}
	...
	//Cleanup:
		if (lpszBuffer) {
			CTraceInfo::Instance().mytrace(lpszBuffer);
			free(lpszBuffer);
			lpszBuffer=NULL;
		}
	}
