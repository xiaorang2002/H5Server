...
//	virtual bool OnGameMessage(WORD sub_cmdid, void* data, WORD data_size, shared_ptr<CServerUserItem> server_user_item);
//	virtual bool OnFrameMessage(WORD sub_cmdid, void* data, WORD data_size, shared_ptr<CServerUserItem> server_user_item);
...
public:
	...
	{
// 		TRACE_INFO(_T("TableFrameSink::OnActionUserOffLine start. [table_id=%d chair_id=%d]"), pIServerUserItem->GetTableID(), chair_id);
...
// 		TRACE_INFO(_T("TableFrameSink::OnActionUserOffLine End."));
...
public:
    ...
    virtual WORD get_table_id() {return table_frame_->GetTableId();}
// protected:
...
// 	bool SendGameConfig(shared_ptr<CServerUserItem> server_user_item);
	...
	bool OnSubHitFish_Normal(std::shared_ptr<CServerUserItem> server_user_item, const CMD_C_HitFish& hit_fish, unsigned short fish_count, BulletScoreT fire_score, const FishKind* fish_kind, short &catch_fish_count, bool &isBoss, bool &bBossOrGlobalBomb, LONG &score, bool &bHlwBoss);
	bool OnSubHitFish_Special(std::shared_ptr<CServerUserItem> server_user_item, const CMD_C_HitFish& hit_fish, unsigned short fish_count, BulletScoreT fire_score, const FishKind* fish_kind, short &catch_fish_count, bool &isBoss, bool &bBossOrGlobalBomb, bool &is_pause_boom, LONG &score);
    ...
    bool GetResult(PONEBKINFO pList ,tagPersonalProfit currentpersomstock,int64_t currentstock,int64_t minstock,int64_t maxstock,int32_t ncount);
...
//                server_user_item->GetNickName(), server_user_item->GetUserId(), table_frame_->GetTableId(), oldChairID);
//            GetAicsEngine()->OnUserStandup(server_user_item->GetUserId(), table_frame_->GetTableId(), oldChairID, FALSE, FALSE);
//            LOG(INFO) << info;

//            snprintf(info,sizeof(info),"玩家昵称=%s,玩家ID=%d,房间号=%d,椅子号=%d)玩家坐下!!!",
//                server_user_item->GetNickName(), server_user_item->GetUserId(), table_frame_->GetTableId(), newChairID);
//            GetAicsEngine()->OnUserSitdown(server_user_item->GetUserId(), table_frame_->GetTableId(), newChairID, FALSE, FALSE);
//            LOG(INFO) << info;
#endif
...
private:
	...
	void writeArithmLog1(WORD chair_id) {
		...
		m_traceArithm->updateData(game_service_option_->wServerID, table_frame_->GetTableID(), chair_id, winScore, 0, 1);
	}
	void writeArithmLog2(WORD chair_id, long long exchangeProfit) {
		m_traceArithm->updateData(game_service_option_->wServerID, table_frame_->GetTableID(), chair_id, 0, exchangeProfit, 2);
	}
#endif
private:
...
private:
    std::shared_ptr<ITableFrame>        table_frame_;
    tagGameRoomInfo*    game_service_attrib_;   // chaircount = startmaxplayer.
    ...
    shared_ptr<CServerUserItem> server_user_item_for_game_scene[MAX_PLAYERS];
...
// 			TRACE_INFO(_T("TableFrameSink::setCollectStats[table_id=%d chair_id=%d stats=0x%02x]"), table_frame_->GetTableID(), chairId, game_config.collectData.collectStats[chairId]);
		}
	}
...
protected:
	...
	muduo::net::TimerId             m_TimeridHLWKeepAlive;                     //海龙王保持打不死
	...
	void OnTimerHLWKeepAlive();
	void SetHaiLongWangInfo(bool bfirstInit, int fishtype = 50);
	...
	bool										m_bKeepHaiLongWangAlive;
	CMD_S_HaiLongWangInfo						m_haiLongWangInfo;					//海龙王生命信息
