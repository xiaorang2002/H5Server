...
//构造函数
CWHEncryptClone::CWHEncryptClone()
{
}
...
//生成密文
bool CWHEncryptClone::MD5Encrypt(LPCTSTR pszSourceData, TCHAR szMD5Result[LEN_MD5])
{
	//变量定义
	CMD5Aide MD5Aide;
	UCHAR szResult[16];
...
//生成密文
bool CWHEncryptClone::XorEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncrypData, WORD wMaxCount)
{
	//变量定义
	CT2CW strSrcData(pszSourceData);
	TCHAR szEncrypData[MAX_ENCRYPT_LEN+1]=TEXT("");
	...
	for (WORD i=0;i<wTimes;i++)
	{
		...
		_stprintf(szEncrypData+i*8,TEXT("%04X%04X"),wRandKey[i%ENCRYPT_KEY_LEN],wTempCode);
	}
...
//解开密文
bool CWHEncryptClone::XorCrevasse(LPCTSTR pszEncrypData, LPTSTR pszSourceData, WORD wMaxCount)
{
	//设置结果
	pszSourceData[0]=0;
	...
	WCHAR szSrcData[MAX_SOURCE_LEN]=TEXT("");
	...
	//获取长度
	TCHAR * pEnd=NULL;
	WORD wSoureLength=(WORD)_ttol(szTempBuffer,&pEnd,16);
	...
	for (INT i=0;i<wSoureLength;i++)
	{
		...
		//提取密钥
		TCHAR wKey=(WCHAR)_ttol(szKeyBuffer,&pEnd,16);
		TCHAR wEncrypt=(WCHAR)_ttol(szTempBuffer,&pEnd,16);
...
//生成密文
bool CWHEncryptClone::MapEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncrypData, WORD wMaxCount)
{
	//效验参数
...
//解开密文
bool CWHEncryptClone::MapCrevasse(LPCTSTR pszEncrypData, LPTSTR pszSourceData, WORD wMaxCount)
{
	//效验参数
	ASSERT(wMaxCount>lstrlen(pszEncrypData));
	ASSERT((pszEncrypData!=NULL)&&(pszSourceData!=NULL));
...
//生成密文
bool CWHEncryptClone::DesEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncrypData, WORD wMaxCount)
{
	//效验参数
	ASSERT(wMaxCount>lstrlen(pszEncrypData));
	ASSERT((pszEncrypData!=NULL)&&(pszSourceData!=NULL));
	...
	//加密数据
	yxyDESClone des;
	std::string src = CT2CA(pszSourceData);
	des.InitializeKey(DES_KEY);
	des.EncryptAnyLength(src);
	char* szDes = des.GetCiphertextAnyLength();

	lstrcpyn(pszEncrypData, CA2CT(szDes), lstrlen(pszEncrypData));

	return true;
}

//解开密文
bool CWHEncryptClone::DesCrevasse(LPCTSTR pszEncrypData, LPTSTR pszSourceData, WORD wMaxCount)
{
	pszSourceData[0]=0;
	//解密数据
	yxyDESClone des;
	std::string src = CT2CA(pszEncrypData);
	des.InitializeKey(DES_KEY);
	des.DecryptAnyLength(src);
	char* szDes = des.GetPlaintextAnyLength();

	lstrcpyn(pszSourceData, CA2CT(szDes), lstrlen(pszEncrypData));
