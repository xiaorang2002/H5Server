...
#endif
	//m_fishProduceController = new FishProduceController();
}
...
FishProduceData* FishProduceDataManager::create(const FishType& fishType)
{
	...
	if (fishBasicData) {
		FishProduceData* fishProduceData = FishProduceAllocator::getInstance()->allocFishProduceData();
                ...
                moveToNextFishID();
				if (fishType.type == 50) //o�ꨢ����?
				{
					fishKind.life = getHaiLongWangLife();
				} 
				else
				{
...
unsigned short FishProduceDataManager::popFishesForSending(unsigned char* buffer, unsigned short bufferSize, unsigned int startTick, const CMD_S_HaiLongWangInfo& haiLongWangInfo)
{
	unsigned short dataSize = 0;
	if (buffer && m_fishesForSending.size()) {
		dataSize = NetworkUtility::encodeProduceFishes(buffer, bufferSize, m_fishesForSending, haiLongWangInfo);
	}
