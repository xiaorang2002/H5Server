...
int	 ConfigManager::CheckReadConfig()
{
    ...
    configDir = str(boost::format("./conf/sgj_config.ini"));
    ...
    m_Config.m_AndroidRatio = pt.get<double>("FruitConfig.AndroidRatio", 1.0);
    ...
    m_Config.m_AgentId = pt.get<int64_t>("FruitConfig.AndroidAgentID", 10000);
    ...
    //机器人假ID配置权重
    {
        TCHAR Bufer[255] = TEXT("");
        TCHAR OutBufer[255] = TEXT("");
        TCHAR INI_ANDROIDUSERID_CONFIG[30] = TEXT("FruitConfig");
        TCHAR KeyNameAnd[64] = TEXT("AndroidUserId");
        //
        int readSize1 = GetPrivateProfileString(INI_ANDROIDUSERID_CONFIG, KeyNameAnd, Bufer, OutBufer, sizeof(OutBufer), configDir.c_str());
        // LOG(WARNING) << " OutBuf "<< OutBuf;
        ...
        //解析
        {
            ...
            int32_t temp = atoi(p);
            m_Config.m_AndroidUserId[0] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidUserId[1] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidUserId[2] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidUserId[3] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidUserId[4] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidUserId[5] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidUserId[6] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidUserId[7] = temp;
            p = mystchr(p, ',') + 1;  temp = atoi(p);
             m_Config.m_AndroidUserId[8] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidUserId[9] = temp;

        }
        TCHAR KeyNameAnd1[64] = TEXT("AndroidWeight");
        int readSize2 = GetPrivateProfileString(INI_ANDROIDUSERID_CONFIG, KeyNameAnd1, Bufer, OutBufer, sizeof(OutBufer), configDir.c_str());
        // LOG(WARNING) << " OutBuf "<< OutBuf;
        ...
        {
            int32_t temp = atoi(p);
            m_Config.m_AndroidWeight[0] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidWeight[1] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidWeight[2] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidWeight[3] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidWeight[4] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidWeight[5] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidWeight[6] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidWeight[7] = temp;
            p = mystchr(p, ',') + 1;  temp = atoi(p);
             m_Config.m_AndroidWeight[8] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
             m_Config.m_AndroidWeight[9] = temp;
        ...
        memset(KeyName, 0, sizeof(KeyName));
        {
            for(int i=0;i<3;i++)
            {
                ...
                //解析
                {
                    ...
                    p = mystchr(p, ',') + 1; temp = atoi(p);
                    Item.nFruitPro[11] = temp;
                    //Bouns的概率
                    p = mystchr(p, ',') + 1;  temp = atoi(p);
                    Item.nBounsPro[0] = temp;
                    p = mystchr(p, ',') + 1; temp = atoi(p);
                    Item.nBounsPro[1] = temp;
                    p = mystchr(p, ',') + 1;  temp = atoi(p);
                    Item.nBounsPro[2] = temp;
                    p = mystchr(p, ',') + 1; temp = atoi(p);
                    Item.nBounsPro[3] = temp;
                    p = mystchr(p, ',') + 1; temp = atoi(p);
                    Item.nBounsPro[4] = temp;
                    p = mystchr(p, ',') + 1; temp = atoi(p);
                    Item.nBounsPro[5] = temp;
                    p = mystchr(p, ',') + 1; temp = atoi(p);
                    Item.nBounsPro[6] = temp;
                    p = mystchr(p, ',') + 1; temp = atoi(p);
                    Item.nBounsPro[7] = temp;
                    p = mystchr(p, ',') + 1; temp = atoi(p);
                    Item.nBounsPro[8] = temp;
                }
...
void ConfigManager::GetConfig(tagAllConfig &config) 
{
   ...
   config.m_AgentId = m_Config.m_AgentId;

   config.m_AndroidRatio = m_Config.m_AndroidRatio;
   ...
   for(int i=0;i<10;i++)
   {
       config.m_AndroidUserId[i] = m_Config.m_AndroidUserId[i];
       config.m_AndroidWeight[i] = m_Config.m_AndroidWeight[i];
   }
}
