...
#include "../Game_QznnLandy/CMD_Game.h"
...
int32_t	CAndroidUserItemSink::m_cbJettonMultiple[MAX_JETTON_MUL] = { 1,5,10,15,20,25};	//下注倍数
int32_t	CAndroidUserItemSink::m_cbBankerMultiple[MAX_BANKER_MUL] = { 1,2,3,4 };		//叫庄倍数
...
//    m_TimerCallBanker.stop();
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (randWeight <= tempweightsize)
        {
            ...
            {
                openLog("GameTimerCallBanker 4 randWeight=%d",randWeight);
                cbCallBankerMul = m_cbBankerMultiple[3];
                break;
            }
        }
    }
...
//    m_TimerAddScore.stop();
    ...
    int   m_jiabeibeishutmp[MAX_JETTON_MUL]={0};
    if(m_cbOxCardData == 0)
...
bool CAndroidUserItemSink::OnGameMessage(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    ...
    {
        Qznn::CMD_S_GameStartAi GameStartAi;
...
bool CAndroidUserItemSink::OnSubCallBankerResult(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    ...
    if (BankerResult.dwbankeruser() != m_wChairID && m_m_cbPlayStatus)
    {
        ...
        float dwAddScore = dwShowTime + rand() % jettontime - 1;//((jettontime - 1)* TIME_BASE);
...
void CAndroidUserItemSink::ReadConfigInformation()
{
    ...
    myprintf(szConfigFileName, sizeof(szConfigFileName), _T("%sconf/qznn_config.ini"), szPath);
...
//            std::cout << "##################ReadConfig Key:"<< KeyName << std::endl;
            int readSize = GetPrivateProfileString(INI_SECTION_CONFIG, KeyName, _T"30,30,15,15,10", OutBuf, sizeof(OutBuf), szConfigFileName);
//            std::cout << "--------------------";
            ASSERT(readSize > 0);
            p = OutBuf;
            if(p==nullptr)
...
//            std::cout << m_qzbeishu[i][0] << "," << m_qzbeishu[i][1] << "," << m_qzbeishu[i][2] << "," << m_qzbeishu[i][3] << std::endl;
    ...
    memset(KeyName, 0, sizeof(KeyName));
    {
        for(int i=0;i<4;i++)
        {
            ...
            int readSize =  GetPrivateProfileString(INI_SECTION_CONFIG, KeyName, _T"30,30,10,10,10,10", OutBuf, sizeof(OutBuf), szConfigFileName);
