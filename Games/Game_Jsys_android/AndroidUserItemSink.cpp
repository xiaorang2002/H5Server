...
bool CAndroidUserItemSink::ReadConfigInformation()
{
	...
	//设置文件名
	if (!boost::filesystem::exists("./conf/jsys_config.ini"))
	{
		LOG(WARNING) << "conf jsys_config.ini not exists";
		return false;
	}

	boost::property_tree::ptree pt;
	boost::property_tree::read_ini("./conf/jsys_config.ini", pt);
    ...
    for (int i = 0; i < BET_ITEM_COUNT; ++i)
	{
		...
		if(i < 5){
			...
			printf("m_AddMaxGold[%d]=%d\n",i,(int)m_AddMaxGold[i]);
		}

		// 区域最大下注筹码
        sprintf(buf, "GAME_AREA_MAX_JETTON.MaxJetton%d",i);
        m_AreaRobotAddGold[i] = pt.get<int64_t>(buf, 1000);
	}
...
//			}
			...
			m_TimerJetton = m_TimerLoopThread->getLoop()->runAfter(1.0f, CALLBACK_0(CAndroidUserItemSink::OnTimerJettonOld, this));
			return true;
		}
