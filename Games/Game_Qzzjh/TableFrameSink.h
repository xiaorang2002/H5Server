...
//配置文件
#define INI_FILENAME "./conf/qzzjh_config.ini"
#define INI_CARDLIST "./conf/qzzjh_cardList.ini"
...
private:
	...
	//牌型倍数 ///
	uint8_t GetMultiple(uint8_t handCardsType) {
		if (handCardsType == ZJH::Tysp)return 1;//散牌
		else if (handCardsType == ZJH::Ty20) return 2;//对子
		else if (handCardsType <= ZJH::Tysc)return 3;//顺子，金花(同花)
		else if (handCardsType >= ZJH::Ty123sc)return 4;//顺金(同花顺)，豹子(炸弹)
	}
private:
	//换牌策略分析 ///
	//收分时，机器人都拿大牌，玩家拿小牌
...
private:
	...
	int callBankerFlag_[GAME_PLAYER];
	//玩家下注(-1:未下注 大于-1:对应筹码索引)
	int addScoreIndex_[GAME_PLAYER];
	//玩家摊牌(-1:未摊牌 大于-1:对应手牌牌型)
	int openCardType_[GAME_PLAYER];
	//可用的叫庄倍数
	int8_t multiCallable_[GAME_PLAYER][MAX_BANKER_MUL];
	//可用的下注倍数
	int8_t multiJettonable_[GAME_PLAYER][MAX_JETTON_MUL];
	//待点杀黑名单玩家
