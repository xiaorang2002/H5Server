...
// 				}
	...
	bool CGameLogic::handinfo_t::Select(int groupindex) {
		if (groupindex >= 0) {
			...
			if (spec_groups.size() > 0) {
				...
				//	groups[select_group_index]->specialTy : Tysp;
			}
			//不含特殊牌型
			else {
				//枚举几组最优解必须存在
	...
	//返回玩家选牌结果
	/*CGameLogic::handinfo_t::SelectTy*/uint8_t CGameLogic::handinfo_t::GetSelectResult() {
		uint8_t c = 0;
