...
#define CHOOSE_COUNT    100				    //造牌最大循环次数
//扎金花
namespace ZJH {
	...
	public:
		...
		///////////////////////////////////////////////////////////
		void clearHaveCard();
    	bool isHaveSameCard(uint8_t *cards,uint8_t cardcount);
    	void addHaveChooseCard(uint8_t *cards,uint8_t cardcount);
    	void resetLestCard();
    	//排列扑克
    	void SortCardList(uint8_t *cards,uint8_t cardcout);
		//根据牌型来制造玩家手牌
		bool MadeHandCards(uint8_t *cards,uint8_t cardType);
		//获取散牌
		bool GetRandSanPai(uint8_t *cards,uint8_t cardcout);
		//获取对子
		bool GetRandDuiZi(uint8_t *cards,uint8_t cardcout,bool bSmall,uint8_t bigvalue=6);
		//获取顺子
		bool GetRandShunZi(uint8_t *cards,uint8_t cardcout);
		//获取金花
		bool GetRandJinHua(uint8_t *cards,uint8_t cardcout);
		//获取顺金
		bool GetRandShunJin(uint8_t *cards,uint8_t cardcout);
		//获取豹子
		bool GetRandBaoZi(uint8_t *cards,uint8_t cardcout);	
		//获取花色
    	uint8_t GetCardColorValue(uint8_t card) { return (card&0xF0)>>4; }	
    	//获取浮点数
    	float RandomBetweenFloat(float a, float b)
    	{
    		return m_random.betweenFloat(a, b).randFloat_mt();
    	}
		////////////////////////////////////////////////////////////
	public:
		//花色：黑>红>梅>方
