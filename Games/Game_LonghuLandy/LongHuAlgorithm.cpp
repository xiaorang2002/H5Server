...
{
    {
        ...
        char Filename[256] = { 0 };
        sprintf(Filename, "./log/longhu/longhu_%d%d%d_%d_%d.log",t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, m_roomID, m_tableID);
...
void LongHuAlgorithm::BlackGetMustkillOpenCard(int8_t &wintag,float probirity[3],int64_t jetton[4],bool isMust)
{
    if(isMust)
    {
        //公平百分之五十出随机结果
    ...
    else
    {
        ...
        {
            if(wintag==3) wintag =2;
            if(wintag==2) wintag =3;
        }
        else if(miCurrentProfit<-500000&&m_killOrReword==0)
        {
            if(wintag==3||wintag==2)
              wintag = m_random.betweenInt(2,3).randInt_mt(true);
        }
        if(wintag==1)
        {
            winscore =-bettingQuantity[He]*m_muticleArr[He];
