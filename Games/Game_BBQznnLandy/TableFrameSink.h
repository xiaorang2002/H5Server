...
private:
    ...
    void MakeCardTest(shared_ptr<CServerUserItem> user,int userIndex);
    void TestTwoKing();
    void TestCard(shared_ptr<CServerUserItem> user,int userIndex);
    //获取玩家最优牌型组合
    void GetMaxHandCard(uint8_t cbMaxCardValue[MAX_COUNT],int chairid);

    void InitLoadini();

	//设置当局游戏详情
	void SetGameRecordDetail(int64_t userScore[], int64_t userWinScorePure[], int32_t calcMulti[]);
...
//    uint8_t                 m_SitTableFrameStatus[GAME_PLAYER];				    //游戏中in play
    ...
    int                     m_sMaxCardType[GAME_PLAYER];                        //玩家开牌(最大牌型点数)

    //玩家标识
    int                     m_wBankerUser;										//庄家
    uint8_t                 m_cbCardData[GAME_PLAYER][MAX_COUNT];				//牌数据
    uint8_t                 m_cbOXCard[GAME_PLAYER][MAX_COUNT];					//牛牛牌数据
    uint8_t                 m_cbKingChangCard[GAME_PLAYER][2];                  //王变后的牌值[0]:小王:[1]:大王

    //玩法类型
    uint8_t                 m_playGameType;                                     //游戏厅的类型
    bool                    m_bPlaySixChooseFive;                               //是否6选5玩法
    bool                    m_bHaveKing;                                        //是否有大小王
    uint8_t                 m_cbHandCardData[GAME_PLAYER][MAX_COUNT_SIX];       //牌数据(6选5时的手牌)
    int                     m_iTypeMultiple[CT_MAXCOUNT];                       //各牌型倍数
...
private:
    static int64_t			m_cbJettonMultiple[MAX_JETTON_MUL];					//下注倍数 1, 5, 8, 11, 15
    static uint8_t			m_cbBankerMultiple[MAX_BANKER_MUL];					//叫庄倍数 1, 2, 3
...
private:
    ...
    uint8_t                    m_testMakeType;
    uint8_t                    m_testSameTypeCount;
    uint8_t                    m_bTestCount;

    double                     stockWeak;                                        //库存衰减
	bool                       m_bTestMakeTwoking;								 //测试带两个王的情况



    STD::Random                m_random;
    int32_t                    m_difficulty;                                      //玩家难度 千分制
//    atomic_int    mitimerstop;
