...
int	 ConfigManager::CheckReadConfig()
{
    ...
    configDir = str(boost::format("./conf/shz_config.ini"));
    if(!boost::filesystem::exists(configDir)) {
        LOG(WARNING) << "./conf/shz_config.ini not exists";
        return 0;
    }
    ...
    m_Config.m_vControlAgentid.clear();
    m_Config.m_vControlAgentid = to_array<int32_t>(pt.get<string>("FruitConfig.AGENTID","0,0,0,0"));
...
void ConfigManager::GetConfig(tagAllConfig &config) 
{
   ...
   config.m_vControlAgentid.clear();
//   for(int i=0;i<m_Config.m_vControlAgentid.size();i++)
//   {
//       config.m_vControlAgentid.push_back(m_Config.m_vControlAgentid[i]);
//   }
   config.m_vControlAgentid.assign(m_Config.m_vControlAgentid.begin(),m_Config.m_vControlAgentid.end());
}
