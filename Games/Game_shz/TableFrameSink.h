...
struct MarryInfo
{
    ...
    {
        waiIndex=0;
        for(int i=0;i<4;i++) neiIconList[i]=0;
        currentWin=0;
    }
    int waiIndex;
    int neiIconList[4];
...
public:
    ...
    bool FillTable(const int32_t wChairID,CMD_S_RoundResult_Tag &SceneOneEnd);
    UserProbabilityItem ScoreGetProbability(const int32_t wChairID,int64_t StoreScore);//, UserProbabilityItem &UserPro);
    ...
    void GetAllResult(int chairid ,int  marrytimes,int64_t &allMarryWin);
private:
    //初始化游戏数据
    void InitGameData();
...
private:
    ...
    bool  m_iUserBeControlAgent[GAME_PLAYER];
    //牌型
    string cardValueStr;
    string iconResStr;
...
private:                                  //Storage Related
    ...
    vector<int32_t> controlledProxy;            //设置需要控制代理列表
};
