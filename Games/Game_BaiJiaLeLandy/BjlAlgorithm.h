#ifndef BJLALGORITHM_H
#define BJLALGORITHM_H
#include <string.h>
#include "stdlib.h"
#include "stdint.h"
...
public:
     ...
     inline void  SetAlgorithemNum(int64_t high,int64_t low,int64_t curr,float ptobali,double mularr[Max],int64_t threshold=5000)
     {
         if(high<low)
         {
             m_highLimit = low;
     ...
     void GetOpenCard(int8_t & wintag, int8_t cardPair[]);

     void CallBackHandle();

     int64_t CurrentProfit();//计算本局不算税的系统赢输

     void BlackGetOpenCard(int8_t &wintag,float probirity[3], int8_t cardPair[]);
     void BlackGetMustkillOpenCard(int8_t &wintag,float probirity[3],int64_t jetton[Max], int8_t cardPair[],bool isMust = true);
     void  openLog(const char *p, ...);
	 long CalculatWinScore(int winTag, int8_t cardPair[]);		//计算出临时赢分
	 long CalculatTestWinScore(int winTag, int8_t cardPair[]);
	 long getAllBetScore();
};
