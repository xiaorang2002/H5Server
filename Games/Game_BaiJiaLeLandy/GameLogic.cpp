...
//牌值是否有效
bool CGameLogic::IsCardValid(uint8_t cbCardData)
{
	if (cbCardData == 0) return false;
	uint8_t value = GetCardValue(cbCardData);
	uint8_t color = GetCardColor(cbCardData);
...
//获取牌点
uint8_t CGameLogic::GetCardPip(uint8_t cbCardData)
{
	//计算牌点
	uint8_t cbCardValue = GetCardValue(cbCardData);
	uint8_t cbPipCount = (cbCardValue >= 10) ? 0 : cbCardValue;

	return cbPipCount;
}

//获取牌点
uint8_t CGameLogic::GetCardListPip(const uint8_t cbCardData[], uint8_t cbCardCount)
{
	//变量定义
	uint8_t cbPipCount = 0;
...
uint8_t CGameLogic::IsCardPair(uint8_t cbBankerData[], uint8_t cbPlayerData[], uint8_t cbCardCount, uint8_t index)
{
	uint8_t nPair = 0;
	switch (index)
	{
	case 0: 
	{
		uint8_t cbBankerValue = GetCardListPip(cbBankerData, cbCardCount);
		uint8_t cbPlayerValue = GetCardListPip(cbPlayerData, cbCardCount);
		...
		{
			if (GetCardLogicValue(cbPlayerData[0]) != GetCardLogicValue(cbBankerData[0]) && GetCardLogicValue(cbPlayerData[0]) != GetCardLogicValue(cbBankerData[1]))
			{
				return 0;
			}
			if (GetCardLogicValue(cbPlayerData[1]) != GetCardLogicValue(cbBankerData[0]) && GetCardLogicValue(cbPlayerData[1]) != GetCardLogicValue(cbBankerData[1]))
			{
				return 0;
			}
			if (GetCardLogicValue(cbPlayerData[0]) == GetCardLogicValue(cbBankerData[0]) && GetCardLogicValue(cbPlayerData[1]) == GetCardLogicValue(cbBankerData[1]))
			{
				return 1;
			}
			if (GetCardLogicValue(cbPlayerData[0]) == GetCardLogicValue(cbBankerData[1]) && GetCardLogicValue(cbPlayerData[1]) == GetCardLogicValue(cbBankerData[0]))
			{
				return 1;
			}
		}
	...
	case 1:
	{
		if (GetCardLogicValue(cbBankerData[0]) == GetCardLogicValue(cbBankerData[1]))
		{
			nPair = 1;
		}
		break;
	...
	case 2:
	{
		if (GetCardLogicValue(cbPlayerData[0]) == GetCardLogicValue(cbPlayerData[1]))
		{
			nPair = 1;
		}
		break;
...
uint8_t CGameLogic::IsCardPair(uint8_t cbCardData[])
{
	uint8_t nPair = 0;
	...
	if (vsize >= 4)
	{
		for (int i=vsize-1;i>=0;i--)
		{
			...
			if (vRecord[i] == index)
			{
				...
				if (typeCount >= 4)
				{
					rooteType = index == 1 ? ChangZhuang : ChangXian;
					break;
				}
			}
			else if (typeCount < 4)
...
//是否逢庄闲跳 index=1 逢庄跳 index=2 逢闲跳
			...
			else if(i - 2 >= 0)				
			{
				if (vTmpRecord[i - 1] != vTmpRecord[i - 2])
				{
                    ...
                    else if (lianID == vTmpRecord[i - 1])
                    {
                        ...
                        if (tiaoNum >= 3)
                        {
                            rooteType = (lianID == 1 ? FengZhuangtiao : FengXiantiao);
                            break;
                        }
                    }
                    else if (tiaoNum < 3)
			...
			if (i >= 3)
			{
				if (vTmpRecord[i] == vTmpRecord[i - 1] && vTmpRecord[i - 1] != vTmpRecord[i - 2] && vTmpRecord[i - 2] != vTmpRecord[i - 3])
				{
                    ...
                    if (i - 3 >= 0)
					{
						...
						{
							rooteType = (lianID == 1 ? YitingLiangfan_Zhuang : YitingLiangfan_Xian);
							break;
						}
					}
				}
        ...
        for (i = vsize - 1;i >= 0;i--)
		{
			if (!firstOK && fisrType == vTmpRecord[i])
			{
				...
				if (curlianNum >= firstLian)
				{
					...
					if (lianNum >= 3)
					{
						rooteType = (lianID == 1 ? FengZhuanglian : FengXianlian);
						break;
					}
				}
			}
...
//获取好路单 斜坡路 > 大路双跳 > 大路单跳 > 拍拍黐 > 一厅两房 (庄/闲)> 逢（庄 / 闲）跳 > 逢（庄 / 闲）连 > 长（庄 / 闲）
int CGameLogic::getGoodRouteType(vector<int> vRecord)
{
	...
	{
		//长(庄/闲)
		rooteType = isGoodType_ChangeZhuangXian(vRecord, 1);
		if(rooteType == NoHave)
