...
void openLog(const char *p, ...)
{
	...
	//aicslog
	//char aicslog[256] = { 0 };
	//snprintf(aicslog, sizeof(aicslog), "./AicsEngineTest_%04d",
	//	roomID);
	//date
	char date[256] = { 0 };
	...
	//filename
	char filename[256] = { 0 };
	snprintf(filename, sizeof(filename), "_%04d_%04d.log",
		roomID,
		tableID);
...
int testAicsDragon2()
{
	...
	////////////////////////////////////////
	//char title[4096] = { 0 };
	//sprintf(title, "房间号\t桌台号\t游戏局数\t流水号\t庄ID\t和注\t龙注\t虎注\t总注\tAI和注\tAI龙注\tAI虎注\tAI总注\t龙\t虎\t中门\t和赢\t龙赢\t虎赢\t本局赢分\t累计赢分\t本轮局数\t本轮赢分\n");
