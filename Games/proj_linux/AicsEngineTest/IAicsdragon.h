...
public:
	virtual uint32 GetTableSeqID(int32 tabid) = 0;
	virtual bool IsTableReset(int32 tabid) = 0;
public: // 出牌计算.
...
public:
	virtual uint32 GetTableSeqID(int32 tabid) {
		return 0;
	}
	virtual bool IsTableReset(int32 tabid) {
		return false;
	}
public:
...
inline IAicsDragonEngine* WINAPI GetAicsDragonEngine()
{
	...
	do
	{
		...
		if (!aics)
		{
			...
			printf("--- *** GetAicsDragonEngine buf[%s] ...\n", buf);
			printf("--- *** >>> waiting Load lib %s\n", buf);
			getchar();
#endif//_WIN32
			...
			if (!pfn) {
				printf("get function of fun0002 failed");
				FreeLibrary(hInst);
				break;
			}
