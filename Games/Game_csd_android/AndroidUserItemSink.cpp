...
#include "proto/Xsgj.Message.pb.h"

#include "../Game_XSGJ/CMD_Game.h"
#include "../Game_XSGJ/GameLogic.h"

using namespace Xsgj;
...
bool CAndroidUserItemSink::ReadConfigInformation()
{
	...
	//设置文件名
    if (!boost::filesystem::exists("./conf/xsgj_config.ini"))
	{
        LOG(WARNING) << "conf xsgj_config.ini not exists";
		return false;
	}
	//
	boost::property_tree::ptree pt;
    boost::property_tree::read_ini("./conf/xsgj_config.ini", pt);
...
bool CAndroidUserItemSink::OnEventSceneMessage(uint8_t cbGameStatus, const uint8_t *pData, uint32_t size)
{
	switch (cbGameStatus)
	{
		...
		case SUB_S_NORMAL: //普通拉霸消息
		{
			...
			// 解析
			int32_t freeCount = pRoundMsg.nfreeroundleft();
			int32_t marryCount = pRoundMsg.nmarrynum();
			int32_t njackpot = pRoundMsg.njackpot();
