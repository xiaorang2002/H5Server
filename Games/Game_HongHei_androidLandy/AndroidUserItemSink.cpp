...
bool CAndroidUserItemSink::ReadConfigInformation()
{
    ...
    //m_ProbabilityWeight[6] = pt.get<uint32_t>("ANDROID_NEWCONFIG.7robability", 10);  //50
    ...
    //每注间隔
    m_JettonIntervalInit        = pt.get<uint32_t>("ANDROID_NEWCONFIG.jiangeTimeInit", 1);
    //最后3秒下注概率
...
void CAndroidUserItemSink::OnTimerJetton()
{
    ...
    if(NowTime - m_TimerStart < (m_Endtimer - 3) * 1000)  //last 3s
    {
        ...
        {
            if(m_JettonIntervalInit==0) m_JettonIntervalInit=1;
...
//         srand((int)time(0));
    ...
    if (!m_IsShenSuanZi && m_2Area <= 95) //95%只能押一个区域（龙虎）
    {
        ...
        {
            if (bJettonArea == 2 && m_SendArea[i] == 3)
            {
                bJettonArea = 3;
                break;
            }
            if (bJettonArea == 3 && m_SendArea[i] == 2)
            {
                bJettonArea = 2;
                break;
            }
...
//    //和区域下注不能大于10
//    if(bJettonArea == SPECIAL && JettonScore > m_pITableFrame->GetGameRoomInfo()->jettons[SPECIAL])
//    {
//        if (rand() % 100 < 50)
//         {
...
//    }
    ...
    if( bJettonArea < 0 && bJettonArea > 3)
        bJettonArea = 2;
...
bool CAndroidUserItemSink::OnEventSceneMessage(uint8_t cbGameStatus, const uint8_t * pData,  uint32_t size)
{
    ...
    switch(cbGameStatus)
	{
        ...
        case SUB_S_START_PLACE_JETTON:		//游戏状态
        {
            ...
            const ::google::protobuf::RepeatedPtrField<::HongHei::PlayerListItem>& player = deskinfo.players();
...
//    sprintf(Filename, "log//HongHei//HongHei_%d%d%d.log",t->tm_year + 1900, t->tm_mon + 1, t->tm_mday);
