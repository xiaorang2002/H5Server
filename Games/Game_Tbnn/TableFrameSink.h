...
public:
    ...
    // virtual bool CanJoinTable(int64_t userId, uint32_t userIp = 0);
...
public:
    ...
    //设置换牌概率
    //virtual void c(int wChangeCardRatio) {}
...
private:
    //CardProvider c;
    //分析
    int64_t AnalyseAndroidWinScore(uint8_t cbCardData[GAME_PLAYER][MAX_COUNT]);
...
//    void ParsePrivateData(char *pBuf);
    ...
    void AnalyseCardExPlus(int iWinSeat);
...
private:
    ...
    uint32_t                 m_cbCurJetton[MAX_JETTON_MUL];						//当前筹码
    string                  mGameIds;
private:
    //扑克信息
