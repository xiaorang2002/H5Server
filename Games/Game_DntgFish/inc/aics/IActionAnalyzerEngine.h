...
// {
// 	unsigned int32 sign;		// 有效性.
// 	unsigned int32 roomid;	// 房间id.
// 	unsigned int32 tableid;	// 桌台id.
// 	unsigned int32 charid;	// 椅子id.
// 	unsigned int32 userid;	// 用户id.
// 	unsigned int32 type;		// 开炮/得分.
// 
// }   ActionAnalyzerPack;
...
typedef struct tagXimalianItem
{
	...
	unsigned long     firescore;	// 开炮分(倍率鱼).
	unsigned long    catchscore;	// 渔获分(倍率鱼).
    unsigned int32		  resv2;    // 保留字段.
}   XimalianItem;
...
{
    unsigned int32		   sign;	// 结构标识字段.
	unsigned int32          tick;	// 发送时间间隔.
	unsigned short			vid;	// 厂商id.
	unsigned short			pid;	// 产品id.
	unsigned short		tableid;	// 桌台id.
	unsigned short		 charid;	// 椅子id.
	unsigned int32		 roomid;	// 房间id.
	unsigned int32		 userid;	// 用户编号.
    unsigned int32	   toubbili;	// 投币比例.
	unsigned int32	    resv[5];	// 保留字段.
    unsigned long     firescore;	// 总进分(所有鱼).
    unsigned long    catchscore;	// 总出分(所有鱼).
