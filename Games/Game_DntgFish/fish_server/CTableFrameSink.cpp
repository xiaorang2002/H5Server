...
#else
        dir += "/dntgfish";
    google::InitGoogleLogging("dntgfish");
#endif//__HJSDS__
        FLAGS_log_dir = dir;
        ...
        for(int i=0;i<MAX_PLAYERS;i++)
        {
            if(m_pITableFrame->GetTableUserItem(i))
            {
                ...
                if(cmdData.powerlevel>=game_config.cannonPowerMin&&cmdData.powerlevel<=game_config.cannonPowerMax)
                {
                    int bytesize = 0;
                    char buffer[1024]={0};
                    ::dntgfish::CMD_S_AddLevel addcanno;
...
bool CTableFrameSink::LoadConfig()
{
    ...
    sprintf(file_name, "conf/dntgfish_config_%u.xml", m_pITableFrame->GetGameRoomInfo()->roomId);
...
bool CTableFrameSink::ReadConfigInformation()
{
    //设置文件名
    //=====config=====
    if(!boost::filesystem::exists("./conf/fish_dn.ini"))
    {
        LOG_INFO << "./conf/fish_dn.ini not exists";
        return false;
    }

    boost::property_tree::ptree pt;
    boost::property_tree::read_ini("./conf/fish_dn.ini", pt);
...
void CTableFrameSink::SetGameRecordDetail(int64_t userid, int32_t chairId, int64_t userScore, int64_t winScore, PlayLog playlog)
{
	...
	::dntgfish::CMD_S_GameRecordDetail details;
	...
	for (auto &iter : _fishRecords)
	{
		...
		// 结算时记录
		::dntgfish::FishRecordDetail* detail = details.add_detail();
		//鱼类型
