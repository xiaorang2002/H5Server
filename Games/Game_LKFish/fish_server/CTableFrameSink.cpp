...
#else
        dir += "/lkfish";
    google::InitGoogleLogging("lkfish");
#endif//__HJSDS__
        FLAGS_log_dir = dir;
...
void CTableFrameSink::OnTimerSynchor()
{
	...
	{
		OnTimerSynchorScore();
		m_TimerIdSynchorScore = m_TimerLoopThread->getLoop()->runAfter(30, boost::bind(&CTableFrameSink::OnTimerSynchor, this));
	}
}
        ...
        for(int i=0;i<MAX_PLAYERS;i++)
        {
            if(m_pITableFrame->GetTableUserItem(i))
            {
                ...
                if(cmdData.powerlevel>=game_config.cannonPowerMin&&cmdData.powerlevel<=game_config.cannonPowerMax)
                {
                    int bytesize = 0;
                    char buffer[1024]={0};
                    ::lkfish::CMD_S_AddLevel addcanno;
...
bool CTableFrameSink::LoadConfig()
{
    ...
    sprintf(file_name, "conf/lkfish_config_%u.xml", m_pITableFrame->GetGameRoomInfo()->roomId);
...
bool CTableFrameSink::OnTimerSceneEnd(WPARAM bind_param, unsigned char* buffer, unsigned short bufferSize)
{
	...
	m_TimerLoopThread->getLoop()->cancel(m_TimerIdSynchorScore);
    m_TimerIdSynchorScore = m_TimerLoopThread->getLoop()->runAfter(kGlobalTimerSynchorScore/1000,boost::bind(&CTableFrameSink::OnTimerSynchor, this));
...
bool CTableFrameSink::ReadConfigInformation()
{
    //设置文件名
    //=====config=====
    if(!boost::filesystem::exists("./conf/fish_lk.ini"))
    {
        LOG_INFO << "./conf/fish_lk.ini not exists";
        return false;
    }

    boost::property_tree::ptree pt;
    boost::property_tree::read_ini("./conf/fish_lk.ini", pt);
...
void CTableFrameSink::writeUserScore(WORD chair_id, double score)
{
    ...
    if(score_info.betScore>0)
    {
        ...
        {
           res= score_info.addScore*(1000.0f-m_pITableFrame->GetGameRoomInfo()->systemReduceRatio)/1000.0f;
        }
        else
        {
...
void CTableFrameSink::SetGameRecordDetail(int64_t userid, int32_t chairId, int64_t userScore, int64_t winScore, PlayLog playlog)
{
	...
	::lkfish::CMD_S_GameRecordDetail details;
	...
	for (auto &iter : _fishRecords)
	{
		...
		// 结算时记录
		::lkfish::FishRecordDetail* detail = details.add_detail();
		//鱼类型
