...
#include "../Game_QZXszSgLandy/CMD_Game.h"
...
//    uint32_t chairId = m_pAndroidUserItem->GetChairId();
    openLog("随机叫庄 i=%d cbCallBankerMul=%d",m_wChairID,(int)cbCallBankerMul);

    XSG_CMD_C_CallBanker callBanker;
    callBanker.set_cbcallflag(cbCallBankerMul);

    //发送信息
    string content = callBanker.SerializeAsString();
    m_pTableFrame->OnGameEvent(m_wChairID,XSG_SUB_C_CALL_BANKER, (uint8_t*)content.data(), content.size());
    LOG(INFO)<<"******************************Send XSG_SUB_C_CALL_BANKER";
...
//     m_pTableFrame->OnGameEvent(m_wChairID,XSG_SUB_C_ADD_SCORE, (uint8_t*)content.data(), content.size());
//     LOG(INFO)<<"******************************Send XSG_SUB_C_ADD_SCORE";
...
void CAndroidUserItemSink::GameTimeOpencard()
{
    ...
    XSG_CMD_C_OpenCard openCard;

    int cbOpenIndex = 0;
    cbOpenIndex = rand()%2; //随机开牌(0:推荐 1:摊牌)
    openCard.set_cbopenflag(cbOpenIndex);
    uint8_t cbdata[MAX_COUNT] = {0};
    int id = 0;
    if (cbOpenIndex==0)
    {
        for (int i = 0; i < MAX_COUNT; ++i)
        {
            openCard.add_cbchoosecard(m_cardData[i]);
            cbdata[id++] = m_cardData[i];
        }        
    }
    else
    {
        int randn = rand()%4;
        for (int i = 0; i < MAX_SENDCOUNT; ++i)
        {
            if(i==randn) continue;
            openCard.add_cbchoosecard(m_HandcardData[i]);
            cbdata[id++] = m_HandcardData[i];
        }
    }
    openLog("随机开牌 i=%d cbOpenIndex=%d cardData=0x%x,0x%x,0x%x",m_wChairID,cbOpenIndex,cbdata[0],cbdata[1],cbdata[2]);

    string content = openCard.SerializeAsString();
    m_pTableFrame->OnGameEvent(m_wChairID,XSG_SUB_C_OPEN_CARD, (uint8_t*)content.data(), content.size());
    // LOG(INFO)<<"******************************Send XSG_CMD_C_OpenCard";
    // LOG(INFO)<<"******************************cbJettonIndex"<<(int)cbJettonIndex;
    // m_pTableFrame->OnGameEvent(m_wChairID, XSG_SUB_C_OPEN_CARD, NULL, 0);
}
...
bool CAndroidUserItemSink::OnGameMessage(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    switch (wSubCmdID)
    {
    case XSG_SUB_S_GAME_START_AI:
    {
        QZXszSg::CMD_S_GameStartAi GameStartAi;
        if (!GameStartAi.ParseFromArray(pData,wDataSize))
            return false;
        m_cbSGCardType = GameStartAi.cbsgcarddata();
        m_isMaxCard = GameStartAi.ismaxcard();
        openLog("XSG_SUB_S_GAME_START_AI m_cbSGCardType=%d m_isMaxCard=%d",m_cbSGCardType,m_isMaxCard);
        m_m_cbPlayStatus = true;
        return true;
    }
    case XSG_SUB_S_GAME_START:
    {
        ClearAndroidCount=0;
        m_m_cbPlayStatus = true;
        ...
        break;
    case XSG_SUB_S_CALL_BANKER:
    {
        return OnSubCallBanker(wSubCmdID,pData, wDataSize);
    }
        break;
    case XSG_SUB_S_CALL_BANKER_RESULT:
    {
        return OnSubCallBankerResult(wSubCmdID,pData, wDataSize);
    }
        break;
    // case XSG_SUB_S_ADD_SCORE_RESULT:
    // {
    //     return OnSubAddScore(wSubCmdID,pData, wDataSize);
    // }
    //     break;
    case XSG_SUB_S_SEND_CARD:
    {
        return OnSubSendCard(wSubCmdID,pData, wDataSize);
    }
    case XSG_SUB_S_OPERATE_FAIL:
    {
        return OnSubOperateFail(wSubCmdID,pData, wDataSize);
    }
        break;
    case XSG_SUB_S_OPEN_CARD_RESULT:
    {
        return OnSubOpenCard(wSubCmdID,pData, wDataSize);
    }
        break;
    case XSG_SUB_S_GAME_END:
    {
        openLog("本局游戏结束:XSG_SUB_S_GAME_END m_cbSGCardType=%d m_isMaxCard=%d",m_cbSGCardType,m_isMaxCard);
...
{
    XSG_CMD_S_CallBankerResult BankerResult;
   ...
   // uint32_t currentChairid = m_pAndroidUserItem->GetChairId();
    ...
    // {
    //     //机器人不是庄
    //     int dwShowTime = 2000;//抢庄动画  3.0
    //     int opentime = BankerResult.cbopencardtime();
    //     int dwAddScore = dwShowTime + rand() % ((opentime - 1)* TIME_BASE);
    //     for(int i = 0; i < 5; ++i)
    //     {
...
// {
//     XSG_CMD_S_AddScoreResult AddScoreResult;
...
bool CAndroidUserItemSink::OnSubSendCard(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    ...
    if(!m_m_cbPlayStatus)
        return true;
    //玩家手牌
    for (int i = 0; i < SendCard.cbsendcard_size(); ++i)
    {
        m_HandcardData[i] = SendCard.cbsendcard(i);
    }
    openLog("OnSubSendCard 玩家手牌 i=%d HandcardData=0x%x,0x%x,0x%x,0x%x",m_wChairID,m_HandcardData[0],m_HandcardData[1],m_HandcardData[2],m_HandcardData[3]);
    //最优牌组合
    for (int i = 0; i < SendCard.cbsgcard_size(); ++i)
    {
         m_cardData[i] = SendCard.cbsgcard(i);
    }
    openLog("OnSubSendCard 最优牌组合牌 i=%d HandcardData=0x%x,0x%x,0x%x,0x%x",m_wChairID,m_cardData[0],m_cardData[1],m_cardData[2]);
   // int dwShowTime = 3300;//抢庄动画  3.0
    int dwShowTime = 2000;//抢庄和发牌动画 3.0
...
bool CAndroidUserItemSink::OnSubOperateFail(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    XSG_CMD_S_OperateFail OperateFail;
    if (!OperateFail.ParseFromArray(pData,wDataSize))
        return false;

    if (OperateFail.cbreason() == 0) //不是最优牌型
    {
        XSG_CMD_C_OpenCard openCard;

        int cbOpenIndex = 0; //随机开牌(0:推荐 1:摊牌)
        openCard.set_cbopenflag(cbOpenIndex);
        uint8_t cbdata[MAX_COUNT] = {0};
        int id = 0;
        for (int i = 0; i < MAX_COUNT; ++i)
        ...
        openLog("选择失败不是最优牌型,返回推荐牌组合 i=%d cardData=0x%x,0x%x,0x%x",m_wChairID,cbdata[0],cbdata[1],cbdata[2]);

        string content = openCard.SerializeAsString();
        m_pTableFrame->OnGameEvent(m_wChairID,XSG_SUB_C_OPEN_CARD, (uint8_t*)content.data(), content.size());
        LOG(INFO)<<"******************************OnSubOperateFail Send XSG_CMD_C_OpenCard";
    }
...
bool CAndroidUserItemSink::ReadConfigInformation()
{
    ...
    //设置文件名
    //=====config=====
    if(!boost::filesystem::exists("./conf/qzxsg_test_config.ini"))
    {
        LOG_INFO << "conf/qzxsg_test_config.ini not exists";
        return false;
    }

    boost::property_tree::ptree pt;
    boost::property_tree::read_ini("./conf/qzxsg_test_config.ini", pt);
...
void  CAndroidUserItemSink::openLog(const char *p, ...)
{
    ...
    sprintf(Filename, "log//QZXsangong//QZXsangong_%d%d%d_%d_%d.log",t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,m_pTableFrame->GetGameRoomInfo()->roomId,m_pTableFrame->GetTableId());
