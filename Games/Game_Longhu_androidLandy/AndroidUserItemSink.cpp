...
//         srand((int)time(0));
    ...
    //和区域下注不能大于10
    if(bJettonArea == 1 && JettonScore > m_currentChips[1])
    {
        if (rand() % 100 < 50)
         {
    ...
    if( bJettonArea < 1 && bJettonArea > 3)
        bJettonArea = 2;
    ...
    if (JettonScore < m_currentChips[0])
        int kk=0;
...
bool CAndroidUserItemSink::OnEventSceneMessage(uint8_t cbGameStatus, const uint8_t * pData,  uint32_t size)
{
    ...
    switch(cbGameStatus)
	{
        ...
        case SUB_S_START_PLACE_JETTON:		//游戏状态
        {
            ...
            const ::google::protobuf::RepeatedPtrField<::Longhu::PlayerListItem>& player = deskinfo.players();
            const ::google::protobuf::RepeatedField<::google::protobuf::int64>& chips = deskinfo.userchips();
            for(int i = 0;i < chips.size(); ++i)
            {
                m_currentChips[i]=chips[i];
