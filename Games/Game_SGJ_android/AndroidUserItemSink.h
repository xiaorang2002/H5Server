...
private:
	...
	bool CmdRound();
	// 
	int RandomChouma(int nLen,int32_t nArray[]);
...
public:    
	...
	int m_nRobotBankStorageMul;			//存款倍数
										//机器人行为控制
public:
	int32_t m_AreaProbability[8];		 //区域下注权重
	int64_t m_AreaRobotAddGold[8];	//游戏区域最多下注金额
	int32_t m_CurrentRobotAddGold[8]; //当前机器人下注
	int m_ProbabilityWeight[6];		 //可下注百分比权重
	int32_t m_AddMaxGold[5];		 //最大下注筹码
	int m_ChouMaWeight[9];			 //筹码权重
	int32_t m_RobotAddGold[8];		 //机器人龙虎和区域下注
	...
	int32_t		m_MaxAndroidBetInterval;    //机器人随机押注时间(在两者之间)
    int32_t		m_MinAndroidBetInterval;    //最大机器人数量
};
