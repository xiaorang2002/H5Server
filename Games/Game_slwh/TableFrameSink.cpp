...
#include "proto/slwh.Message.pb.h"
...
TableFrameSink::TableFrameSink()
{
	...
	int32_t weight[BET_ITEM_COUNT] = { 1149,702,401,205,1315,828,460,235,2280,1295,760,370,0,0,0 };
	for (int i = 0;i < BET_ITEM_COUNT;i++)
	{
	...
	int pribility = 0;
	int res = 0;	
	int32_t weight_hzx[3] = { 100,450,450 };
	int32_t weight_openType[5] = { 900,35,35,15,15 };
	int hzxIndex = 0;
	int openType = 0;
	int idxsd = -1;
	for (int i = 0; i < RES_RECORD_COUNT; i++)
	{
		...
		for (int j = 0;j < BET_ITEM_COUNT - 3;j++)
		{
			res += weight[j];
			if (pribility - res <= 0)
			{
		...
		// 和庄闲
		res = 0;
		pribility = m_random.betweenInt(0, 1000).randInt_mt(true);
		for (int j = 0;j < 3;j++)
		{
			res += weight_hzx[j];
			if (pribility - res <= 0)
		...
		//开奖类型 0:正常开奖 1:霹雳闪电 2:送灯 3:大三元 4:大四喜
		res = 0;
		pribility = m_random.betweenInt(0, 1000).randInt_mt(true);
		for (int j = 0;j < 5;j++)
		{
			res += weight_openType[j];
			if (pribility - res <= 0)
			{
				openType = j;
				break;
		...
		if (openType == 2)
		{
			do 
			{
				idxsd = m_random.betweenInt(0, 11).randInt_mt(true);
			} while (idxsd == iAnimalId);
		}
		tagResultInfo resultInfo;
		resultInfo.iAnimalId = iAnimalId;
		resultInfo.iAnimalId_sd = idxsd;
		resultInfo.iHZXId = hzxIndex;
		resultInfo.iOpenType = openType;

		m_vResultRecored.push_back(resultInfo);
		updateResultList(resultInfo);
	}
	...
	m_testOpenType = -1;
	m_testResultOpenType = -1;
	m_testResultOpenType = 0;
    m_lOpenResultCount = 0;
	m_lOpenResultAllCount = 20;
    m_bTest = 0;
    m_nTestTimes = 0;
    m_bWritelog = false;
    m_nTestType = 0;
	...
	m_iStopColourIndex = 0;
	m_iStopColourIndex_sd = 0;
	m_iChooseMuilt = -1;
	memset(m_nAnimalCircle, 0, sizeof(m_nAnimalCircle));
	memset(m_nColourCircle, 0, sizeof(m_nColourCircle));
	memset(m_iColourCount, 0, sizeof(m_iColourCount));
	memset(m_nAllMuilt, 0, sizeof(m_nAllMuilt));	
	m_iResultHZXId = 13;
	m_iResultTypeTag = 0;
	m_iResultMul_hzx = 0;
    memset(m_iResultId_tmp, 0, sizeof(m_iResultId_tmp));
	memset(m_lOpenTypeCount, 0, sizeof(m_lOpenTypeCount));
	m_bHaveColWeight = false;
	memset(m_TmpRetWeight, 0, sizeof(m_TmpRetWeight));
	memset(m_TmpnMuilt, 0, sizeof(m_TmpnMuilt));
	memset(m_fAverageMuilt, 0.0f, sizeof(m_fAverageMuilt));
	memset(m_HzxRetWeight, 0, sizeof(m_HzxRetWeight));

	m_lastStopIndex = 0;
	m_lastResultHZXId = 12;

	//测试变量
	memset(m_testdAreaJetton, 0, sizeof(m_testdAreaJetton));
	m_testTotalIn = 0;
	m_testdAllOut = 0;
	m_TotalIn = 0;
	m_TotalOut = 0;
...
{ 
	...
	int64_t userScore = m_pPlayer->GetUserScore();
    for(int i = 0;i < CHIP_COUNT;i++)
    {
       if(userScore >= m_userScoreLevel[0] && userScore < m_userScoreLevel[1])
       {
           m_currentchips[wCharid][i]=m_userChips[0][i];
       }
       else if(userScore >= m_userScoreLevel[1] && userScore < m_userScoreLevel[2])
       {
           m_currentchips[wCharid][i]=m_userChips[1][i];
       }
       else if(userScore >= m_userScoreLevel[2] && userScore < m_userScoreLevel[3])
       {
    ...
    { 
        ...
        // 修改miGameSceneState
        m_iGameState = slwh::SUB_S_GameStart; 
        // 启动定时器
        GamefirstInGame();
		...
		{
			m_LoopThread->getLoop()->cancel(m_TimerIdTest);
			m_TimerIdTest = m_LoopThread->getLoop()->runEvery(m_fTestTime, boost::bind(&TableFrameSink::TestGameMessage, this, wCharid, m_nTestType));
    ...
    switch (m_iGameState)
    {
    case slwh::SUB_S_GameStart:
    {
        SendGameSceneStart(userid, false);
        break;
    }
    case slwh::SUB_S_GameEnd:
    {
...
void TableFrameSink::GameTimerStart()
{
	...
	{
		// 刷新动物圈排序
		int nAinmalID[CIRCLE_COUNT] = { 0 };
		int index = 0;
		for (int i = 0;i < CIRCLE_COUNT;++i)
		{
			if (m_iResultTypeTag == (int)OpenType::eSongDeng)
			{
				index = (i + CIRCLE_COUNT + m_iStopColourIndex_sd - m_iStopIndex_sd) % CIRCLE_COUNT;
			}
			else
			{
				index = (i + CIRCLE_COUNT + m_iStopColourIndex - m_iStopIndex) % CIRCLE_COUNT;
			}
			nAinmalID[index] = m_nAnimalCircle[i];
		}
		memcpy(m_nAnimalCircle, nAinmalID, sizeof(m_nAnimalCircle));
	}
    ...
    // 设置游戏状态
    m_iGameState = slwh::SUB_S_GameStart;
    m_pITableFrame->SetGameStatus(slwh::SUB_S_GameStart);
	m_bGameEnd = false;
    m_timetick = 0;
    float betCountDown = m_betCountDown + m_fGameStartAniTime; //1s,预留前端的开始动画
    m_timetickAll = betCountDown;
    ...
    if (m_bIsContinue)
    {
        ...
        m_LoopThread->getLoop()->cancel(m_TimerIdEnd);
		/*if (m_bTest > 0)
			m_TimerIdEnd = m_LoopThread->getLoop()->runAfter(m_fTestTime, boost::bind(&TableFrameSink::GameTimerEnd, this));
		else*/
...
void TableFrameSink::GameTimerEnd()
{
    ...
    // 更新状态
    m_iGameState = slwh::SUB_S_GameEnd;
    m_pITableFrame->SetGameStatus(slwh::SUB_S_GameEnd);
    m_timetick = 0;
    ...
    LOG(WARNING) << "--- 启动结算定时器---"; 	
	/*if (m_bTest > 0)
		m_TimerIdStart = m_LoopThread->getLoop()->runAfter(m_fTestTime + m_fTestTime/2, CALLBACK_0(TableFrameSink::GameTimerStart, this));
	else*/
	{
		m_TimerIdStart = m_LoopThread->getLoop()->runAfter(endCountDown, CALLBACK_0(TableFrameSink::GameTimerStart, this));
		m_TimerIdResetPlayerList = m_LoopThread->getLoop()->runAfter(m_fResetPlayerTime + addTime, CALLBACK_0(TableFrameSink::RefreshPlayerList, this, true));
	}
    // 更新配置的频率
    // if (m_bTest==0)
    {
	...
	openSLog("本局游戏结束;\n");
	//if (m_bTest > 0 && m_lTestOpenCount >= TEST_ALLCOUNT)
	//{
	//	//m_bTest = 0;
	//	m_lTestOpenCount = 0;
	//	m_testTimes++;
	...
	//		memset(m_testdAreaJetton, 0, sizeof(m_testdAreaJetton));
	//		//大三元
	//		if (m_testBetIndex >= 4) m_testBetIndex = 0;
	//		m_testdAreaJetton[m_testBetIndex] = 10000;
	//		m_testdAreaJetton[m_testBetIndex + 4] = 10000;
	//		m_testdAreaJetton[m_testBetIndex + 8] = 10000;
	//		//大四喜
	//		/*if (m_testBetIndex >= 3) m_testBetIndex = 0;
	//		for (int i = m_testBetIndex * 4;i < m_testBetIndex * 4 + 4;i++)
	//		{
	//			m_testdAreaJetton[i] = 10000;
	//		}*/
	//		//送灯
	//		/*if (m_testBetIndex >= 6) m_testBetIndex = 0;
	//		m_testdAreaJetton[m_testBetIndex * 2] = 10000;
	//		m_testdAreaJetton[m_testBetIndex * 2 + 1] = 10000;*/
	//		//单项、霹雳闪电
	//		/*if (m_testBetIndex >= 12) m_testBetIndex = 0;
	//		m_testdAreaJetton[m_testBetIndex] = 10000;*/
	//	}
	//}
...
{
    // 游戏状态判断
    if (slwh::SUB_S_GameStart != m_iGameState)
    {
       return;
    }
    int betTimes = 0;
    ...
    {                
        ...
        slwh::CMD_C_PlaceJet mPlaceJet;
        mPlaceJet.set_cbjettonarea(bJettonArea);
        mPlaceJet.set_ljettonscore(JettonScore);

        string content = mPlaceJet.SerializeAsString();
        OnGameMessage(wChairID, slwh::SUB_C_USER_JETTON, (uint8_t *)content.data(), content.size());
    }
    while(tmpbetAllScore>0);*/
	int32_t bJettonArea = 0;
	//大三元
	m_testdAreaJetton[m_testBetIndex] = 10000;
	m_testdAreaJetton[m_testBetIndex + 4] = 10000;
	m_testdAreaJetton[m_testBetIndex + 8] = 10000;
	/*m_testdAreaJetton[2] = 10000;
	m_testdAreaJetton[6] = 10000;
	m_testdAreaJetton[10] = 10000;*/
	/*m_testdAreaJetton[1] = 10000;
	m_testdAreaJetton[5] = 10000;
	m_testdAreaJetton[9] = 10000;*/
	/*m_testdAreaJetton[0] = 10000;
	m_testdAreaJetton[4] = 10000;
	m_testdAreaJetton[8] = 10000;*/
	//大四喜
	/*for (int i = m_testBetIndex*4;i < m_testBetIndex*4 + 4;i++)
	{
		m_testdAreaJetton[i] = 10000;
		LOG(WARNING) << "----下注金额---" << m_testdAreaJetton[i] << " 区域;" << i;
	}*/
	//送灯
	/*m_testdAreaJetton[m_testBetIndex * 2] = 10000;
	m_testdAreaJetton[m_testBetIndex * 2 + 1] = 10000;*/
...
{ 
    ...
    slwh::CMD_C_CurState curStateReq;
    ...
    slwh::CMD_S_CurState curStateRsp;
    ...
    curStateRsp.set_nstopidx(m_iStopIndex); 
    curStateRsp.set_ncurstate((int)(slwh::SUB_S_GameStart != m_iGameState)); //当前状态
    ...
    //curStateRsp.set_nlastretid(m_lastResultInfo);
	slwh::GameResultInfo *lastResultInfo = curStateRsp.add_nlastresultinfo();
	lastResultInfo->set_ihzxid(m_lastResultInfo.iHZXId);
	lastResultInfo->set_ianimalid(m_lastResultInfo.iAnimalId);
	lastResultInfo->set_ianimalid_sd(m_lastResultInfo.iAnimalId_sd);
	lastResultInfo->set_iopentype(m_lastResultInfo.iOpenType);

    //记录
	int recordCount = 0;
    for (int i = m_vResultRecored.size() - 1; i >= 0; i--){
		slwh::GameResultInfo *resultInfo = curStateRsp.add_resultinfos();
		resultInfo->set_ihzxid(m_vResultRecored[i].iHZXId);
		resultInfo->set_ianimalid(m_vResultRecored[i].iAnimalId);
		resultInfo->set_ianimalid_sd(m_vResultRecored[i].iAnimalId_sd);
		resultInfo->set_iopentype(m_vResultRecored[i].iOpenType);
		recordCount++;
		if (recordCount >= 11) break;
    }

    LOG(ERROR) << "查询当前游戏状态 =>UserScore=" << player->GetUserScore() << ",betCount= "<< betCount << ",pUserRealMoney" << m_UserInfo[player->GetUserId()]->RealMoney << ",userid= " << userid;
    if(slwh::SUB_S_GameStart == m_iGameState)
    {
    ...
    else
    {
		...
		curStateRsp.set_hzxidex(m_iResultHZXId);
		curStateRsp.set_colourindex(m_iStopColourIndex);
		if (m_iResultTypeTag == (int)OpenType::eSongDeng)
		{
			curStateRsp.set_colourindex_sd(m_iStopColourIndex_sd);
			curStateRsp.set_cartype_sd(m_iResultId_sd);
			curStateRsp.set_winindex_sd(m_iStopIndex_sd);
		}
		curStateRsp.set_iopentype(m_iResultTypeTag);
		curStateRsp.set_winindex(m_iResultId);
		curStateRsp.set_cbopentime(m_endCountDown);
    }
    curStateRsp.set_userwinscore(m_UserInfo[player->GetUserId()]->RealMoney);

	for (int j = 0; j < CIRCLE_COUNT; j++)
	{
        curStateRsp.add_nanimalcircle(m_nAnimalCircle[j]);
        curStateRsp.add_ncolourcircle(m_nColourCircle[j]);
	}
	vector<shared_ptr<OpenResultInfo>> vResultList;
	...
	curStateRsp.set_laststopindex(m_lastStopIndex);
	curStateRsp.set_lastresulthzxid(m_lastResultHZXId);

    std::string sendData = curStateRsp.SerializeAsString();
    m_pITableFrame->SendTableData(chairid, slwh::SUB_S_QUERY_CUR_STATE, (uint8_t *)sendData.c_str(), sendData.size());
}
...
void TableFrameSink::QueryGameRecord(int32_t chairid, uint8_t subid, const uint8_t *data, uint32_t datasize)
{
	...
	slwh::CMD_C_GameRecord gameRecordReq;
...
{ 
    ...
    string strRet = str(boost::format("%d:%d:%d-%d") % m_iResultHZXId % m_iResultTypeTag % m_iResultId % m_iRetMul);
	...
	{		
		//送灯时赠送的动物
		strRet2 = str(boost::format("|%d-%d") % m_iResultId_sd % m_nMuilt[m_iResultId_sd]);
		winAllMul += m_nMuilt[m_iResultId_sd];
		//m_replay.addStep((uint32_t)time(nullptr) - m_dwReadyTime, strRet2, -1, opUnkonw0, -1, -1);
		strRet += strRet2;
	}
	else if (m_iResultTypeTag == (int)OpenType::eDaSanYuan)
	{
		int animID = m_iResultId % 4;
		int resultID = 0;
		for (int i = 0;i < 3;++i)
		{
			resultID = animID + i * 4;
			if (resultID == m_iResultId) continue;
			strRet2 = str(boost::format("|%d-%d") % resultID % m_nMuilt[resultID]);
			strRet += strRet2;
		}		
	} 
	else if (m_iResultTypeTag == (int)OpenType::eDaSiXi)
	{
		int animID = m_iResultId / 4;
		int resultID = 0;
		for (int i = 0;i < 4;++i)
	...
	LOG(WARNING) << "本局开奖结果记录 =>" << strRet;
	openSLog("本局开奖结果记录:%s", strRet.c_str());
    ...
    for (auto &it : m_UserInfo)
    {
        ...
        for(int betIdx = 0;betIdx < BET_ITEM_COUNT; betIdx++){  
			...
			if (m_iResultHZXId == (int)AnimType::eAreaHe)
			{
				if (betIdx != (int)AnimType::eAreaZhuang && betIdx != (int)AnimType::eAreaXian)
				{
			...
			{
				if (betIdx <= (int)AnimType::eAreaHe)
				{
					youxiaotouzhu += betScore;
				}
				else if(!bChoose)
				{
					int zhuang = (int)AnimType::eAreaZhuang;
					int xian = (int)AnimType::eAreaXian;
        ...
        scoreInfo.rWinScore = youxiaotouzhu; //开和时,不算庄闲的有效投注
        //牌型
...
{
    CINIHelp *pini = new CINIHelp("conf/slwh_config.ini");

    //int32_t odds[BET_ITEM_COUNT] = {6,8,8,12,12,8,8,6,24,99,2,2};
	int32_t weight[BET_ITEM_COUNT + 1] = { 1149,702,401,205,1315,828,460,235,2280,1295,760,370,0,0,0,0 };
	int32_t retWeight[BET_ITEM_COUNT + 1] = { 1149,702,401,205,1315,828,460,235,2280,1295,760,370,0,0,0,0 };
	float fAverageMuilt[5] = { 0.0f,4.0f,3.6f,6.0f,8.0f };
    ...
    m_dControlkill = pini->GetFloat("GAME_CONFIG", "Controlkill", 0.8);
    m_lLimitPoints = pini->GetLong("GAME_CONFIG", "LimitPoint", 500000);
    char buf[32] = { 0 };
    for(int i = 0;i<BET_ITEM_COUNT;i++){
        // 押分区最大押分值
        sprintf(buf, "MaxJetton%d",i);
        m_nAreaMaxJetton[i] = COIN_RATE * pini->GetLong("GAME_AREA_MAX_JETTON", buf, 10000);
        assert(m_nAreaMaxJetton[i]>0);
        if(m_nAreaMaxJetton[i]==0)
            m_nAreaMaxJetton[i] = nAreaMaxJetton[i];
		//LOG(WARNING) << "--- ReadInI--- " << m_nMuilt[i] << " " << m_nAreaMaxJetton[i];
    }
	for (int i = 0;i < BET_ITEM_COUNT + 1;i++) {
		// 开奖权重 
		sprintf(buf, "Ret%d", i);
		...
		//LOG(WARNING) << "--- ReadInI---  weight " << weight[i] << " " << m_retWeight[i];
	}
    ...
    m_fTaxRate = pini->GetFloat("GAME_CONFIG", "REVENUE", 0.05);
	...
	m_testResultOpenType = pini->GetFloat("TEST", "TestOpenType",0);
	m_testResultInex_zx = pini->GetFloat("TEST", "TestIndex_ZX", 0);
	...
	if (!m_bIsOpen)
	{
		std::string str1(pini->GetString("GAME_ANIMAL_CIRCLE_LIST", "ANIMAL_CIRCLE_LIST"));
		int j = 0, p;
		while (!str1.empty()) {
			p = str1.find_first_of('|');
			if (p == -1) {
				if (!str1.empty()) {
					m_nAnimalCircle[j++] = atoi(str1.c_str());
				}
				break;
			}
			m_nAnimalCircle[j++] = atoi(str1.substr(0, p).c_str());
			str1 = str1.substr(p + 1, -1);
		}
	}
	for (int i = 0; i < 3; ++i) {
		//颜色圈数目
		strIndex = "ColorNum_" + to_string(i);
		std::string str1(pini->GetString("GAME_COLOUR_COUNT", strIndex.c_str()));
		int j = 0, p;
		while (!str1.empty()) {
			p = str1.find_first_of('|');
			if (p == -1) {
				if (!str1.empty()) {
					m_iColourCount[j++][i] = atoi(str1.c_str());
				}
				break;
			}
			m_iColourCount[j++][i] = atoi(str1.substr(0, p).c_str());
			str1 = str1.substr(p + 1, -1);
		}
	}
	int ntempMuilt[3][BET_ITEM_COUNT] = { 0 };
	for (int i = 0;i < BET_ITEM_COUNT;i++)
	{
		//赔率
		strIndex = "AREA_" + to_string(i);
		std::string str1(pini->GetString("GAME_MULTIPLIER", strIndex.c_str()));
		int j = 0, p;
		while (!str1.empty()) {
			p = str1.find_first_of('|');
			if (p == -1) {
				if (!str1.empty()) {
					m_nAllMuilt[j++][i] = atoi(str1.c_str());
				}
				break;
			}
			m_nAllMuilt[j++][i] = atoi(str1.substr(0, p).c_str());
			str1 = str1.substr(p + 1, -1);
		}
	}

	//设置特殊类型开奖权重是平均动物倍数的几倍
	for (int i = 0;i < 5;i++) 
	{
		sprintf(buf, "AvMuilt%d", i);
		m_fAverageMuilt[i] = pini->GetFloat("GAME_AVMUILT", buf, fAverageMuilt[i]);
		if (m_fAverageMuilt[i] == 0)
			m_fAverageMuilt[i] = fAverageMuilt[i];
		LOG(WARNING) << "--- ReadInI---  m_fAverageMuilt " << fAverageMuilt[i] << " " << m_fAverageMuilt[i];
	}
	
	//和庄闲开奖权重
	int32_t hzxRetWeight[3] = { 10,45,45 };
	for (int i = 0;i < 3;i++)
	{
		sprintf(buf, "HzxRet%d", i);
		m_HzxRetWeight[i] = pini->GetLong("GAME_HZXRET", buf, hzxRetWeight[i]);
		if (m_HzxRetWeight[i] == 0)
			m_HzxRetWeight[i] = hzxRetWeight[i];
	}
	m_lMustKillScore = pini->GetFloat("GAME_CONFIG", "MustKillScore", 0);
	m_lMustKillRate = pini->GetFloat("GAME_CONFIG", "MustKillRate", 500);
...
void TableFrameSink::NormalGameStart()
{
    ...
    for (auto &it : m_UserInfo)
    {
        ...
        slwh::CMD_S_GameStart gameStart;
		...
		for (int j = 0; j < CIRCLE_COUNT; j++)
		{
			gameStart.add_nanimalcircle(m_nAnimalCircle[j]);
			gameStart.add_ncolourcircle(m_nColourCircle[j]);
		}
		vector<shared_ptr<OpenResultInfo>> vResultList;
		vResultList = m_vOpenResultListCount_last[0];
		int32_t openCount = vResultList.size();
		...
		gameStart.set_laststopindex(m_lastStopIndex);
		gameStart.set_lastresulthzxid(m_lastResultHZXId);
		...
		for (auto &uInfoItem : m_pPlayerInfoList)
		{
            ...
            slwh::PlayerListItem* palyer = gameStart.add_players();
		...
		*/
        ...
        m_pITableFrame->SendTableData(pUserItem->GetChairId(), slwh::SUB_S_GameStart, (uint8_t *)endData.c_str(), endData.size());
    }
}
...
void TableFrameSink::Algorithm()
{
    ...
    float cotrolWeight[BET_ITEM_COUNT + 1]={0};
	float cotrolWeightHzx[3] = { 0 };
    for(int i = 0;i < BET_ITEM_COUNT + 1;i++)
        cotrolWeight[i]=m_retWeight[i];
	for (int i = 0;i < 3;i++)
	{
		cotrolWeightHzx[i] = m_HzxRetWeight[i];
	}
    for(int i=0;i<MAX_PLAYER;i++)
    {
    ...
    slwhAlgorithm.SetSendOdds(m_iResultId_sd);
    int32_t randomRet=m_random.betweenInt(0,1000).randInt_mt(true);
    //难度值命中概率,全局难度,命中概率系统必须赚钱
    if(randomRet<m_difficulty)
    {
        slwhAlgorithm.SetAlgorithemNum(m_llStorageHighLimit ,m_llStorageLowerLimit, m_llStorageControl,m_dControlkill,m_nMuilt, m_lLimitPoints);//设置上下限,当前利润,干预难度,超红判断值
        slwhAlgorithm.SetMustKill(-1);
        for(int i=0;i<MAX_JETTON_AREA;i++)
         slwhAlgorithm.SetBetting(i,m_pBetArea->AndroidmifourAreajetton[i]);//设置每门真人押注值
        slwhAlgorithm.GetOpenCard(m_iResultId, cotrolWeight, m_iResultTypeTag, m_iResultId_tmp);
		slwhAlgorithm.GetOpenCard_HZX(m_iResultHZXId);
        openSLog("全局难度,正常算法最终出结果: %d, 开奖类型:%d,%d ",m_iResultId, m_iResultTypeTag, m_iResultHZXId);
    }
    else
    {
		//判断玩家是不是倍投庄闲
        ...
        {
            float probilityAll[BET_ITEM_COUNT + 1]={0.0f};
			float probilityHzx[3] = { 0.0f };
			bool isMustKill = false;
			bool anyCase = true;
			int64_t mustBeKillId = 0;
			int64_t mustBeKillBlackId = 0;
			bool isMustKillBlack = false;
            //user probilty count
            for(int i=0;i<MAX_PLAYER;i++)
            {
                shared_ptr<CServerUserItem> user=m_pITableFrame->GetTableUserItem(i);
                ...
                if(user->GetBlacklistInfo().status == 1 )
                {
					...
					int64_t allBetScoreHzx = 0;
                    for(int j=0;j< BET_ITEM_COUNT;j++)
                    {
						if (j < (int)AnimType::eAreaHe)
						{
							allWinscore += m_nMuilt[j] * m_pBetArea->AndroidmifourAreajetton[j];
						} 
						else
						{
							allBetScoreHzx += m_pBetArea->AndroidmifourAreajetton[j];
						}
                    }
                   for(int j=0;j < BET_ITEM_COUNT;j++)
                   {
					   if (j < (int)AnimType::eAreaHe)
					   {
						   if (allWinscore > 0)
						   {
							   ...
							   openSLog("黑名单玩家 :%d 押注:%d 确定调用黑名单算法 每门传的黑名单控制概率是[%d]：%f", user->GetUserId(), m_UserInfo[user->GetUserId()]->dAreaJetton[j], j, (gailv*(float)m_UserInfo[user->GetUserId()]->dAreaJetton[j] * m_nMuilt[j]) / (float)allWinscore);
						   }
					   } 
					   else
					   {
						   if (allBetScoreHzx > 0)
						   {
							   probilityHzx[j - 12] += (gailv*(float)m_UserInfo[user->GetUserId()]->dAreaJetton[j]) / (float)allBetScoreHzx;
							   openSLog("黑名单玩家 :%d 押注:%d 确定调用黑名单算法 和庄闲传的黑名单控制概率是[%d]：%f", user->GetUserId(), m_UserInfo[user->GetUserId()]->dAreaJetton[j], j, (gailv*(float)m_UserInfo[user->GetUserId()]->dAreaJetton[j]) / (float)allBetScoreHzx);
						   }
					   }
                   }
				   //既是黑名单，又是倍投玩家
				   if (!isMustKill && m_UserInfo[user->GetUserId()]->isDoubleCast)
				   {
					   isMustKill = true;
					   anyCase = false;
					   mustBeKillId = user->GetUserId();
					   LOG(ERROR) << "开始既是黑名单，又是倍投判断 ";
					   openSLog("开始既是黑名单，又是倍投判断 mustBeKillId: %d ", mustBeKillId);
				   }
				   if (!isMustKill && shuzhi == 1000)
				   {
					   isMustKill = true;
					   anyCase = true;
					   mustBeKillId = user->GetUserId();
					   openSLog("点杀黑名单玩家 mustBeKillId: %d ", mustBeKillId);
				   }
				   //有必杀黑名单玩家
				   if (!isMustKillBlack && shuzhi == 1000)
				   {
					   isMustKillBlack = true;
					   mustBeKillBlackId = user->GetUserId();
					   openSLog("有必杀黑名单玩家 mustBeKillBlackId: %d ", mustBeKillBlackId);
				   }
                }
            }
            int64_t allWin=0;
            ...
            for(int i=0;i< BET_ITEM_COUNT;i++)
            {
				...
				/*else
				{
					if (probilityHzx[i-12] > 0.0001)
					{
						float weight = 0.0;
						weight = m_HzxRetWeight[i-12] * (1 - probilityHzx[i - 12]);
						cotrolWeightHzx[i-12] = weight;
					}
					else
					{
						cotrolWeightHzx[i-12] = m_HzxRetWeight[i-12];
					}
				}*/
            }
            slwhAlgorithm.SetAlgorithemNum(m_llStorageHighLimit ,m_llStorageLowerLimit, m_llStorageControl,m_dControlkill,m_nMuilt, m_lLimitPoints, m_stockPercentRate, m_lMustKillScore);//设置上下限,当前利润,干预难度,超红判断值
            for(int i=0;i<MAX_JETTON_AREA;i++)
             slwhAlgorithm.SetBetting(i,m_pBetArea->AndroidmifourAreajetton[i]);//设置每门真人押注值

			// 动物结果
			int randnum = m_random.betweenInt(0, 1000).randInt_mt(true);
			if (isMustKillBlack && randnum <= m_lMustKillRate)
			{
				bool bKillSeccse = slwhAlgorithm.KillBlackGetOpenCard(m_iResultId, m_iResultTypeTag, m_UserInfo[mustBeKillBlackId]->dAreaJetton);
				if (!bKillSeccse)
				{
					slwhAlgorithm.BlackGetOpenCard(m_iResultId, cotrolWeight, m_iResultTypeTag, m_iResultId_tmp);
				}
			}
			else
			{
			...
			if (isMustKill)
			{
				slwhAlgorithm.BlackGetMustkillOpenCard_HZX(m_iResultHZXId, probilityHzx, m_UserInfo[mustBeKillId]->dAreaJetton, anyCase);
			}
			else
			{
				slwhAlgorithm.BlackGetOpenCard_HZX(m_iResultHZXId, probilityHzx);
			}
            
            openSLog("黑名单算法最终出结果: %d, 开奖类型:%d,%d ",m_iResultId, m_iResultTypeTag, m_iResultHZXId);
        }
        else
        {
            slwhAlgorithm.SetAlgorithemNum(m_llStorageHighLimit ,m_llStorageLowerLimit, m_llStorageControl,m_dControlkill,m_nMuilt, m_lLimitPoints);//设置上下限,当前利润,干预难度,超红判断值
            ...
            slwhAlgorithm.GetOpenCard(m_iResultId, cotrolWeight, m_iResultTypeTag, m_iResultId_tmp);
			slwhAlgorithm.GetOpenCard_HZX(m_iResultHZXId);
            openSLog("正常算法最终出结果: %d, 开奖类型:%d,%d ",m_iResultId, m_iResultTypeTag, m_iResultHZXId);
        }
    }
	...
	{
		if (m_testResultInex >= 0 && m_testResultInex <= (int)AnimType::eYellowLion)
		{
		...
		{
			m_testInex++;
			if (m_testInex > (int)AnimType::eYellowLion)
			{
				m_testInex = 0;
			}
		}
		m_iResultId = m_testInex;
		if (m_testResultOpenType >= 0)
		{
			m_iResultTypeTag = m_testResultOpenType;
		} 
		else
		{
			m_testOpenType++;
			if (m_testOpenType > (int)OpenType::eDaSiXi)
			{
				m_testOpenType = 0;
			}
			m_iResultTypeTag = m_testOpenType;
		}
		
		if (m_iResultTypeTag == (int)OpenType::eSongDeng)
		{
			if (m_testResultInex_sd >= 0)
			{
			...
			else
			{
				...
				{
					m_testInex_sd++;
					if (m_testInex_sd > (int)AnimType::eYellowLion)
					{
						m_testInex_sd = 0;
					}
				} while (m_testInex_sd == m_iResultId);
				
			}
			m_iResultId_sd = m_testInex_sd;
		}
		//庄闲和
		if (m_testResultInex_zx > 0)
		{
            m_testInex_zx = m_testResultInex_zx;
		} 
		else
		{
            m_testInex_zx++;
        ...
        m_iResultHZXId = m_testInex_zx + 12;
	}
	...
	switch (m_iResultId)
	{
	case (int)AnimType::eRedRabbit: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "红兔子"; strm = "红兔子";}break;
	case (int)AnimType::eRedMonkey: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "红猴子"; strm = "红猴子";}break;
	case (int)AnimType::eRedPanda: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "红熊猫"; strm = "红熊猫";}break;
	case (int)AnimType::eRedLion: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "红狮子"; strm = "红狮子";}break;

	case (int)AnimType::eGreenRabbit: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "绿兔子"; strm = "绿兔子";}break;
	case (int)AnimType::eGreenMonkey: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "绿猴子"; strm = "绿猴子";}break;
	case (int)AnimType::eGreenPanda: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "绿熊猫"; strm = "绿熊猫";}break;
	case (int)AnimType::eGreenLion: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "绿狮子"; strm = "绿狮子";}break;

	case (int)AnimType::eYellowRabbit: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "黄兔子"; strm = "黄兔子";}break;
	case (int)AnimType::eYellowMonkey: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "黄猴子"; strm = "黄猴子";}break;
	case (int)AnimType::eYellowPanda: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "黄熊猫"; strm = "黄熊猫";}break;
	case (int)AnimType::eYellowLion: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex, m_iStopIndex_sd, m_iStopColourIndex_sd); LOG(WARNING) << "黄狮子"; strm = "黄狮子";}break;
	...
	if (m_iResultTypeTag == (int)OpenType::eSongDeng)
	{
		m_lastStopIndex = m_iStopColourIndex_sd;
	} 
	else
	{
		m_lastStopIndex = m_iStopColourIndex;
	}
	m_lastResultHZXId = m_iResultHZXId;
	...
	m_lTestOpenCount++;
	m_lOpenResultAllCount++;
	if (m_lOpenResultAllCount >= 10000000 && m_lOpenResultAllCount % 6 == 0)
	{
		m_lOpenResultAllCount = 120;
	}
	/*if (m_iResultTypeTag == (int)OpenType::eSongDeng)
	{
		m_lOpenResultIndexCount[m_iResultId_sd]++;
	} 
	else if (m_iResultTypeTag == (int)OpenType::eDaSanYuan)
	{
		int animID = m_iResultId % 4;
		...
		for (int i = 0;i < 3;++i)
		{
			resultID = animID + i * 4;
			if (resultID == m_iResultId) continue;
			m_lOpenResultIndexCount[resultID]++;
		}
	}
	else if (m_iResultTypeTag == (int)OpenType::eDaSiXi)
	{
	...
	m_iResultMul_hzx = m_nMuilt[m_iResultHZXId];
	m_lOpenResultIndexCount[m_iResultHZXId]++;
	m_lOpenTypeCount[m_iResultTypeTag]++;
	string strHZX = "";
	if (m_iResultHZXId == (int)AnimType::eAreaHe)
		strHZX = " 和 " + to_string(m_iResultMul_hzx) + "倍 ";
	else if(m_iResultHZXId == (int)AnimType::eAreaZhuang)
		strHZX = " 庄 " + to_string(m_iResultMul_hzx) + "倍 ";
	else if(m_iResultHZXId == (int)AnimType::eAreaXian)
		strHZX = " 闲 " + to_string(m_iResultMul_hzx) + "倍 ";	
	strm = strHZX + strm;
	LOG(WARNING) << "开奖倍率 " << m_iRetMul << " 开奖结果 " << m_iResultId << " 动物下标 " << m_iStopIndex << " 颜色下标 " << m_iStopColourIndex << strm << " 累计开奖次数 " << m_lOpenResultCount;
	openSLog("本局开奖结果:动物ID:%d,%s,动物下标:%d,颜色下标:%d,累计开奖次数:%d;", m_iResultId, strm.c_str(), m_iStopIndex, m_iStopColourIndex, m_lOpenResultCount);
	...
	for (int i = 0; i < 5; ++i)
	{
		openStr += to_string(i) + "_" + to_string(m_lOpenTypeCount[i]) + "  ";
	}
	LOG(WARNING) << openStr;
	openSLog("%s", openStr.c_str());
    if (m_lOpenResultCount>1000000 || m_lOpenResultIndexCount[m_iResultId]>10000000)
    {
        m_lOpenResultCount = 0;
		m_lTestOpenCount = 0;
...
void TableFrameSink::ArgorithResult()
{
    ...
    for (int index = 0; index < BET_ITEM_COUNT-3; index++)
    {
        // 押分 *  倍数 < 允许最大输分值
    ...
    if (rand()%100 < m_testRet)
    {
        if (m_testResultInex>=0 && m_testResultInex<=(int)AnimType::eYellowRabbit)
        {
            m_testInex = m_testResultInex;
        }
        else
        {
            m_testInex++;
            if (m_testInex>(int)AnimType::eYellowRabbit)
            {
                m_testInex=0;
            }
        }        
    ...
    switch (m_iResultId)
    {
		case (int)AnimType::eRedLion: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "红狮子"; strm = "红狮子";}break;
		case (int)AnimType::eRedPanda: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "红熊猫"; strm = "红熊猫";}break;
		case (int)AnimType::eRedMonkey: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "红猴子"; strm = "红猴子";}break;
		case (int)AnimType::eRedRabbit: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "红兔子"; strm = "红兔子";}break;

		case (int)AnimType::eGreenLion: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "绿狮子"; strm = "绿狮子";}break;
		case (int)AnimType::eGreenPanda: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "绿熊猫"; strm = "绿熊猫";}break;
		case (int)AnimType::eGreenMonkey: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "绿猴子"; strm = "绿猴子";}break;
		case (int)AnimType::eGreenRabbit: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "绿兔子"; strm = "绿兔子";}break;

		case (int)AnimType::eYellowLion: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "黄狮子"; strm = "黄狮子";}break;
		case (int)AnimType::eYellowPanda: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "黄熊猫"; strm = "黄熊猫";}break;
		case (int)AnimType::eYellowMonkey: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "黄猴子"; strm = "黄猴子";}break;
		case (int)AnimType::eYellowRabbit: { GetStopIndex(m_iResultId, m_iStopIndex, m_iStopColourIndex); LOG(WARNING) << "黄兔子"; strm = "黄兔子";}break;
    ...
    //         myprintf(szConfigFileName, sizeof(szConfigFileName), _T("%sconf/slwh_config.ini"), szPath);
...
void TableFrameSink::NormalCalculate()
{
    ...
    for (auto &it : m_UserInfo)
    {
        ...
        //设置玩家倍率
        userInfoItem->SetPlayerMuticl(m_iResultId, m_iRetMul, m_nMuilt, m_iResultId_sd, m_nMuilt[m_iResultId_sd], m_iResultTypeTag, m_iResultHZXId);
        //税收比例
	...
	m_bGameEnd = true;
	// 填充路单(长度9)
	m_lastResultInfo = m_vResultRecored[0];
	m_vResultRecored.erase(m_vResultRecored.begin());

	tagResultInfo ResultInfo;
	ResultInfo.iAnimalId = m_iResultId;
	ResultInfo.iAnimalId_sd = m_iResultId_sd;
	ResultInfo.iHZXId = m_iResultHZXId;
	ResultInfo.iOpenType = m_iResultTypeTag;
	m_vResultRecored.push_back(ResultInfo);
    ...
    for (auto &it : m_UserInfo)
    {
		...
		gameEnd.set_hzxidex(m_iResultHZXId);
		gameEnd.set_colourindex(m_iStopColourIndex);
		...
		if (m_iResultTypeTag == (int)OpenType::eSongDeng)
		{
			gameEnd.set_colourindex_sd(m_iStopColourIndex_sd);
			gameEnd.set_cartype_sd(m_iResultId_sd);
			gameEnd.set_winindex_sd(m_iStopIndex_sd);
		}
		gameEnd.set_iopentype(m_iResultTypeTag);
		gameEnd.set_cbopentime(m_endCountDown);
		/*
		i = 0;
        for (auto &uInfoItem : m_pPlayerInfoList)
		{
            ...
            slwh::PlayerListItem *palyer = gameEnd.add_players();
            palyer->set_dwuserid(uInfoItem->wUserid);
            palyer->set_headerid(uInfoItem->headerId);
			palyer->set_nviplevel(0);
            palyer->set_nickname(uInfoItem->nickName);
		...
		*/
        ...
        // if (m_bTest==0)
            m_pITableFrame->SendTableData(pUserItem->GetChairId(), slwh::SUB_S_GameEnd, (uint8_t *)endData.c_str(), endData.size());
	...
	for (auto &it : m_UserInfo)
	{
		...
		curJetton = userInfoItem->dAreaJetton[12] + userInfoItem->dAreaJetton[13] + userInfoItem->dAreaJetton[14];
		EndWinScore = userInfoItem->dAreaWin[12] + userInfoItem->dAreaWin[13] + userInfoItem->dAreaWin[14];
		//统计完以后把本次下注记录为玩家上次下注值
		pUserItem = m_pITableFrame->GetTableUserItem(userInfoItem->wCharid);
		if (!pUserItem || pUserItem->IsAndroidUser() || m_iResultHZXId == 12 || curJetton == 0)
			continue;
		userInfoItem->lLastJetton = curJetton;
		userInfoItem->lLastWinOrLost = EndWinScore >= 0 ? 1 : 0;
	...
	/*if (m_bTest > 0)
	{
		int64_t  TestWinScore = CalculatTestWinScore(m_iResultId, m_iResultTypeTag, m_iResultHZXId);
		m_testdAllOut += TestWinScore;
		m_TotalOut += TestWinScore;
		for (int i = 0;i < BET_ITEM_COUNT;i++)
		{
			m_testTotalIn += m_testdAreaJetton[i];
			m_TotalIn += m_testdAreaJetton[i];
		}
		int64_t winscore = (m_testTotalIn - m_testdAllOut);
		int64_t allWinScore = m_TotalIn - m_TotalOut;
		LOG(ERROR) << "测试结果:累计开奖次数:" << m_lOpenResultCount;
		LOG(ERROR) << "测试结果:累计押注(总进): " << m_testTotalIn;
		LOG(ERROR) << "测试结果:累计赢分(总出): " << m_testdAllOut;
		LOG(ERROR) << "测试结果:累计赢分(总进-总出): " << winscore;
		if (m_lOpenResultCount >= TEST_COUNT)
		{
			openSLog("本次测试结果:累计押注(总进): %ld, 累计赢分(总出): %ld, 总进-总出 = %ld,累计次数: %d;", m_testTotalIn, m_testdAllOut, winscore, m_lOpenResultCount);
			openSLog("测试结果:累计总押注: %ld, 累计总赢分: %ld, 总进-总出 = %ld,累计总次数: %d; \n", m_TotalIn, m_TotalOut, allWinScore, m_lTestOpenCount);
			m_lOpenResultCount = 0;
			m_testTotalIn = 0;
			m_testdAllOut = 0;
			memset(m_lOpenTypeCount, 0, sizeof(m_lOpenTypeCount));
		...
		{
			openSLog("测试结果:累计押注(总进): %ld, 累计赢分(总出): %ld, 总进-总出= %ld,累计开奖次数: %d;", m_testTotalIn, m_testdAllOut, winscore, m_lOpenResultCount);
		}
	}*/
}
...
{
    if (m_iGameState == slwh::SUB_S_GameEnd)
    {
        if (m_vPlayerList.size() > 1)
        {
            slwh::CMD_S_UserWinList PlayerList;
            for (int i = m_vPlayerList.size() - 2; i > -1; i--)
            {
                ...
                slwh::PlayerInfo *playerinfo = PlayerList.add_player();
            ...
            m_pITableFrame->SendTableData(chairid, slwh::SUB_S_PLAYERLIST, (uint8_t *)sendData.c_str(), sendData.size());
        }
    }
    else
    {
...
bool TableFrameSink::GamePlayerJetton(uint32_t chairid, uint8_t subid, const uint8_t *data, uint32_t datasize)
{
    ...
    // if (slwh::SUB_S_GameStart != m_iGameState)
    // {
    //     LOG(WARNING) << "----------------游戏状态不对，不能押注----------" << m_iGameState << " " << slwh::SUB_S_GameStart;
    //     return true;
    // }
    ...
    // 玩家下注无效
    if (!bPlaceJetScuccess || slwh::SUB_S_GameStart != m_iGameState)
    {
        // 非机器人才返回
        if(!pSelf->IsAndroidUser()){
            slwh::CMD_S_PlaceJettonFail placeJetFail;
        	...
        	std::string sendData = placeJetFail.SerializeAsString();
             //if (m_bTest==0)
       		   m_pITableFrame->SendTableData(pSelf->GetChairId(), slwh::SUB_S_JETTON_FAIL, (uint8_t *)sendData.c_str(), sendData.size());
        }
    ...
    for (auto &it : m_UserInfo)
    {
        ...
        if(!pServerUserItem)
            continue;
        slwh::CMD_S_PlaceJetSuccess placeJetSucc;
        placeJetSucc.Clear();
        ...
        std::string sendData = placeJetSucc.SerializeAsString();
         //if (m_bTest==0)
            m_pITableFrame->SendTableData(userInfoItem->wCharid, slwh::SUB_S_JETTON_SUCCESS, (uint8_t *)sendData.c_str(), sendData.size());
        pmayernum++;
    }
...
void TableFrameSink::Rejetton(uint32_t chairid, uint8_t subid, const uint8_t *data, uint32_t datasize)
{
    ...
    if (alljetton == 0 || Usr->GetUserScore() < alljetton)
    {
        ...
        std::string sendData = placeJetFail.SerializeAsString();
        m_pITableFrame->SendTableData(Usr->GetChairId(), slwh::SUB_S_JETTON_FAIL, (uint8_t *)sendData.c_str(), sendData.size());
    ...
    for (int i = 0; i < BET_ITEM_COUNT; i++)
    {
        ...
        if (m_nAreaMaxJetton[i] < m_UserInfo[userid]->dAreaJetton[i] + m_UserInfo[userid]->dLastJetton[i])
        {
            ...
            std::string data = placeJetFail.SerializeAsString();
			 //if (m_bTest==0)
            	m_pITableFrame->SendTableData(Usr->GetChairId(), slwh::SUB_S_JETTON_FAIL, (uint8_t *)data.c_str(), data.size());
...
void TableFrameSink::GamePlayerQueryPlayerList(uint32_t chairId, uint8_t subid, const uint8_t *data, uint32_t datasize, bool IsEnd)
{
    ...
    slwh::CMD_C_PlayerList querylist;
    ...
    int32_t endindex = 0;
    slwh::CMD_S_PlayerList playerlist;
    ...
    for (vector<UserInfo>::iterator it = m_pPlayerInfoList_20.begin(); it != m_pPlayerInfoList_20.end(); it++)
    {
        ...
        slwh::PlayerListItem *item = playerlist.add_players();
    ...
    m_pITableFrame->SendTableData(pIIServerUserItem->GetChairId(), slwh::SUB_S_QUERY_PLAYLIST, (uint8_t *)lstData.c_str(), lstData.size());
	PlayerRichSorting();
}
// 发送场景消息(押分阶段)
void TableFrameSink::SendGameSceneStart(int64_t userid, bool lookon)
{
    ...
    slwh::CMD_S_Scene_GameStart scenceStart;
    ...
    for (int i = 0; i < BET_ITEM_COUNT; i++)
    {
        slwh::CMD_AeraInfo *areainfo = scenceStart.add_aerainfo();
    ...
    //         {
    //             slwh::RepeatInfo *RepeatInfo = scenceStart.add_trepeat();
	...
	for (int i = m_vResultRecored.size() - 1; i >= 0; i--)
	{
		slwh::GameResultInfo *resultInfo = scenceStart.add_resultinfos();
		resultInfo->set_ihzxid(m_vResultRecored[i].iHZXId);
		resultInfo->set_ianimalid(m_vResultRecored[i].iAnimalId);
		resultInfo->set_ianimalid_sd(m_vResultRecored[i].iAnimalId_sd);
		resultInfo->set_iopentype(m_vResultRecored[i].iOpenType);
	...
	slwh::GameResultInfo *lastResultInfo = scenceStart.add_nlastresultinfo();
	lastResultInfo->set_ihzxid(m_lastResultInfo.iHZXId);
	lastResultInfo->set_ianimalid(m_lastResultInfo.iAnimalId);
	lastResultInfo->set_ianimalid_sd(m_lastResultInfo.iAnimalId_sd);
	lastResultInfo->set_iopentype(m_lastResultInfo.iOpenType);
    ...
    scenceStart.set_nstopidx(m_iStopIndex); 
	...
	for (auto &uInfoItem : m_pPlayerInfoList)
	{
        ...
        slwh::PlayerListItem *palyerItem = scenceStart.add_players();
	...
	*/
	...
	for (int j = 0; j < CIRCLE_COUNT; j++)
	{
        scenceStart.add_nanimalcircle(m_nAnimalCircle[j]);
        scenceStart.add_ncolourcircle(m_nColourCircle[j]);
	}	
	vector<shared_ptr<OpenResultInfo>> vResultList;
	vResultList = m_vOpenResultListCount_last[0];
	int32_t openCount = vResultList.size();
	...
	scenceStart.set_laststopindex(m_lastStopIndex);
	scenceStart.set_lastresulthzxid(m_lastResultHZXId);

    std::string sendData = scenceStart.SerializeAsString();
    m_pITableFrame->SendTableData(player->GetChairId(), slwh::SUB_S_SCENE_START, (uint8_t *)sendData.c_str(), sendData.size());
}
...
void TableFrameSink::SendGameSceneEnd(int64_t userid, bool lookon)
{
  ...
  //筹码数量
    ...
    //scenceStart.set_nlastretid(m_lastResultInfo);
	slwh::GameResultInfo *lastResultInfo = scenceStart.add_nlastresultinfo();
	lastResultInfo->set_ihzxid(m_lastResultInfo.iHZXId);
	lastResultInfo->set_ianimalid(m_lastResultInfo.iAnimalId);
	lastResultInfo->set_ianimalid_sd(m_lastResultInfo.iAnimalId_sd);
	...
	*/
	...
	if (slwh::SUB_S_GameStart != m_iGameState)
	{
		scenceStart.set_hzxidex(m_iResultHZXId);
		scenceStart.set_colourindex(m_iStopColourIndex);
		if (m_iResultTypeTag == (int)OpenType::eSongDeng)
		{
			scenceStart.set_colourindex_sd(m_iStopColourIndex_sd);
			scenceStart.set_cartype_sd(m_iResultId_sd);
			scenceStart.set_nstopidx_sd(m_iStopIndex_sd);
		}
		scenceStart.set_iopentype(m_iResultTypeTag);
		scenceStart.set_cbopentime(m_endCountDown);
	}
...
int TableFrameSink::PlayerJettonPanDuan(int index, int32_t score, int32_t chairid, shared_ptr<CServerUserItem> &pSelf)
{
    ...
    */
    ...
    {
        //LOG(WARNING) << "---------小于准许进入分--------" << " UserId:" << pSelf->GetUserId() << " UserScore: " << pSelf->GetUserScore() << " allBetScore: " << allBetScore << " betScore: " << score;
        return 3;
    }
	// 押注项超出限制范围
    // if (m_nAreaMaxJetton[index] < m_pBetArea->mifourAreajetton[index] + score)
...
void TableFrameSink::OnTimerSendOtherBetJetton()
{
    ...
    //     slwh::CMD_S_OtherPlaceJetSuccess otherPlaceJetSucc;
    ...
    //     // m_pITableFrame->SendTableData(i, slwh::SUB_S_JETTON_SUCCESS_OTHER, (uint8_t *)data.c_str(), data.size());
...
void TableFrameSink::openSLog(const char *p, ...)
{
    ...
    time(&tt);
    t = localtime(&tt);
	/*if (m_bTest > 0 && m_lOpenResultCount == TEST_COUNT)
	{
		sprintf(Filename, "log//slwh//%d_%d_%d_%d_%d.log", m_testBetType, m_testBetIndex, m_tsetOdds, m_lOpenResultCount, m_testTimes);
	} 
	else*/
	{
		sprintf(Filename, "log//slwh//slwh_%d%d%d.log", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, m_pITableFrame->GetGameRoomInfo()->roomId, m_pITableFrame->GetTableId());
	}
...
void TableFrameSink::SetGameRecordDetail(int64_t userid, int32_t chairId, int64_t userScore, shared_ptr<UserInfo> userInfo)
{
	...
	//记录当局游戏详情binary
	string strDetail = "";
	slwh::CMD_S_GameRecordDetail details;
	...
	//开奖类型[0:正常开奖 1:霹雳闪电 2:送灯 3:大三元 4:大四喜]
	int32 resType = m_iResultTypeTag;
	...
	if (m_iResultTypeTag == (int)OpenType::eSongDeng)
	{
		details.set_residxsd(m_iResultId_sd);				//送灯ID
		details.set_idxmulsd(m_nMuilt[m_iResultId_sd]);				//送灯倍数
	}
	details.set_hzxidx(m_iResultHZXId);				    //庄闲和结果
	details.set_hzxidxmul(m_nMuilt[m_iResultHZXId]);	//庄闲和倍数
	...
	for (int betIdx = 0;betIdx < BET_ITEM_COUNT; betIdx++)
	{
		...
		slwh::BetAreaRecordDetail* detail = details.add_detail();
		//下注区域
...
//更新100局开奖结果信息
void TableFrameSink::updateResultList(tagResultInfo resultInfo)
{
	vector<shared_ptr<OpenResultInfo>> vResultList;
	int resultId = 0;
	for (int i = 0;i < BET_ITEM_COUNT;++i)
	{
		if (i <= (int)AnimType::eYellowLion)
		{
			resultId = resultInfo.iAnimalId;
		} 
		else
		{
			resultId = resultInfo.iHZXId;
		}
		...
		iResultIndex->iHZXId = resultInfo.iHZXId;
		iResultIndex->iAnimalId = resultInfo.iAnimalId;
		iResultIndex->iAnimalId_sd = resultInfo.iAnimalId_sd;
		iResultIndex->iopenTypeTag = resultInfo.iOpenType;
...
{
	//记录
	slwh::CMD_S_GameRecord gameRecordRsp;

	vector<shared_ptr<OpenResultInfo>> vResultList;
	vResultList = m_vOpenResultListCount_last[0];
	int32_t openCount = vResultList.size();
	gameRecordRsp.set_opencount(openCount);
	gameRecordRsp.set_openallcount(m_lOpenResultAllCount);

	for (int i = 0;i < BET_ITEM_COUNT; ++i)
	{
		vResultList = m_vOpenResultListCount_last[i];
		slwh::GameRecordList *gamerecordList = gameRecordRsp.add_recordlists();
		for (int j = 0; j < openCount; j++)
		{
			if (i == 0)
			{
				slwh::GameResultInfo *gameResultInfo = gameRecordRsp.add_iresultinfo();
				gameResultInfo->set_ihzxid(vResultList[j]->iHZXId);
				gameResultInfo->set_ianimalid(vResultList[j]->iAnimalId);
				gameResultInfo->set_ianimalid_sd(vResultList[j]->iAnimalId_sd);
				gameResultInfo->set_iopentype(vResultList[j]->iopenTypeTag);
			}
			slwh::GameRecordListItem *listItem = gamerecordList->add_records();
			listItem->set_notopencount(vResultList[j]->notOpenCount);
		}
	}

	std::string sendData = gameRecordRsp.SerializeAsString();
	m_pITableFrame->SendTableData(chairid, slwh::SUB_S_QUERY_GAMERECORD, (uint8_t *)sendData.c_str(), sendData.size());
}
...
// 获取本局颜色圈排序
void TableFrameSink::getColourCircle()
{
	m_iChooseMuilt++;
	if (m_iChooseMuilt > 2) m_iChooseMuilt = 0;

	//m_iChooseMuilt = 0;

	memcpy(m_nMuilt, m_nAllMuilt[m_iChooseMuilt], sizeof(m_nMuilt));
	CalculatWeightAndOpenType(m_iResultTypeTag);
	memset(m_nColourCircle, -1, sizeof(m_nColourCircle));
	int colourIndex = 0;
	int redCount = 0;
	int greenCount = 0;
	int yellowCount = 0;
	...
	do
	{
		colourIndex = m_random.betweenInt(0, 2).randInt_mt(true);
		if (colourIndex == COLOUR_RED) // 红色
		{
			if (redCount < m_iColourCount[m_iChooseMuilt][0])
			{
				m_nColourCircle[allCount] = COLOUR_RED;
				allCount++;
				redCount++;
			}
			else
			{
				colourIndex = m_random.betweenInt(1, 2).randInt_mt(true);
				if (colourIndex == COLOUR_GREEN) //绿色
				{
					if (greenCount < m_iColourCount[m_iChooseMuilt][1])
					{
						m_nColourCircle[allCount] = COLOUR_GREEN;
						allCount++;
						greenCount++;
					}
					else if (yellowCount < m_iColourCount[m_iChooseMuilt][2])
					{
						m_nColourCircle[allCount] = COLOUR_YELLOW;
						allCount++;
						yellowCount++;
					}
				}
				else if (colourIndex == COLOUR_YELLOW) //黄色
				{
					if (yellowCount < m_iColourCount[m_iChooseMuilt][2])
					{
						m_nColourCircle[allCount] = COLOUR_YELLOW;
						allCount++;
						yellowCount++;
					}
					else if (greenCount < m_iColourCount[m_iChooseMuilt][1])
					{
						m_nColourCircle[allCount] = COLOUR_GREEN;
						allCount++;
						greenCount++;
		...
		else if (colourIndex == COLOUR_GREEN) //绿色
		{
			if (greenCount < m_iColourCount[m_iChooseMuilt][1])
			{
				m_nColourCircle[allCount] = COLOUR_GREEN;
			...
			{
				colourIndex = m_random.betweenInt(0, 1).randInt_mt(true);
				if (colourIndex == COLOUR_RED) //红色
				{
					if (redCount < m_iColourCount[m_iChooseMuilt][0])
					{
				...
				else  //黄色
				{
					...
					else if (redCount < m_iColourCount[m_iChooseMuilt][0])
					{
						m_nColourCircle[allCount] = COLOUR_RED;
						allCount++;
						redCount++;
	...
	for (int i = 0;i < 24;i++)
	{
		str += to_string(i) + ":" + to_string(m_nColourCircle[i]) + " ";
		animStr += to_string(i) + ":" + to_string(m_nAnimalCircle[i]) + " ";
	}
	openSLog("本局动物排序:%s ", animStr.c_str());
	openSLog("本局颜色排序:%s, 赔率表:%d, 总数:%d, 红色:%d, 绿色:%d, 黄色:%d, ", str.c_str(), m_iChooseMuilt, allCount, redCount, greenCount, yellowCount);
}

//获取停止的动物ID
void TableFrameSink::GetStopIndex(int animalid, int& stopIndex, int& colourIndex, int& stopIndex_sd, int& colourIndex_sd)
{
	int animalIndex = 0;
	int Animal = animalid % 4;
	int Colour = animalid / 4;
	vector<int32_t> vColourIndex[3];
	vector<int32_t> vAnimalIndex[4];
	for (int i = 0;i < CIRCLE_COUNT;++i)
	{
		if (m_nColourCircle[i] == COLOUR_RED)
		{
			vColourIndex[COLOUR_RED].push_back(i);
		} 
		else if (m_nColourCircle[i] == COLOUR_GREEN)
		{
			vColourIndex[COLOUR_GREEN].push_back(i);
		}
		else if (m_nColourCircle[i] == COLOUR_YELLOW)
		{
			vColourIndex[COLOUR_YELLOW].push_back(i);
		}
		if (m_nAnimalCircle[i] == 0)
		{
			vAnimalIndex[0].push_back(i); // 兔子
		} 
		else if (m_nAnimalCircle[i] == 1)
		{
			vAnimalIndex[1].push_back(i); // 猴子
		}
		else if (m_nAnimalCircle[i] == 2)
		{
			vAnimalIndex[2].push_back(i); // 熊猫
		}
		else if (m_nAnimalCircle[i] == 3)
		{
			vAnimalIndex[3].push_back(i); // 狮子
		}
	}

	int AnimalCount = vAnimalIndex[Animal].size() - 1;
	int randNum = m_random.betweenInt(0, AnimalCount).randInt_mt(true);
	stopIndex = vAnimalIndex[Animal][randNum];

	int colourCount = vColourIndex[Colour].size() - 1;
	randNum = m_random.betweenInt(0, colourCount).randInt_mt(true);
	colourIndex = vColourIndex[Colour][randNum];

	if (m_iResultTypeTag == (int)OpenType::eSongDeng)
	{
		Animal = m_iResultId_sd % 4;
		Colour = m_iResultId_sd / 4;

		AnimalCount = vAnimalIndex[Animal].size() - 1;
		randNum = m_random.betweenInt(0, AnimalCount).randInt_mt(true);
		stopIndex_sd = vAnimalIndex[Animal][randNum];

		colourCount = vColourIndex[Colour].size() - 1;
		randNum = m_random.betweenInt(0, colourCount).randInt_mt(true);
		colourIndex_sd = vColourIndex[Colour][randNum];

		LOG(INFO) << "本局开奖类型:" << m_iResultTypeTag << ", 开奖id:" << animalid << ", 动物圈停止ID:" << stopIndex << ", 颜色圈停止ID:" << colourIndex << " 本局送灯id:" << m_iResultId_sd << ", 送灯动物圈停止ID:" << stopIndex_sd << ", 送灯颜色圈停止ID:" << colourIndex_sd;
		openSLog("本局开奖类型:%d,动物ID:%d, 动物圈停止ID:%d, 颜色圈停止ID:%d, 送灯id:%d, 送灯动物圈停止ID:%d, 送灯颜色圈停止ID:%d", m_iResultTypeTag, animalid, stopIndex, colourIndex, m_iResultId_sd, stopIndex_sd, colourIndex_sd);
	}
	else
	{
		LOG(INFO) << "本局开奖类型:" << m_iResultTypeTag << ", 开奖id:" << animalid << ", 动物圈停止ID:" << stopIndex << ", 颜色圈停止ID:" << colourIndex;
		openSLog("本局开奖类型:%d,动物ID:%d, 动物圈停止ID:%d, 颜色圈停止ID:%d", m_iResultTypeTag, animalid, stopIndex, colourIndex);
	}	
}

// 统计随机开奖时开奖类型 
// 0:正常开奖 1:霹雳闪电 2:送灯 3:大三元 4:大四喜
void TableFrameSink::CalculatWeightAndOpenType(int &typeTag)
{
	//int nWeight[5] = {100,100,100,100,100}; // {800,60,60,40,40}
	//int wholeweight = 0;
	//int res = 0;
	//for (int i = 0;i < 5;i++)
	//{
	//	wholeweight += nWeight[i];
	//}
	//// 本局预开奖的类型
	...
	for (int i = 0;i < 5;i++)
	{
		...
		{
			str = " 0 正常开奖";
			animid = getRandAnimalID();
			m_iResultId_tmp[i] = animid;
			continue;
		}
		else if (i == 1)
		{
			str = " 1 霹雳闪电";
			animid = getRandAnimalID();
			nMuilt[i] = getAverageMuilt(0, animid) * 2;
			m_iResultId_tmp[i] = animid;
			float addMuilt = m_TmpnMuilt[animid] * 2.0f;
			float fOddz = 0.0f;
			fOddz = (addMuilt / nMuilt[i]);			
			if (fOddz > 1.0f)
			{
				nMuilt[i] += addMuilt * fOddz * m_fAverageMuilt[i] * 0.90f;
			}
			else
			{
				nMuilt[i] += addMuilt * fOddz * m_fAverageMuilt[i];
			}
		}
		else if (i == 2)
		{
			...
			do
			{
				m_iResultId_sd = getRandAnimalID();
			} while (m_iResultId_sd == animid);
			str = " 2 送灯:" + to_string(m_iResultId_sd);
			nMuilt[i] = getAverageMuilt(0, animid) * 2;
			m_iResultId_tmp[i] = animid;
			float addMuilt = (m_TmpnMuilt[animid] + m_TmpnMuilt[m_iResultId_sd]) * 1.0f;
			float fOddz = 0.0f;
			fOddz = (addMuilt / nMuilt[i]);
			nMuilt[i] += addMuilt * fOddz * m_fAverageMuilt[i];
			/*if (fOddz < 1.0f)
			...
			{
				nMuilt[i] += addMuilt * fOddz * (m_fAverageMuilt[i] + 0.15f);
			}*/
		}
		else if (i == 3)
		{
			str = " 3 大三元";
			m_iResultId_tmp[i] = getRandAnimalID();
			animid = m_iResultId_tmp[i] % 4;			
			nMuilt[i] = getAverageMuilt(0, animid) * 3;
			float addMuilt = 0.0f;
			float fOddz = 0.0f;
			for (int j = 0;j < 3;++j)
			{
				addMuilt += m_TmpnMuilt[animid + j * 4];
			}
			fOddz = (addMuilt / nMuilt[i]);
			if (animid <= 1)
			{
				nMuilt[i] += addMuilt * fOddz * (m_fAverageMuilt[i] + 0.12f);
			}
			else if (animid == 2)
			{
				nMuilt[i] += addMuilt * fOddz * (m_fAverageMuilt[i] * 0.98f);
			}
			else
			{
				nMuilt[i] += addMuilt * fOddz * (m_fAverageMuilt[i] * 0.96f);
			}
		}
		else if (i == 4)
		{
			...
			m_iResultId_tmp[i] = getRandAnimalID();
			animid = m_iResultId_tmp[i] / 4;
			nMuilt[i] = getAverageMuilt(0, animid) * 4;
			float addMuilt = 0.0f;
			float fOddz = 0.0f;
			for (int j = 0;j < 4;++j)
			{
				addMuilt += m_TmpnMuilt[animid * 4 + j];
			}
			fOddz = (addMuilt / nMuilt[i]);	
			nMuilt[i] += addMuilt * fOddz * m_fAverageMuilt[i];
			/*if (fOddz > 1.1f)
	...
	// 重新计算权重
	int nAllMuilt[BET_ITEM_COUNT + 1] = { 0 };
	float weight = 0.0f;
	int allWeight = 0;
	string strWeight = "";
	for (int i = 0;i < BET_ITEM_COUNT + 1;++i)
	{
		if (i<12)
		{
			nAllMuilt[i] = m_TmpnMuilt[i];
		} 
		else
		{
			nAllMuilt[i] = nMuilt[i - 11];
		}
		weight = 1.0f * 10000.0f / nAllMuilt[i];
		m_retWeight[i] = (int)weight;
		allWeight += m_retWeight[i];
	}
	if (allWeight < 10000)
	{
		int lessweight = (10000 - allWeight) / 12;
		int less = 10000 - allWeight - lessweight * 12;
		for (int i = 0;i < 12;++i)
		{
			m_retWeight[i] += lessweight;
		}
		m_retWeight[0] += less;
	}
	else
	{
		...
		for (int i = 0;i < 16;++i)
		{
			lessweight = 1.0f * moreNum * m_retWeight[i] / allWeight;
			if (m_retWeight[i] >= lessweight)
			{
				m_retWeight[i] -= lessweight;
				less += lessweight;
			}
	...
	{
		allWeight += m_retWeight[i];
		strWeight += to_string(i) + "_" + to_string(m_retWeight[i]) + "  ";
		str += to_string(i) + "_" + to_string(nAllMuilt[i]) + "  ";
	}
	LOG(WARNING) << "本局开奖倍数:" << str;
	LOG(WARNING) << "本局开奖权重:" << strWeight << " 总权重:" << allWeight;
	openSLog("本局开奖倍数:%s", str.c_str());
	openSLog("本局开奖权重:%s,总权重:%d", strWeight.c_str(), allWeight);
}

//获取本局的动物平均赔率(0:所有动物;1:霹雳闪电;2:送灯;3:大三元:4:大四喜)
int TableFrameSink::getAverageMuilt(int type, int animalid)
{
	int AllMuilt = 0;
	int AverageMuilt = 0;
	for (int i = 0;i < 12;++i)
	...
	AverageMuilt = (AllMuilt * 1.0f / 12);
	if (type <= 2 )
	{	
		if (type > 0)
		{
			AverageMuilt = m_nMuilt[animalid];
		}
	} 
	else
	{
		...
		if (type == 3)
		{
			...
			for (int j = 0;j < 3;++j)
			{
				AllMuilt += m_nMuilt[id + j * 4];
			}
			AverageMuilt = ceil(AllMuilt * 1.0f / 3);
		} 
		else
		{
			int id = animalid / 4;
			for (int j = 0;j < 4;++j)
			{
				AllMuilt += m_nMuilt[id * 4 + j];
			}
			AverageMuilt = ceil(AllMuilt * 1.0f / 4);
		}
	}
	
	/*LOG(WARNING) << " 动物平均赔率 AverageMuilt:" << " ,AllMuilt:" << AllMuilt << AverageMuilt << " ,type:" << type ;
	openSLog("动物平均赔率 AllMuilt:%d,AverageMuilt:%d,type:%d", AllMuilt, AverageMuilt, type);*/
	return AverageMuilt;
}
// 按动物权重获取动物
int TableFrameSink::getRandAnimalID(int type)
{
	int anilmalID = 0;
	int allWeight = 0;
	if (!m_bHaveColWeight)
	{
		...
		for (int i = 0;i < 3;++i)
		{
			for (int j = 0;j < 12;++j)
			{
				nMuilt[j] += m_nAllMuilt[i][j];
			}
		}
		for (int j = 0;j < 12;++j)
		{
			//nMuilt[j] = ceil(nMuilt[j] * 1.0f / 3);
			nMuilt[j] = m_nMuilt[j];
		}
		m_bHaveColWeight = true;
		memcpy(m_TmpnMuilt, nMuilt, sizeof(nMuilt));
		// 获取12个动物的权重 
		float weight = 0.0f;
		allWeight = 0;
		string strWeight = "";
		for (int i = 0;i < 12;++i)
		{
			weight = 1.0f * 10000.0f / nMuilt[i];
			m_TmpRetWeight[i] = (int)weight;
			allWeight += m_TmpRetWeight[i];
		}
		if (allWeight < 10000)
		{
			int lessweight = (10000 - allWeight) / 12;
			...
			for (int i = 0;i < 12;++i)
			{
				m_TmpRetWeight[i] += lessweight;
			}
			m_TmpRetWeight[0] += less;
		}
		else
		{
			int moreNum = allWeight - 10000;
			...
			for (int i = 0;i < 12;++i)
			{
				lessweight = 1.0f * moreNum * m_TmpRetWeight[i] / allWeight;
				if (m_TmpRetWeight[i] >= lessweight)
				{
					m_TmpRetWeight[i] -= lessweight;
					less += lessweight;
				}
				...
				{
					less += m_TmpRetWeight[i];
					m_TmpRetWeight[i] = 0;
				}
			}
			m_TmpRetWeight[0] -= (moreNum - less);
			for (int i = 0;i < 12;++i)
			{
				if (m_TmpRetWeight[i] < 0)
                {
                    break;
				}
			}
		...
		{
			allWeight += m_TmpRetWeight[i];
			str += to_string(i) + "_" + to_string(nMuilt[i]) + "  ";
			strWeight += to_string(i) + "_" + to_string(m_TmpRetWeight[i]) + "  ";
		}
		LOG(WARNING) << "计算动物倍数:" << str;
		LOG(WARNING) << "计算动物权重:" << strWeight << " 总权重:" << allWeight;
		openSLog("计算平均倍数:%s", str.c_str());
		openSLog("计算动物权重:%s,总权重:%d", strWeight.c_str(), allWeight);
	}
	...
	{
		int ID[12] = { 0 };
		int sortprobirity[12] = { 0 };
		memcpy(sortprobirity, m_TmpRetWeight, sizeof(sortprobirity));
		allWeight = 0;
		for (int i = 0;i < 12;i++)
		{
			ID[i] = i;
			allWeight += sortprobirity[i];
		}
		...
		int randNum = m_random.betweenInt(0, allWeight).randInt_mt(true);
		int res = 0;
		for (int i = 0;i < 12;i++)
		{
			res += sortprobirity[i];
	...
	{
		anilmalID = m_random.betweenInt(0, 11).randInt_mt(true);
	}
	return anilmalID;
}

//计算测试赢分
long TableFrameSink::CalculatTestWinScore(int winTag, int typeTag, int winTagHzx)
{
	long winscore = 0;
	if (typeTag == (int)OpenType::eNormal)
	{
		winscore += m_testdAreaJetton[winTag] * m_nMuilt[winTag];
	}
	else if (typeTag == (int)OpenType::ePiliShanDian)
	{
		winscore += m_testdAreaJetton[winTag] * m_nMuilt[winTag] * 2;
	}
	else if (typeTag == (int)OpenType::eSongDeng)
	{
		winscore += m_testdAreaJetton[winTag] * m_nMuilt[winTag];
		winscore += m_testdAreaJetton[m_iResultId_sd] * m_nMuilt[m_iResultId_sd]; //送灯的赔分
	}
	else if (typeTag == (int)OpenType::eDaSanYuan)
	{
		...
		for (int i = 0;i < 3;++i)
		{
			winscore += m_testdAreaJetton[animID + i * 4] * m_nMuilt[animID + i * 4];
		}
	}
	else if (typeTag == (int)OpenType::eDaSiXi)
	{
		...
		for (int i = 0;i < 4;++i)
		{
			winscore += m_testdAreaJetton[animID * 4 + i] * m_nMuilt[animID * 4 + i];
		}
	}

	winscore += m_testdAreaJetton[winTagHzx] * m_nMuilt[winTagHzx];

	LOG(WARNING) << " 计算测试赢分 winscore:" << winscore << "  winTagHzx:" << winTagHzx;
	openSLog("计算测试赢分 winscore:%d, winTagHzx:%d", winscore, winTagHzx);
	return winscore;
}


//判断玩家是不是倍投
void TableFrameSink::JudgmentUserIsDoubleCast()
{
	shared_ptr<IServerUserItem> pUserItem;
	shared_ptr<UserInfo> userInfoItem;
	for (auto &it : m_UserInfo)
	{
		...
		if (!pUserItem || pUserItem->IsAndroidUser())
			continue;
		int64_t curJetton = userInfoItem->dAreaJetton[12] + userInfoItem->dAreaJetton[13] + userInfoItem->dAreaJetton[14];
		if (curJetton < 1)
		{
			continue;
		}
