...
#include "../Game_sghjk/MSG_HJK.h"
#include "../Game_sghjk/GameLogic.h"
...
bool CAndroidUserItemSink::OnGameMessage(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    ...
    {
        sghjk::CMD_S_GameStartAi GameStartAi;
...
void CAndroidUserItemSink::ReadConfigInformation()
{
    ...
    myprintf(szConfigFileName, sizeof(szConfigFileName), _T("%sconf/sghjk_config.ini"), szPath);
...
void  CAndroidUserItemSink::openLog(const char *p, ...)
{
    ...
    sprintf(Filename, "log//sghjk//sghjk_%d%d%d_%d.log",t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,m_pTableFrame->GetGameRoomInfo()->roomId);
