...
public:
    ...
    int64_t GetTwentyWin()
    {
		...
		lTwentyWinCount = 0;
        for (vector<int>::iterator it = ListWinOrLose.begin(); it != ListWinOrLose.end(); it++)
        {
            if ((*it) > 0)
				lTwentyWinCount += 1;
        }
    ...
    int64_t TotalIn;
    int64_t iMulticle[BET_ITEM_COUNT];
...
public:
    ...
    {
        iResultDian = result.iDianshu;
        if(result.winList[eDan])
        {
            iIsDanShuangBao = 0;
        }
        else if(result.winList[eShuang])
        {
            iIsDanShuangBao = 1;
        }
        else
        ...
        if(result.winList[eDa])
        {
            iIsDaXiaoBao = 0;
        }
        else if(result.winList[eXiao])
        {
            iIsDaXiaoBao = 1;
        }
        else
    ...
    int iResultDian;    //点数，包括3点，18点
    int iIsDanShuangBao;//单双或者豹子
    int iIsDaXiaoBao;   //大小或者豹子
...
private: 
	...
	//开金鲨是的倍数和彩金倍数
    int64_t m_ctrlGradePercentRate[2];  // 比例[下标0:低倍;1:中倍]
