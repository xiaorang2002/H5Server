...
#else
        dir += "/Toubao";
        google::InitGoogleLogging("Toubao");
#endif //__HJSDS__
...
{ 
    ...
    { 
        ...
        // 修改miGameSceneState
        m_iGameState = Tb::SUB_S_GameStart;
        // 启动定时器
        GamefirstInGame();
    ...
    switch (m_iGameState)
    {
    case Tb::SUB_S_GameStart:
    {
        SendGameSceneStart(userid, false);
        break;
    }
    case Tb::SUB_S_GameEnd:
    {
...
void TableFrameSink::GameTimerStart()
{
    ...
    // 设置游戏状态
    m_iGameState = Tb::SUB_S_GameStart;
    m_pITableFrame->SetGameStatus(Tb::SUB_S_GameStart);
	m_bGameEnd = false;
    m_timetick = 0;
    //float betCountDown = m_betCountDown + m_fGameStartAniTime; //1s,预留前端的开始动画
    m_timetickAll = m_betCountDown;
    ...
    if (m_bIsContinue)
    {
        ...
        // else
            m_TimerIdEnd = m_LoopThread->getLoop()->runAfter(m_betCountDown, boost::bind(&TableFrameSink::GameTimerEnd, this));
            m_TimerIdJetton = m_LoopThread->getLoop()->runAfter(3, boost::bind(&TableFrameSink::GameTimerJetton, this));
    }
    else
    {
        clearUserIdVec.clear();
...
void TableFrameSink::GameTimerEnd()
{
    ...
    // 更新状态
    m_iGameState = Tb::SUB_S_GameEnd;
    m_bBetStatus =0;
    m_pITableFrame->SetGameStatus(Tb::SUB_S_GameEnd);
    m_timetick = 0;
...
{
    // 游戏状态判断
    if (Tb::SUB_S_GameStart != m_iGameState)
    {
       return;
    }
    int betTimes = 0;
...
{ 
    ...
    Tb::CMD_S_CurState curStateRsp;

    int32_t iEapseTime = m_betCountDown;
    if(Tb::SUB_S_GameEnd == m_iGameState)
    {
        iEapseTime = m_endCountDown;
    }
    curStateRsp.set_ncurretidx(m_iStopIndex); 
    curStateRsp.set_ncurstate((int)(Tb::SUB_S_GameStart != m_iGameState)); //当前状态
    ...
    Tb::ResultStr * lastresult = curStateRsp.mutable_nlastresult();
    lastresult->set_ndanshuang(m_lastRetId.iDanshuang);
    lastresult->set_ndaxiao(m_lastRetId.iDaxiao);
    for(int i=0;i<eMax;i++)
    {
        if(i<3)lastresult->add_ndicepoints(m_lastRetId.touList[i]);
    ...
    for (int i = m_vResultRecored.size() - 1; i >= 0; i--)
    {
        ...
        Tb::ResultStr * sResultList = curStateRsp.add_sresultlist();
        ...
        for(int j=0;j<eMax;j++)
        {
            if(j<3)
            {
                sResultList->add_ndicepoints(m_vResultRecored[i].touList[j]);
                LOG(ERROR)<<"m_vResultRecored[i].touList[j]="<<m_vResultRecored[i].touList[j];
            }
    ...
    for (auto &uInfoItem : m_pPlayerInfoList)
    {
        ...
        Tb::PlayerListItem* palyer = curStateRsp.add_players();
    ...
    m_pITableFrame->SendTableData(chairid, Tb::SUB_S_QUERY_CUR_STATE, (uint8_t *)sendData.c_str(), sendData.size());
}
...
void TableFrameSink::QueryGameRecord(int32_t chairid, uint8_t subid, const uint8_t *data, uint32_t datasize)
{
	...
	if (!player)
		return;
    Tb::CMD_C_GameRecord gameRecordReq;
...
{ 
    ...
    infoVec.clear(); 
    //三颗骰子点数|大小骰-倍数|单双骰-倍数|对子-倍数|围骰-倍数|点数-倍数(3,18点没有)
    int32_t mudaxiao = 2;
    ...
    int32_t duizi = 0;
    int32_t duiziMul = 0;
    for(int i=eDuizi1;i<eWeiTou1;i++)
    {
        if(m_strResultInfo.muticlList[i]>0)
        {
            duizi = i-eDuizi1+1;
            duiziMul = m_strResultInfo.muticlList[i];
        }
    }
    int32_t weisai = 0;
    int32_t weisaiMul = 0;
    for(int i=eWeiTou1;i<eDian4;i++)
    {
        if(m_strResultInfo.muticlList[i]>0)
        {
            weisai = i-eWeiTou1+1;
            weisaiMul = m_strResultInfo.muticlList[i];
        }
    }
    int32_t dianshuMul = 0;
    for(int i=eDian4;i<eSanjun1;i++)
    {
        if(m_strResultInfo.muticlList[i]>0)
        {
            dianshuMul = m_strResultInfo.muticlList[i];
        }
    }
    string strRet = str(boost::format("%d-%d-%d|%d-%d|%d-%d|%d-%d|%d-%d|%d-%d")
                        % m_strResultInfo.touList[0]% m_strResultInfo.touList[1]% m_strResultInfo.touList[2]
            %m_strResultInfo.iDaxiao%mudaxiao%m_strResultInfo.iDanshuang%mudaxiao
            %duizi%duiziMul%weisai%weisaiMul%m_strResultInfo.iDianshu%dianshuMul);
    ...
    for (auto &it : m_UserInfo)
    {
        ...
        int64_t betDaXiao = userInfoItem->dAreaJetton[0]-userInfoItem->dAreaJetton[1];
        int64_t betDanShuang = userInfoItem->dAreaJetton[2]-userInfoItem->dAreaJetton[3];
        int64_t betFour = 0;
        for(int i=0;i<4;i++)
        {
            betFour+=userInfoItem->dAreaJetton[i];
        }
        if(betDaXiao<0)
        {
        ...
        scoreInfo.rWinScore = userInfoItem->dCurrentPlayerJetton-betFour+betDanShuang+betDaXiao; //
        //牌型
...
{
    CINIHelp *pini = new CINIHelp("conf/tb_config.ini");
    ...
    m_lBlackBetLimit = pini->GetLong("GAME_CONFIG", "BlackBetLimit", 10000);
    m_lControlAndLoseLimit = pini->GetLong("GAME_CONFIG", "ControlAndLoseLimit", 500000);
	...
	// 
    testIsOpen = pini->GetFloat("TEST", "TestRet", 0);
    testList[0] = pini->GetFloat("TEST", "TestList1", 1);
    testList[1] = pini->GetFloat("TEST", "TestList2", 2);
    testList[2] = pini->GetFloat("TEST", "TestList3", 2);
...
void TableFrameSink::NormalGameStart()
{
    ...
    for (auto &it : m_UserInfo)
    {
		...
		for (auto &uInfoItem : m_pPlayerInfoList)
		{
            ...
            Tb::PlayerListItem* palyer = gameStart.add_players();
            palyer->set_dwuserid(uInfoItem->wUserid);
			palyer->set_headerid(uInfoItem->headerId);
			palyer->set_nviplevel(0);
			palyer->set_nickname(uInfoItem->nickName);
        ...
        m_pITableFrame->SendTableData(pUserItem->GetChairId(), Tb::SUB_S_GameStart, (uint8_t *)endData.c_str(), endData.size());
    }
}
...
void TableFrameSink::Algorithm()
{
    ...
    if(randomRet<m_difficulty)
    {
        ...
        tbAlgorithm.SetAlgorithemNum(m_llStorageHighLimit ,m_llStorageLowerLimit, m_llStorageControl,m_dControlkill, m_lLimitPoints);//设置上下限,当前利润,干预难度,超红判断值
        tbAlgorithm.SetMustKill(-1);
        for(int i=0;i<MAX_JETTON_AREA;i++)
         tbAlgorithm.SetBetting(i,m_pBetArea->AndroidmifourAreajetton[i]);//设置每门真人押注值
        tbAlgorithm.GetOpenCard(m_strResultInfo);
    ...
    else
    {
        ...
        {
            float probilityAll[10]={0.0f};  //每一门的控制系数
            //user probilty count
...
//                   }
            ...
            for(int i=0;i<MAX_JETTON_AREA;i++)
             tbAlgorithm.SetBetting(i,m_pBetArea->AndroidmifourAreajetton[i]);//设置每门真人押注值
            tbAlgorithm.BlackGetOpenCard(m_strResultInfo,gailv,blackerBet,m_lControlAndLoseLimit);
            openSLog("黑名单算法最终出结果: %d     %d      %d",m_strResultInfo.touList[0],m_strResultInfo.touList[1],m_strResultInfo.touList[2]);
        }
        else
        {
            tbAlgorithm.SetAlgorithemNum(m_llStorageHighLimit ,m_llStorageLowerLimit, m_llStorageControl,m_dControlkill, m_lLimitPoints);//设置上下限,当前利润,干预难度,超红判断值
            for(int i=0;i<MAX_JETTON_AREA;i++)
             tbAlgorithm.SetBetting(i,m_pBetArea->AndroidmifourAreajetton[i]);//设置每门真人押注值
            tbAlgorithm.GetOpenCard(m_strResultInfo);
            openSLog("正常算法最终出结果: %d     %d      %d",m_strResultInfo.touList[0],m_strResultInfo.touList[1],m_strResultInfo.touList[2]);
        }
    ...
    m_lOpenResultAllCount++;
    if (m_lOpenResultAllCount >= 10000000)
    {
        m_lOpenResultAllCount = 200;
    }
}
...
void TableFrameSink::NormalCalculate()
{
    ...
    if(testIsOpen)
    {
        ...
        for(int i=0;i<MAX_JETTON_AREA;i++)
         tbAlgorithm.SetBetting(i,m_pBetArea->AndroidmifourAreajetton[i]);//设置每门真人押注值
        tbAlgorithm.TestOpenCard(m_strResultInfo,testList);
    }

    LOG(WARNING) <<"骰子1**********"<<m_strResultInfo.touList[0];
    LOG(WARNING) <<"骰子2**********"<<m_strResultInfo.touList[1];
    LOG(WARNING) <<"骰子3**********"<<m_strResultInfo.touList[2];
    LOG(WARNING) <<"**********点数"<<m_strResultInfo.iDianshu;
    for(int i=0;i<eMax;i++)
    {
    ...
    for (auto &it : m_UserInfo)
    {
        ...
        Tb::ResultStr* res = gameEnd.mutable_resultinfo();
        for(int i=0;i<3;i++)
         res->add_ndicepoints(m_strResultInfo.touList[i]);
        for(int i=0;i<eMax;i++)
         res->add_nwinmutic(m_strResultInfo.winList[i]);
        ...
        for (auto &uInfoItem : m_pPlayerInfoList)
		{
            ...
            Tb::PlayerListItem *palyer = gameEnd.add_players();
            palyer->set_dwuserid(uInfoItem->wUserid);
            palyer->set_headerid(uInfoItem->headerId);
			palyer->set_nviplevel(0);
            palyer->set_nickname(uInfoItem->nickName);
        ...
        m_pITableFrame->SendTableData(pUserItem->GetChairId(), Tb::SUB_S_GameEnd, (uint8_t *)endData.c_str(), endData.size());
...
void TableFrameSink::SendPlayerList(uint32_t chairid, uint8_t subid, const uint8_t *data, uint32_t datasize)
{
    ...
    if (m_iGameState == Tb::SUB_S_GameEnd)
    {
        if (m_vPlayerList.size() > 1)
        {
            Tb::CMD_S_UserWinList PlayerList;
            for (int i = m_vPlayerList.size() - 2; i > -1; i--)
            {
                ...
                Tb::PlayerInfo *playerinfo = PlayerList.add_player();
            ...
            m_pITableFrame->SendTableData(chairid, Tb::SUB_S_PLAYERLIST, (uint8_t *)sendData.c_str(), sendData.size());
        }
    }
    else
    {
...
bool TableFrameSink::GamePlayerJetton(uint32_t chairid, uint8_t subid, const uint8_t *data, uint32_t datasize)
{
    ...
    if (!bPlaceJetScuccess || 1!= m_bBetStatus)
    {
        ...
        if(!pSelf->IsAndroidUser()){
            Tb::CMD_S_PlaceJettonFail placeJetFail;
            ...
            // if (m_bTest==0)
               m_pITableFrame->SendTableData(pSelf->GetChairId(), Tb::SUB_S_JETTON_FAIL, (uint8_t *)sendData.c_str(), sendData.size());
        }
    ...
    for (auto &it : m_UserInfo)
    {
        ...
        if(!pServerUserItem)
            continue;
        Tb::CMD_S_PlaceJetSuccess placeJetSucc;
        placeJetSucc.Clear();
        ...
        // if (m_bTest==0)
            m_pITableFrame->SendTableData(userInfoItem->wCharid, Tb::SUB_S_JETTON_SUCCESS, (uint8_t *)sendData.c_str(), sendData.size());
        pmayernum++;
    }
...
//    if (1!= m_bBetStatus)
//    {
//        LOG(WARNING) << "----------------游戏状态不对，不能押注----------" << m_iGameState << " " << Tb::SUB_S_GameStart;
//        return;
//    }
    int alljetton = 0;
    ...
    if(1!= m_bBetStatus)
    {
        ...
        std::string sendData = placeJetFail.SerializeAsString();
        m_pITableFrame->SendTableData(Usr->GetChairId(), Tb::SUB_S_JETTON_FAIL, (uint8_t *)sendData.c_str(), sendData.size());
        return;
    }
    // 续押失败
    ...
    for (int i = 0; i < BET_ITEM_COUNT; i++)
    {
        ...
        if (m_nAreaMaxJetton[i] < m_UserInfo[userid]->dAreaJetton[i] + m_UserInfo[userid]->dLastJetton[i])
        {
            ...
            std::string data = placeJetFail.SerializeAsString();
             //if (m_bTest==0)
                m_pITableFrame->SendTableData(Usr->GetChairId(), Tb::SUB_S_JETTON_FAIL, (uint8_t *)data.c_str(), data.size());
...
void TableFrameSink::GamePlayerQueryPlayerList(uint32_t chairId, uint8_t subid, const uint8_t *data, uint32_t datasize, bool IsEnd)
{
    ...
    int32_t endindex = 0;
    Tb::CMD_S_PlayerList playerlist;
    ...
    for (vector<UserInfo>::iterator it = m_pPlayerInfoList_20.begin(); it != m_pPlayerInfoList_20.end(); it++)
    {
        ...
        Tb::PlayerListItem *item = playerlist.add_players();
    ...
    m_pITableFrame->SendTableData(pIIServerUserItem->GetChairId(), Tb::SUB_S_QUERY_PLAYLIST, (uint8_t *)lstData.c_str(), lstData.size());
	PlayerRichSorting();
}
// 发送场景消息(押分阶段)
void TableFrameSink::SendGameSceneStart(int64_t userid, bool lookon)
{
    ...
    Tb::CMD_S_Scene_GameStart scenceStart;
    ...
    for (int i = 0; i < BET_ITEM_COUNT; i++)
    {
        Tb::CMD_AeraInfo *areainfo = scenceStart.add_aerainfo();
	...
	for (auto &uInfoItem : m_pPlayerInfoList)
	{
        ...
        Tb::PlayerListItem *palyerItem = scenceStart.add_players();
    ...
    Tb::ResultStr * lastresult = scenceStart.mutable_nlastresult();
    lastresult->set_ndanshuang(m_lastRetId.iDanshuang);
    lastresult->set_ndaxiao(m_lastRetId.iDaxiao);
    for(int i=0;i<eMax;i++)
    {
    ...
    for (int i = m_vResultRecored.size() - 1; i >= 0; i--)
    {
        ...
        Tb::ResultStr * sResultList = scenceStart.add_sresultlist();
        sResultList->set_ndanshuang(m_vResultRecored[i].iDanshuang);
        sResultList->set_ndaxiao(m_vResultRecored[i].iDaxiao);
        for(int j=0;j<eMax;j++)
        {
            if(j<3)sResultList->add_ndicepoints(m_vResultRecored[i].touList[j]);
            sResultList->add_nwinmutic(m_vResultRecored[i].muticlList[j]);
        }
    }
    ...
    m_pITableFrame->SendTableData(player->GetChairId(), Tb::SUB_S_SCENE_START, (uint8_t *)sendData.c_str(), sendData.size());
}
...
void TableFrameSink::SetGameRecordDetail(int64_t userid, int32_t chairId, int64_t userScore, shared_ptr<UserInfo> userInfo)
{
	...
	//记录当局游戏详情binary
	string strDetail = "";
    Tb::CMD_S_GameRecordDetail details;
	details.set_gameid(m_pITableFrame->GetGameRoomInfo()->gameId);
    Tb::ResultStr* res = details.mutable_res();
    res->set_ndanshuang(m_strResultInfo.iDanshuang);
    res->set_ndaxiao(m_strResultInfo.iDaxiao);
    for(int i=0;i<eMax;i++)
    {
        if(i<3)res->add_ndicepoints(m_strResultInfo.touList[i]);
	...
	//if (!m_pITableFrame->IsExistUser(chairId))
	{
		for (int betIdx = 0;betIdx < BET_ITEM_COUNT; betIdx++)
		{
			...
			{
                Tb::BetAreaRecordDetail* detail = details.add_detail();
				//下注区域
...
{
	//记录
    Tb::CMD_S_GameRecord gameRecordRsp;
    ...
    for(int i=0;i<eMax;i++)
    {
        ...
        Tb::GameRecordList *gamerecordList = gameRecordRsp.add_recordlists();
        for (int j = 0; j < vResultList.size(); j++)
        {
            Tb::GameRecordListItem *listItem = gamerecordList->add_records();
            listItem->set_danshuang(vResultList[j].iIsDanShuangBao);
            listItem->set_daxiao(vResultList[j].iIsDaXiaoBao);
            listItem->set_points(vResultList[j].iResultDian);
    ...
    m_pITableFrame->SendTableData(chairid, Tb::SUB_S_QUERY_GAMERECORD, (uint8_t *)sendData.c_str(), sendData.size());
}
