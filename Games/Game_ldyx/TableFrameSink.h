...
public:
    ...
    virtual int BuildRoundScene(const int32_t wChairID, int64_t lScore);

    virtual int BuildMarryScene(const int32_t wChairID);



    virtual int BuildBibeiScene(const int32_t wChairID,const int32_t bettingPlace);
    bool CheckRule(CMD_S_RoundResult_Tag &SceneOneEnd, bool IsFreeScene);
    bool CalculateScore(CMD_S_RoundResult_Tag &SceneOneEnd, const int32_t wChairID);
    bool FillTable(CMD_S_RoundResult_Tag &SceneOneEnd);
    UserProbabilityItem ScoreGetProbability(int64_t StoreScore);//, UserProbabilityItem &UserPro);
    bool MarrySceneWriteScore(const int32_t wChairID, const int64_t lWinScore, int64_t &llGameTax,string cardValueStr);
    bool RoundSceneWriteScore(const int32_t wChairID,  const int64_t lBetScore, const int64_t lWinScore, int64_t &llGameTax,string cardValuesStr);
    void WriteFreeGameRecord(int32_t wChairID,int64_t betInfo,int32_t freeLeft,int32_t marryLeft,int32_t allMarry);


    bool WriteBiBeiGameRecord(const int32_t wChairID, const int64_t lWinScore, const int64_t lBetScore,int64_t &lGameTax,string cardValueStr);
    ...
    void GetAllResult(CMD_S_RoundResult_Tag &SceneOneEnd,int chairid ,int  marrytimes,int64_t &allMarryWin);
private:
    //初始化游戏数据
    void InitGameData();
...
private:
    ...
    int64_t m_llUserCurrentScore[GAME_PLAYER]; //用户原始积分

    int64_t m_llUserJettonScore[GAME_PLAYER][FRUIT_MAX]; //下注分数

    int32_t m_GameMutical[FRUIT_MAX]; //下注分数
	STD::Random m_random;
    ...
    int64_t m_lUserBetScore[GAME_PLAYER];      //玩家下注总金额
    int64_t m_lUserWinScore[GAME_PLAYER];      //玩家旋转场景赢分
    int64_t m_lUserBiBeiBet[GAME_PLAYER];
    int64_t m_lUserBiBeiBetOld[GAME_PLAYER];
    int32_t m_lUserIsAward[GAME_PLAYER];       //是否中大奖,方便后面计算分数
    uint8_t m_lUserAwardIndex[GAME_PLAYER];    //中大奖的具体水果下标
    uint8_t m_lUserAwardMutical[FRUIT_MAX];    //中大奖的具体水果的倍率表


    bool m_IsPressJetton[GAME_PLAYER];      //玩家下注总金额
    int64_t m_lBibeiAllWin[GAME_PLAYER];
...
private:                                  //Storage Related
    ...
    vector<MarryInfo>  vtMarryResult; //
