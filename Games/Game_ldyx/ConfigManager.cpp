...
int	 ConfigManager::CheckReadConfig()
{
    ...
    configDir = str(boost::format("./conf/ldyx_config.ini"));
    if(!boost::filesystem::exists(configDir)) {
        LOG(WARNING) << "./conf/ldyx_config.ini not exists";
        return 0;
    }
    ...
    m_Config.maxBetScore = pt.get<int32_t>("FruitConfig.MaxBetScore", 9);
        ...
        memset(KeyName, 0, sizeof(KeyName));
        {
            for(int i=0;i<3;i++)
            {
                ...
                //解析
                {
                    ...
                    p = mystchr(p, ',') + 1;  temp = atoi(p);
                    Item.nFruitPro[12] = temp;
                    p = mystchr(p, ',') + 1;  temp = atoi(p);
                    Item.nFruitPro[13] = temp;
                    p = mystchr(p, ',') + 1;  temp = atoi(p);
                    Item.nFruitPro[14] = temp;
                    p = mystchr(p, ',') + 1;  temp = atoi(p);
                    Item.nFruitPro[15] = temp;
                    p = mystchr(p, ',') + 1;  temp = atoi(p);
                    Item.nFruitPro[16] = temp;
                    p = mystchr(p, ',') + 1;  temp = atoi(p);
                    Item.nFruitPro[17] = temp;
                }
    ...
    //测试
    {
        ...
        m_Config.m_testMarry = pt.get<int64_t>("Test.testMarry", 0);
        m_Config.m_marryIndex = pt.get<int64_t>("Test.marryindex", 0);
        m_Config.m_testBibei = pt.get<int64_t>("Test.testBibei", 0);
        ...
        if(m_Config.m_testBibei)
        {
            int readSize = GetPrivateProfileString(INI_SECTION_CONFIG, "bibeiList", DefualtBuf, OutBuf, sizeof(OutBuf), configDir.c_str());
            LOG(WARNING) << " OutBuf "<< OutBuf;
            ASSERT(readSize > 0);
            p = OutBuf;
            if(p==nullptr){
            ...
            int32_t temp = atoi(p);
            m_Config.m_bibiList[0] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
            m_Config.m_bibiList[1] = temp;
        }
        if(m_Config.m_testMarry > 0)
        {
            int readSize = GetPrivateProfileString(INI_SECTION_CONFIG, "marrylist", DefualtBuf, OutBuf, sizeof(OutBuf), configDir.c_str());
            LOG(WARNING) << " OutBuf "<< OutBuf;
            ASSERT(readSize > 0);
            p = OutBuf;
            if(p==nullptr){
            ...
            int32_t temp = atoi(p);
            m_Config.m_marryList[0] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
            m_Config.m_marryList[1] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
            m_Config.m_marryList[2] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
            m_Config.m_marryList[3] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
            m_Config.m_marryList[4] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
            m_Config.m_marryList[5] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
            m_Config.m_marryList[6] = temp;
            p = mystchr(p, ',') + 1; temp = atoi(p);
            m_Config.m_marryList[7] = temp;
        }
        if( m_Config.m_testLine > 0 )
        {
            m_Config.m_line[0] = pt.get<int64_t>("Test.line1", 0);
...
void ConfigManager::GetConfig(tagAllConfig &config) 
{
   ...
   config.m_testMarry = m_Config.m_testMarry;
   config.m_marryIndex = m_Config.m_marryIndex;
   config.m_testBibei = m_Config.m_testBibei;

   config.maxBetScore = m_Config.maxBetScore;
   for(size_t i = 0; i < 8; i++)
   {
       if(i<2) config.m_bibiList[i]=m_Config.m_bibiList[i];

       config.m_marryList[i] = m_Config.m_marryList[i];
   }
   ...
   config.m_line[0] = m_Config.m_line[0];
}
