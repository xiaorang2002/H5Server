...
//////////////////////////////////////////////////////////////////////////////////////
int32_t CTableFrameSink::m_cbBankerMultiple[MAX_BANKER_MUL] = { 1,2,3,4 };		//叫庄倍数
int32_t CTableFrameSink::m_cbJettonMultiple[MAX_JETTON_MUL] = {1, 5,10,15,20,25};	//下注倍数
...
//        memcpy(m_cbJettonMultiple, (vector<uint8_t>)m_pGameRoomInfo->jettons, sizeof(m_cbJettonMultiple));
		...
		{
			m_isMatch = true;
			m_cbBankerMultiple[0] = 1;
			m_cbBankerMultiple[1] = 2;
			m_cbBankerMultiple[2] = 4;
		}
    }
    //初始化游戏数据
    m_wGameStatus = GAME_STATUS_INIT;
...
//        for (int i = 0; i < MAX_JETTON_MUL; ++i)
//            GameCall.add_cbjettonmultiple(m_cbJettonMultiple[i]);
		int MaxBankerMul = m_isMatch ? 3 : MAX_BANKER_MUL;
        for (int i = 0; i < MaxBankerMul; ++i){
...
//        GameCall.set_szheadurl(pIServerUser->GetHeadBoxID());
    ...
    case GAME_STATUS_SCORE:		//下注状态
    {
		...
		int MaxJettonMul = m_isMatch ? 4 : MAX_JETTON_MUL;
        for (int i = 0; i < MaxJettonMul; ++i)
        {
...
int CTableFrameSink::OnUserCallBanker(int wChairID, int wCallFlag)
{
    ...
    if (wCallFlag != 0)
    {
        for (int cbIndex = 0; cbIndex != MaxBankerMul; ++cbIndex)
        {
    ...
    if (wBankerUser != INVALID_CHAIR)
    {
        ...
        for(int j = 0; j < GAME_PLAYER; j++ )
        {
            if( m_cbPlayStatus[j] == 1 && j != wBankerUser)
            {
				...
				int jettonsLen = m_pGameRoomInfo->jettons.size();
                if( callMaxTimes >= m_pGameRoomInfo->jettons[jettonsLen-1]){
                    for (int i = 0; i < MaxJettonMul; ++i)
                    {
                        m_cbJettonable[j][i]=m_cbJettonMultiple[i];
                        CallResult.add_cbjettonmultiple(m_cbJettonMultiple[i]);
                    }
                }else if(callMaxTimes >= jettonsLen){
                    for (int i = 0; i < MaxJettonMul; ++i)
                    {
                        if(i==0)
                        {
                           m_cbJettonable[j][i]=1;
                        }
                        else
                        {
                            m_cbJettonable[j][i]=callMaxTimes*(i+1) / jettonsLen;
                        }
                        CallResult.add_cbjettonmultiple(m_cbJettonable[j][i]);
                    }
                }else{
                    for (int i = 1; i <= MaxJettonMul; ++i)
                    {
                        if(i <= callMaxTimes)
                        {
...
int CTableFrameSink::OnUserAddScore(int wChairID, int wJettonIndex)
{
    ...
    //下注筹码错误
	int MaxJetton = m_isMatch ? 4 : MAX_JETTON_MUL;
    if (wJettonIndex < 0 || wJettonIndex >= MaxJetton)  //5 10 15 20
    {
        return 3;
    }
...
void CTableFrameSink::NormalGameEnd(int dwChairID)
{
	...
	//记录当局游戏详情binary
	Qznn::CMD_S_GameRecordDetail details;
...
//        }
		...
		Qznn::PlayerRecordDetail* detail = details.add_detail();
		//账号/昵称
		...
		//手牌/牌型
        Qznn::HandCards* pcards = detail->mutable_handcards();
