...
//检验是否服务器的牌值
bool CGameLogic::checkSameCard(uint8_t *servercards, uint8_t *clientcards)
{
	//排序大到小
...
//是否同样的牌值
bool CGameLogic::IsSameCard(uint8_t cbFirstCard[], uint8_t cbNextCard[])
{
    //获取点数
    uint8_t cbFirstType = GetCardType(cbFirstCard, MAX_COUNT);
    uint8_t cbNextType = GetCardType(cbNextCard, MAX_COUNT);
    ...
    else if(cbFirstType == SG_THREE_POKER)
    {
        ...
        //比较颜色
        if (GetCardColor(bFirstTemp[0]) > GetCardColor(bNextTemp[0]))
            bSame = false;
    }
    else
    {
...
uint8_t CGameLogic::GetMultiple(uint8_t cbCardData[])
{
    ...
    if (cbCardType <= 6)return 2;
    else if (cbCardType <= 9)return 3;
    else if (cbCardType == SG_THREE_POKER)return 10;
    else if (cbCardType == SG_BOMB)return 20;
    else if (cbCardType == SG_BOMB_NINE)return 40;
...
bool CGameLogic::GetMatchPointCard(int32_t iPoint,uint8_t cbCardData[MAX_SENDCOUNT], uint8_t byCardDataArray[MAX_CARD_TOTAL])
{
    ...
    if(iPoint < SG_THREE_POKER)  //点数牌
    {
        ...
        else
        {
            ...
            }while(IsUsedCard(cbCardData[0]) || GetCardValue(cbCardData[0]) < 10);
            InsertUsedCard(cbCardData[0]);
            ...
            }while(IsUsedCard(cbCardData[1]) || (cbCardData[0] == cbCardData[1])||(GetCardValue(cbCardData[1]))>10);
            InsertUsedCard(cbCardData[1]);
        }
        ...
        else
        {
            ...
            //第4张牌
            uint8_t cbCardData4 = GetSameTypeCard(cbCardData, byCardDataArray);        
            cbCardData[3] = cbCardData4;
            InsertUsedCard(cbCardData[3]);
            return true;
        }
    }
    else if(iPoint == SG_BOMB_NINE) //爆玖
    {
        //随机获取花色
        bNeedValue = 3;
    ...
    else if(iPoint == SG_THREE_POKER) //三公
    {
        do
        {
            cbCardData[0] = byCardDataArray[index++];
...
//添加一张牌得到相同牌型
uint8_t CGameLogic::GetSameTypeCard(uint8_t cbCardData[MAX_SENDCOUNT], uint8_t byCardDataArray[MAX_CARD_TOTAL])
{
    uint8_t cbData = 0;
    int index2 = 0;
    uint8_t bcardTemp[MAX_COUNT] = { 0 };
    uint8_t bcardTemp2[MAX_SENDCOUNT] = { 0 };
    for (int i = 0; i < MAX_COUNT; ++i)
    {
        bcardTemp[i] = cbCardData[i];
    }
    memcpy(bcardTemp2, cbCardData, sizeof(bcardTemp2));
    int cardType = GetCardType(bcardTemp, MAX_COUNT);
    bool isBreak = false;
    bool isFirst = true;
    do
    {
        cbData = byCardDataArray[index2++];
        if(index2 == 52) 
        {
            index2 = 0;
            isFirst = false;           
        }
        else if (!isFirst && IsUsedCard(cbData))
        {
            break;
        }
        bool isUsed = IsUsedCard(cbData);
        bool IsSameType = IsSameTypeCard(bcardTemp2,cbData,cardType);
        if(!isUsed && IsSameType)
            isBreak = true;
    }while(!isBreak);

    return cbData;
}
bool CGameLogic::GetResolvePointCard(string& sValue,uint8_t cbCardData[MAX_SENDCOUNT], uint8_t byCardDataArray[MAX_CARD_TOTAL])
{
    ...
    if(position1 != sValue.npos)
    {
        ...
        for( vector<string>::iterator it = vStr.begin(); it != vStr.end(); ++ it )
        {
            ...
            InsertUsedCard(stod((*it).c_str()));
        }
        // //第4张牌
        // uint8_t cbCardData4 = GetSameTypeCard(cbCardData, byCardDataArray);        
        // cbCardData[Index++] = cbCardData4;
        // InsertUsedCard(cbCardData4);
...
//获取玩家最优牌型组合
uint8_t CGameLogic::GetBigTypeCard(uint8_t cbHandCardData[MAX_SENDCOUNT], uint8_t cbCardData[MAX_COUNT])
{
        ...
        for (int j = i+1; j < MAX_SENDCOUNT-1; ++j)
        {
            for (int k = j+1; k < MAX_SENDCOUNT; ++k)
            {
                ...
                else
                {
                    ...
                    {
                        if (CompareCard(bTempcard,cbCardData))
                        {
                            memcpy(cbCardData,bTempcard,MAX_COUNT);
                        }
                    }
...
bool CGameLogic::IsSameTypeCard(uint8_t cbCardData[MAX_SENDCOUNT],uint8_t cbData,int cardType)
{
    ...
    for (int i = 0; i < MAX_SENDCOUNT-2; ++i)
    {
        for (int j = i+1; j < MAX_SENDCOUNT-1; ++j)
        {
            for (int k = j+1; k < MAX_SENDCOUNT; ++k)
            {
                bTempcard[0] = bTempcard2[i];
                bTempcard[1] = bTempcard2[j];
                bTempcard[2] = bTempcard2[k];
                tempType = GetCardType(bTempcard,MAX_COUNT);
                if (tempType>maxType)
                {
                    maxType = tempType;
