...
public:
    CGLogInit()
    {
        ...
        google::InitGoogleLogging("QZXSanGong");
...
void  CTableFrameSink::openSLog(const char *p, ...)
{
    ...
    sprintf(Filename, "log//QZXsangong//QZXsangong_table_%d%d%d_%d_%d.log",t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,m_pITableFrame->GetGameRoomInfo()->roomId,m_pITableFrame->GetTableId());
...
{
    //=====config=====
    if(!boost::filesystem::exists("./conf/qzxsg_config.ini"))
    {
        return 0;
    }

    boost::property_tree::ptree pt;
    boost::property_tree::ini_parser::read_ini("./conf/qzxsg_config.ini", pt);

    stockWeak=pt.get<double>("Global.STOCK_WEAK",1.0);
    // 是否写日记
    m_bWritelog = false;
    if (pt.get<uint32_t>("Global.nWritelog", 1)!=0)
    {
        m_bWritelog = true;
    }
...
void CTableFrameSink::PaoMaDeng()
{
    ...
    for(int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        // if(PaoMaDengCardType[i] == SG_BOMB_NINE)
        // {
        ...
        //     }
        // }
        // else if(PaoMaDengCardType[i] == SG_BOMB)
        // {
        //     if(PaoMaDengWinScore[i] >= broadcastScore)
        //     {
        //         YouXiaoWin[i] = PaoMaDengWinScore[i];
        ...
        //     }
        // }
        // else if(PaoMaDengCardType[i] == SG_THREE_POKER)
        // {
        //     if(PaoMaDengWinScore[i] >= broadcastScore)
        //     {
        //         YouXiaoWin[i] = PaoMaDengWinScore[i];
...
bool CTableFrameSink::OnUserEnter(int64_t userId, bool bisLookon)
{
    ...
    if(m_pITableFrame->GetGameStatus() == GAME_STATUS_INIT || m_cbStatus == XSG_GAME_STATUS_DEFAULT)
    {
...
bool CTableFrameSink::RepayRecordGameScene(int64_t chairid, bool bisLookon)
{
    ...
    switch (m_cbStatus)
    {
    case XSG_GAME_STATUS_INIT:
    case XSG_GAME_STATUS_READY:			//空闲状态
    case XSG_GAME_STATUS_LOCK_PLAYING:
    {
        XSG_MSG_GS_FREE GameFree;
        GameFree.set_dcellscore((m_dwCellScore));//单元分


        uint32_t dwStartTime = 0;
        if(m_cbStatus == XSG_GAME_STATUS_INIT)
            dwStartTime = m_dwInitTime;
        else if(m_cbStatus == XSG_GAME_STATUS_READY)
    ...
    case XSG_GAME_STATUS_CALL:			//叫庄状态
    {
        XSG_MSG_GS_CALL GameCall;
    ...
    // case XSG_GAME_STATUS_SCORE:		//下注状态
    // {
    //     XSG_MSG_GS_SCORE GameScore;
    ...
    //     break;
    // }
    case XSG_GAME_STATUS_OPEN:		//开牌场景消息
    {
        XSG_MSG_GS_OPEN GameOpen;
...
bool CTableFrameSink::OnUserLeft(int64_t userId, bool islookon)
{
    ...
    if(m_pITableFrame->GetGameStatus() == GAME_STATUS_INIT || m_cbStatus == XSG_GAME_STATUS_READY ||
    ...
    if(XSG_GAME_STATUS_READY == m_cbStatus)
    {
        int userNum = 0;
        for(int i = 0; i < GAME_PLAYER; ++i)
        {
...
void CTableFrameSink::GameTimerReadyOver()
{
    ...
    if(wUserCount >= 2)
    {
        ...
        m_cbStatus = XSG_GAME_STATUS_LOCK_PLAYING;
    ...
    else
    {
        ...
        ClearAllTimer();
        m_mUserInfo.clear(); // Add by guansheng 20190401
        m_cbStatus = XSG_GAME_STATUS_INIT;
...
void CTableFrameSink::OnGameStart()
{
    ...
    for (int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        if (bTestCard)
        {
            if(m_UserTest.end() != itAcc)
            {
                // find Account
               // m_TestRandn++;
               // if (m_TestRandn>12) m_TestRandn = 0;
               // m_UserTest[strAccount] = "suiji" + to_string(m_TestRandn);
                openSLog("------------测试账号做牌---------------,chairid = %d;",i);
        ...
        else
        {
            memcpy(&m_cbHandCardData[i], &cbTempArray[m_cbLeftCardCount], sizeof(uint8_t)*MAX_SENDCOUNT);
             ...
             // {
             //     uint8_t cbText[MAX_SENDCOUNT] = { 0x2b,0x1a,0x16,0x2a };
    ...
    for (int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        m_pITableFrame->SendTableData(i, XSG_SUB_S_GAME_START_AI, (uint8_t*)data.c_str(), data.size());
    }
    ...
    for (int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        //发送数据
        m_pITableFrame->SendTableData(i, XSG_SUB_S_GAME_START, (uint8_t*)data.c_str(), data.size());
        //m_mOutCardRecord[i].append(" fetch " + StringCards(m_cbCardData[i], MAX_COUNT));
    }

    //设置游戏状态
    m_cbStatus = XSG_GAME_STATUS_CALL;
    ...
    if(m_bReplayRecordStart)
    {
        ...
        RepayRecord(INVALID_CHAIR, XSG_SUB_S_GAME_START, data.data(),data.size());
    }
...
int CTableFrameSink::OnUserCallBanker(int wChairID, int wCallFlag)
{
    ...
    //叫庄
    XSG_CMD_S_CallBanker callBanker;
    ...
    {
        if (m_cbPlayStatus[i])
        m_pITableFrame->SendTableData(i, XSG_SUB_S_CALL_BANKER, (uint8_t*)data.c_str(), data.size());
    }

    //tmp......
    if(m_bReplayRecordStart)
    {
        RepayRecord(INVALID_CHAIR, XSG_SUB_S_CALL_BANKER, data.c_str(),data.size());
    }
    //记录叫庄玩家
    int wBankerUser = INVALID_CHAIR;

    XSG_CMD_S_CallBankerResult CallResult;
    ...
    if(wTotalCount == m_wPlayCount)   //所有的玩家都进行了叫庄操作(叫庄或者不叫)
    {
        ...
        //设置游戏状态
        // m_cbStatus = XSG_GAME_STATUS_OPEN;   // XSG_GAME_STATUS_SCORE;
        ...
        if (wBankerUser != INVALID_CHAIR)
        {
            ...
            LOG(ERROR) << "XSG_SUB_S_CALL_BANKER_RESULT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" ;
            ...
            {                
                if (1 == m_cbPlayStatus[i])
                    m_pITableFrame->SendTableData(i, XSG_SUB_S_CALL_BANKER_RESULT, (uint8_t*)data.c_str(), data.size());
            }

            //tmp......
            if(m_bReplayRecordStart)
            {
                RepayRecord(INVALID_CHAIR, XSG_SUB_S_CALL_BANKER_RESULT, data.c_str(),data.size());
            }
            //发玩家手牌
            OnSendHandCard();
        }  
...
//     XSG_CMD_S_AddScoreResult CallResult;
...
//         m_cbStatus = XSG_GAME_STATUS_OPEN;
...
//     {
//         std::string data = CallResult.SerializeAsString();
//         RepayRecord(INVALID_CHAIR, XSG_SUB_S_ADD_SCORE_RESULT, data.c_str(),data.size());
//     }
...
//         std::string data = CallResult.SerializeAsString();
//         m_pITableFrame->SendTableData(i, XSG_SUB_S_ADD_SCORE_RESULT, (uint8_t*)data.c_str(), data.size());

//         if (1 == cbIsSendCard)
//         {
//             //有发牌
//             XSG_CMD_S_SendCard sendCard;
//             for(int j = 0; j < MAX_COUNT; ++j)
//             {
...
//             m_pITableFrame->SendTableData(i, XSG_SUB_S_SEND_CARD, (uint8_t*)data.c_str(), data.size());

//             //tmp......
//             if(m_bReplayRecordStart && 1 == m_cbPlayStatus[i])
//                 //if(m_bReplayRecordStart )
//             {
//                 RepayRecord(i, XSG_SUB_S_SEND_CARD, data.data(),data.size());
//             }
//         }
//     }
//     return 4;
    ...
    for (int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        if (1 == cbIsSendCard)
        {
            ...
            if(m_cbPlayStatus[i] != 0 )
            {
                ...
                LOG(INFO)<<"-------玩家手牌:wChairID="<<i<<",cbHandCardData="<<ChangCardIndex(m_cbHandCardData[i][0])<<","<<ChangCardIndex(m_cbHandCardData[i][1])<<","<<ChangCardIndex(m_cbHandCardData[i][2])<<","<<ChangCardIndex(m_cbHandCardData[i][3]);
                openSLog("玩家手牌:wChairID=%d:cbHandCardData=0x%02x,0x%02x,0x%02x,0x%02x;",i,m_cbHandCardData[i][0],m_cbHandCardData[i][1],m_cbHandCardData[i][2],m_cbHandCardData[i][3]);
                //最优牌型数据
                for(int j = 0; j < MAX_COUNT; ++j)
                {
                    sendCard.set_cbsgcard(j, cbSGCard[j]);
...
int CTableFrameSink::OnUserOpenCard(int wChairID,int cbOpenFlag, uint8_t cbCardData[MAX_COUNT])
{
    ...
    int nBigType = (int)m_GameLogic.GetCardType(m_cbCardData[wChairID], MAX_COUNT);
    int nChooseType = 0;
    //判断是否选择最优牌型
    if(cbOpenFlag!=0) //玩家自己选牌
    {
		...
		if (!bCheck)
		{
			XSG_CMD_S_OperateFail OperateFail;
			int reason = 0;
			OperateFail.set_cbreason(reason);

			std::string data = OperateFail.SerializeAsString();
			//发送数据
			m_pITableFrame->SendTableData(wChairID, XSG_SUB_S_OPERATE_FAIL, (uint8_t*)data.c_str(), data.size());
    ...
    //记录牌型
    m_sCardType[wChairID] = nBigType;//(int)m_GameLogic.GetCardType(m_cbCardData[wChairID], MAX_COUNT);
    ...
    //发送开牌结果
    XSG_CMD_S_OpenCardResult OpenCardResult;
    ...
    m_pITableFrame->SendTableData(INVALID_CHAIR, XSG_SUB_S_OPEN_CARD_RESULT, (uint8_t*)data.c_str(), data.size());

    //tmp......
    if(m_bReplayRecordStart)
    {
        RepayRecord(INVALID_CHAIR,XSG_SUB_S_OPEN_CARD_RESULT, data.c_str(),data.size());
    }
    //处理下注
    int wTotalCount = 0;
    for (int i = 0; i < GAME_PLAYER; ++i)
...
void CTableFrameSink::NormalGameEnd(int dwChairID)
{
    ...
    ****/
    ...
    LOG(INFO)<<" 庄家牌型倍数 m_wBankerUser====="<<(int)m_wBankerUser<<","<<(int)m_GameLogic.GetCardType(m_cbCardData[m_wBankerUser], MAX_COUNT)<<","<<(int)cbBankerCardMultiple;
    openSLog("庄家牌型倍数:m_wBankerUser=%d,cardType=%d,beishu=%d;",m_wBankerUser,(int)m_GameLogic.GetCardType(m_cbCardData[m_wBankerUser], MAX_COUNT),(int)cbBankerCardMultiple);
    //计算输赢
    for(int i = 0; i < GAME_PLAYER; ++i)
    {
        if(0 == m_cbPlayStatus[i])
        ...
        LOG(INFO)<<" 闲家牌型倍数:cbUserCardMultiple[i]="<< i <<","<<(int)m_GameLogic.GetCardType(m_cbCardData[i], MAX_COUNT)<<","<< (int)cbUserCardMultiple[i];
        openSLog("闲家牌型倍数:chairID=%d,cardType=%d,beishu=%d;",i,(int)m_GameLogic.GetCardType(m_cbCardData[i], MAX_COUNT),(int)cbUserCardMultiple[i]);
        //闲家原始积分
   ...
   // m_pITableFrame->WriteUserScore(m_RecordScoreInfo, userCount, m_strRoundId);
    ...
    m_pITableFrame->SendTableData(INVALID_CHAIR, XSG_SUB_S_GAME_END, (uint8_t*)data.c_str(), data.size());
    PaoMaDeng();//發送跑馬燈
    //tmp......
    if(m_bReplayRecordStart)
    {
        RepayRecord(INVALID_CHAIR,XSG_SUB_S_GAME_END, data.c_str(),data.size());
    }
...
void CTableFrameSink::WriteUserScore()
{
    ...
    for(auto &it : m_mUserInfo)
    {
        ...
        if(pUserItem)
        {
            ...
            if(m_wBankerUser != userInfoItem->chairId)
                m_replay.addResult(scoreInfo.chairId,scoreInfo.chairId,scoreInfo.betScore,scoreInfo.addScore,to_string(m_GameLogic.GetCardType(m_cbCardData[scoreInfo.chairId], MAX_COUNT))+":"+GlobalFunc::converToHex( m_cbHandCardData[scoreInfo.chairId] ,MAX_SENDCOUNT)+":"+GlobalFunc::converToHex( m_cbCardData[scoreInfo.chairId] ,MAX_COUNT),scoreInfo.isBanker==1);

            scoreInfo.startTime = m_startTime;
            scoreInfoVec.push_back(scoreInfo);
            LOG(ERROR)<<"QZXszSg scoreInfo.addScore="<<scoreInfo.addScore<<"---------------------";
            LOG(ERROR)<<"QZXszSg scoreInfo.revenue="<<scoreInfo.revenue<<"---------------------";
        }
    }
    ...
    m_replay.addResult(bankerChair,bankerChair,rBankerWinscore,scoreInfoVec[bankerScoreIdx].addScore,
                       to_string(m_GameLogic.GetCardType(m_cbCardData[bankerChair], MAX_COUNT))+":"+GlobalFunc::converToHex( m_cbHandCardData[bankerChair] ,MAX_SENDCOUNT)+":"+GlobalFunc::converToHex(m_cbCardData[bankerChair], MAX_COUNT),true);
    ...
    LOG(ERROR)<<"QZXszSg write score bWrite = "<<(int)bWrite<<"---------------------";
}
...
void CTableFrameSink::GameTimerEnd()
{
    ...
    //设置游戏状态
   // m_pITableFrame->SetGameStatus(GAME_STATUS_READY);
    m_cbStatus = XSG_GAME_STATUS_READY;
...
bool CTableFrameSink::OnGameMessage( uint32_t wChairID, uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    ...
    switch (wSubCmdID)
    {
        case XSG_SUB_C_CALL_BANKER:					//叫庄消息
        {
            LOG(INFO)<<"***************************USER CALL BANK1";
            //游戏状态不对
            if (XSG_GAME_STATUS_CALL != m_cbStatus)
            {
            ...
            //变量定义
            XSG_CMD_C_CallBanker  pCallBanker;
        ...
        // case XSG_SUB_C_ADD_SCORE:					//下注消息
        // {
        //     //游戏状态不对            
        //     if (XSG_GAME_STATUS_SCORE != m_cbStatus)
        //     {
        //         return false;
        //     }
        ...
        //     return wRtn > 0;
        // }
        case XSG_SUB_C_OPEN_CARD:					//开牌消息
        {
            m_bPlayerIsOperation[wChairID]=true;
            if (XSG_GAME_STATUS_OPEN != m_cbStatus)
            {
                //游戏状态不对
                return false;
            }
...
{
    int dwShowTime = 0;
    if (m_cbStatus == XSG_GAME_STATUS_CALL)
    {
        //删除超时定时器
        m_TimerLoopThread->getLoop()->cancel(m_TimerIdReadyOver);
        m_TimerLoopThread->getLoop()->cancel(m_TimerIdEnterPlaying);
    ...
    // else if (m_cbStatus == XSG_GAME_STATUS_SCORE)
    // {
    //     //删除超时定时器
    ...
    // }
    else if (m_cbStatus == XSG_GAME_STATUS_OPEN)
    {
        //删除超时定时器
        m_TimerLoopThread->getLoop()->cancel(m_TimerIdCallBanker);
        m_TimerLoopThread->getLoop()->cancel(m_TimerIdOpenCard);
    ...
    else if (m_cbStatus == XSG_GAME_STATUS_END)
    {
        //删除超时定时器
        m_TimerLoopThread->getLoop()->cancel(m_TimerIdOpenCard);
        m_TimerLoopThread->getLoop()->cancel(m_TimerIdEnd);
...
void CTableFrameSink::SetGameRecordDetail(int64_t userScore[], int64_t userWinScorePure[], int32_t calcMulti[])
{
	...
	for (int i = 0; i < GAME_PLAYER; ++i) {
		...
		int nType = (int)m_GameLogic.GetCardType(m_cbCardData[i], MAX_COUNT);
		pcards->set_ty(nType);
		//牌型倍数
