...
#include "proto/sghjk.Message.pb.h"
...
public:
    CGLogInit()
    {
        ...
        google::InitGoogleLogging("sghjk");
...
{
	assert(m_cbGameStatus < GAME_STATUS_START);
    //到这里已经可以达到开始的条件了，就可以把准备阶段的定时器关闭
    m_TimerLoopThread->getLoop()->cancel(m_TimerIdGameReadyOver);
    ...
    //   因为到这里还会清理掉线玩家和分数不足的玩家，所以还得重新判断人数是否到达最低开场人数只要到达最低要求可以开始
...
//游戏开始,到了这里是真正的开始了，开工没有回头箭，启动这里之后，必须一局以后才能退出 游戏了
...
bool CGameProcess::OnEventGameScene(uint32_t chairId, bool bIsLookUser)
{
    ...
    case GAME_STATUS_LOCK_PLAYING:
    {
        ...
        for(int i = 0; i < GAME_PLAYER-1; ++i)
        {
            ...
            if(user)
            {
                ::sghjk::HJK_PlayerItem* player = StatusFree.add_gameplayers();
    ...
    case GAME_STATUS_BET:
    {
        ...
        for(int i = 0; i < m_wBankerUser; ++i)
        {
            ...
            {
                ::sghjk::HJK_PlayerItem* player = StatusScore.add_gameplayers();
                player->set_cbchairid(i);
    ...
    case GAME_STATUS_INSURE:
    {
        ...
        for(int i = 0; i < m_wBankerUser; ++i)
        {
            ...
            if(hand->isactivated)
            {
                ::sghjk::HJK_PlayerItem* player = StatusInsure.add_gameplayers();
                player->set_cbchairid(i);
                if( hand->masterChair != -1)
                {
                    user = m_pITableFrame->GetTableUserItem(hand->masterChair);
                ...
                ::sghjk::HandItem* firstHand = player->mutable_cbfirsthand();
    ...
    case GAME_STATUS_PLAY:	//游戏状态
    {
        ...
        for(int i = 0; i < m_wBankerUser; i++)
        {
            ...
            if(hand->isactivated)
            {
                ::sghjk::HJK_PlayerItem* player = StatusPlay.add_gameplayers();
                player->set_cbchairid(i);
                if( hand->masterChair != -1)
                {
                    user = m_pITableFrame->GetTableUserItem(hand->masterChair);
                ...
                if(hand->isactivated)
                {
                    ::sghjk::HandItem* secondHandItem = player->mutable_cbsecondhand();
...
bool CGameProcess::OnUserInsure(int wChairID, bool buy,int betSeat )
{
    ...
    if( allInsured )
    {
        ...
        LOG(ERROR) << "No body insured or someone insured but banker is not sghjk";
        m_cbGameStatus = GAME_STATUS_PLAY;
...
void CGameProcess::InitDealCards()
{
    ...
    for(int i = 0; i < GAME_PLAYER; i++)
    {
        ...
        ::sghjk::HandCard* handCard = DealCard.add_cbhandcards();
    ...
    }else
    {
        ...
        // {
        //     shared_ptr<Hand> bankerHand = m_Hands[m_wBankerUser];
        //     if(bankerHand->cardtype == CT_sghjk)
        //     {
...
//                LOG(INFO) << "Score Index:" << (int)scoreIndex;
    ...
    for(vector<tagScoreInfo>::iterator it0 = scoreInfoVec.begin(); it0 != scoreInfoVec.end(); it0++)
    {
        ...
        ::sghjk::CMD_S_EndPlayer* player = GameEnd.add_pendplayers();
