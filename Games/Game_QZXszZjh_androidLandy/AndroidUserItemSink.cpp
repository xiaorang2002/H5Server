...
#include "../Game_QZXszZjhLandy/CMD_Game.h"
...
//    uint32_t chairId = m_pAndroidUserItem->GetChairId();
    ...
    XJH_CMD_C_CallBanker callBanker;
    callBanker.set_cbcallflag(cbCallBankerMul);

    //发送信息
    string content = callBanker.SerializeAsString();
    m_pTableFrame->OnGameEvent(m_wChairID,XJH_SUB_C_CALL_BANKER, (uint8_t*)content.data(), content.size());
    LOG(INFO)<<"******************************Send XJH_SUB_C_CALL_BANKER";
...
//     m_pTableFrame->OnGameEvent(m_wChairID,XJH_SUB_C_ADD_SCORE, (uint8_t*)content.data(), content.size());
//     LOG(INFO)<<"******************************Send XJH_SUB_C_ADD_SCORE";
...
void CAndroidUserItemSink::GameTimeOpencard()
{
    ...
    m_pTableFrame->OnGameEvent(m_wChairID,XJH_SUB_C_OPEN_CARD, (uint8_t*)content.data(), content.size());
    // LOG(INFO)<<"******************************Send XJH_CMD_C_OpenCard";
    // LOG(INFO)<<"******************************cbJettonIndex"<<(int)cbJettonIndex;
    // m_pTableFrame->OnGameEvent(m_wChairID, XJH_SUB_C_OPEN_CARD, NULL, 0);
}
...
bool CAndroidUserItemSink::OnGameMessage(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    switch (wSubCmdID)
    {
    case XJH_SUB_S_GAME_START_AI:
    {
        QZXszZjh::CMD_S_GameStartAi GameStartAi;
        if (!GameStartAi.ParseFromArray(pData,wDataSize))
            return false;
        m_cbSGCardType = GameStartAi.cbsgcarddata();
        m_isMaxCard = GameStartAi.ismaxcard();
        openLog("XJH_SUB_S_GAME_START_AI m_cbSGCardType=%d m_isMaxCard=%d",m_cbSGCardType,m_isMaxCard);
        m_m_cbPlayStatus = true;
        return true;
    }
    case XJH_SUB_S_GAME_START:
    {
        ClearAndroidCount=0;
        m_m_cbPlayStatus = true;
...
{
    XJH_CMD_S_CallBankerResult BankerResult;
...
// {
//     XJH_CMD_S_AddScoreResult AddScoreResult;
...
bool CAndroidUserItemSink::OnSubSendCard(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    ...
    openLog("OnSubSendCard 玩家手牌 i=%d HandcardData=0x%x,0x%x,0x%x,0x%x,0x%x;",m_wChairID,m_HandcardData[0],m_HandcardData[1],m_HandcardData[2],m_HandcardData[3],m_HandcardData[4]);
    //最优牌组合
...
bool CAndroidUserItemSink::OnSubOperateFail(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    ...
    if (OperateFail.cbreason() == 0) //不是最优牌型
    {
        ...
        string content = openCard.SerializeAsString();
        m_pTableFrame->OnGameEvent(m_wChairID,XJH_SUB_C_OPEN_CARD, (uint8_t*)content.data(), content.size());
        LOG(INFO)<<"******************************OnSubOperateFail Send XJH_CMD_C_OpenCard";
    }
...
bool CAndroidUserItemSink::ReadConfigInformation()
{
    ...
    //设置文件名
    //=====config=====
    if(!boost::filesystem::exists("./conf/xjh_config.ini"))
    {
        LOG_INFO << "conf/xjh_config.ini not exists";
        return false;
    }

    boost::property_tree::ptree pt;
    boost::property_tree::read_ini("./conf/xjh_config.ini", pt);
...
void  CAndroidUserItemSink::openLog(const char *p, ...)
{
    ...
    sprintf(Filename, "log//QZXjinhua//QZXjinhua_%d%d%d_%d_%d.log",t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,m_pTableFrame->GetGameRoomInfo()->roomId,m_pTableFrame->GetTableId());
