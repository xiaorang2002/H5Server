...
BYTE	CAndroidUserItemSink::m_cbJettonMultiple[MAX_JETTON_MUL] = { 5,10,15,20 };	//下注倍数
...
void CAndroidUserItemSink::GameTimerCallBanker()
{
    ...
    qzzjh::CMD_C_CallBanker callBanker;
    callBanker.set_cbcallflag(cbCallBankerMul);


    //发送信息
    string content = callBanker.SerializeAsString();
    m_pTableFrame->OnGameEvent(m_pAndroidUserItem->GetChairId(),qzzjh::SUB_C_CALL_BANKER, (uint8_t*)content.data(), content.size());
}
...
//    m_TimerAddScore.stop();
    ...
    m_pTableFrame->OnGameEvent(m_pAndroidUserItem->GetChairId(),qzzjh::SUB_C_ADD_SCORE, (uint8_t*)content.data(), content.size());
}

void CAndroidUserItemSink::GameTimeOpencard()
{
    ThisThreadTimer->cancel(m_TimerOpenCard);
    m_pTableFrame->OnGameEvent(m_pAndroidUserItem->GetChairId(), qzzjh::SUB_C_OPEN_CARD, NULL, 0);
}
...
bool CAndroidUserItemSink::OnGameMessage(uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    switch (wSubCmdID)
    {
    case qzzjh::SUB_S_GAME_START_AI:
    {
		qzzjh::CMD_S_GameStartAi GameStartAi;
        if (!GameStartAi.ParseFromArray(pData,wDataSize))
            return false;
        m_cbOxCardData = GameStartAi.cboxcarddata();
        m_isMaxCard = GameStartAi.ismaxcard();
        openLog("qzzjh::SUB_S_GAME_START_AI m_cbOxCardData=%d m_isMaxCard=%d",m_cbOxCardData,m_isMaxCard);
        m_m_cbPlayStatus = true;
        return true;
    }
    case qzzjh::SC_GAMESCENE_FREE:
    {
        m_m_cbPlayStatus = false;
        return OnFreeGameScene(wSubCmdID,pData, wDataSize);
    }
    case qzzjh::SUB_S_GAME_START:
    {
        ClearAndroidCount=0;
        m_m_cbPlayStatus = true;
...
{
    qzzjh::CMD_S_GameStart GameStart;
...
{
    qzzjh::CMD_S_CallBanker CallBanker;
...
{
    qzzjh::CMD_S_CallBankerResult BankerResult;
...
{
    qzzjh::CMD_S_AddScoreResult AddScoreResult;
...
void CAndroidUserItemSink::ReadConfigInformation()
{
    ...
    myprintf(szConfigFileName, sizeof(szConfigFileName), _T("%sconf/qzzjh_config.ini"), szPath);
...
void  CAndroidUserItemSink::openLog(const char *p, ...)
{
    ...
    sprintf(Filename, "log/qzzjh/qzzjh_%d%d%d_%d.log",t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,m_pTableFrame->GetGameRoomInfo()->roomId);
