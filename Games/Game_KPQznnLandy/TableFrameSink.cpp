...
#define TIME_OPEN_CARD					10					//开牌定时器(5S)
...
//////////////////////////////////////////////////////////////////////////////////////
uint32_t CTableFrameSink::m_cbBankerMultiple[MAX_BANKER_MUL] = { 1,2,4 };		//叫庄倍数
uint32_t CTableFrameSink::m_cbJettonMultiple[MAX_JETTON_MUL] = { 5,10,15,20 };	//下注倍数
...
public:
    CGLogInit()
    {
        ...
        google::InitGoogleLogging("kpqznn");
...
CTableFrameSink::CTableFrameSink(void) :  m_dwCellScore(1), m_pITableFrame(NULL), m_iTodayTime(time(NULL))
{
...
{
    double YouXiaoWin[GAME_PLAYER]={0};

    string HuoQuNiu[GAME_PLAYER]={""};
    for(int i = 0; i < GAME_PLAYER; ++i)
    {
        if(m_cbPlayStatus[i] != 1)
            continue;
        // if(PaoMaDengWinScore[i] > 100)
        // {
        //     if(PaoMaDengCardType[i] == OX_FIVE_SMALL)
        //     {
        //         YouXiaoWin[i] = PaoMaDengWinScore[i];
        //         HuoQuNiu[i] = "五小牛";
        //     }else if(PaoMaDengCardType[i] == OX_GOLDEN_BULL)
        //     {
        //         YouXiaoWin[i] = PaoMaDengWinScore[i];
        //         HuoQuNiu[i] = "五花牛";
        //     }else if(PaoMaDengCardType[i] == OX_BOMB)
        //     {
        //         YouXiaoWin[i] = PaoMaDengWinScore[i];
        //         HuoQuNiu[i] = "炸弹牛";
        //     }else if(PaoMaDengCardType[i] == OX_SILVERY_BULL)
        //     {
        //         YouXiaoWin[i] = PaoMaDengWinScore[i];
        //         HuoQuNiu[i] = "四花牛";
        //     }else
        //     {
        //          if(m_GameLogic.GetMultiple(m_cbCardData[i]) == 3)
        //          {
        //              YouXiaoWin[i] = PaoMaDengWinScore[i];
        //              HuoQuNiu[i] = "牛牛";
        //          }
    ...
    //     }
    // }
    // if(bigone >= m_pITableFrame->GetGameRoomInfo()->broadcastScore)
    // {
    //     int sgType = SMT_GLOBAL|SMT_SCROLL;
    //     m_pITableFrame->SendGameMessage(index, "", sgType, bigone);
    // }
}
...
int CTableFrameSink::IsMaxCard(int wChairID)
{
    ...
    for (int i = 0; i < GAME_PLAYER; ++i)
    {
        if (0 == m_cbPlayStatus[i] || i == wChairID||m_pITableFrame->IsAndroidUser(i))
            continue;
...
void CTableFrameSink::Readini()
{
    ...
    ratioGamePlayerWeight_[0]=pt.get<int64_t>("GAME_CONFIG.PLAYER_CONF_0",0);
    ratioGamePlayerWeight_[1]=pt.get<int64_t>("GAME_CONFIG.PLAYER_CONF_1",50);
    ratioGamePlayerWeight_[2]=pt.get<int64_t>("GAME_CONFIG.PLAYER_CONF_2",50);
    ratioGamePlayerWeight_[3]=pt.get<int64_t>("GAME_CONFIG.PLAYER_CONF_3",50);

    stockWeak=pt.get<double>("GAME_CONFIG.STOCK_WEAK",1.0);

    StrageCard=pt.get<double>("GAME_CONFIG.STRAGECARD",0.5);
...
//    uint32_t chairId = pIServerUserItem->GetChairId();
    if( m_wGameStatus < GAME_STATUS_START || m_wGameStatus == GAME_STATUS_END)// ||  !m_bJoinedGame[chairId]
        return true;
    else
        return false;
}
...
bool CTableFrameSink::OnEventGameScene(uint32_t chairId, bool bisLookon)
{
    ...
    case GAME_STATUS_CALL:			//叫庄状态
    {
        ...
        {
             GameCall.add_cbhandcard(m_cbCardData[pIServerUser->GetChairId()][i]);
        }

         // caiqing
        LogCardData(pIServerUser->GetChairId(),m_cbCardData[pIServerUser->GetChairId()],SHOW_COUNT,"GAME_STATUS_CALL:" + mGameIds);
    ...
    case GAME_STATUS_SCORE:		//下注状态
    {
        ...
        for (int i = 0; i < SHOW_COUNT; ++i)
        {
             GameScore.add_cbhandcard(m_cbCardData[pIServerUser->GetChairId()][i]);
        }
        // caiqing
        LogCardData(pIServerUser->GetChairId(),m_cbCardData[pIServerUser->GetChairId()],SHOW_COUNT,"GAME_STATUS_SCORE:" + mGameIds);
    ...
    case GAME_STATUS_OPEN:		//开牌场景消息
    {
        ...
        for (int i = 0; i < GAME_PLAYER; ++i)
        {
            ...
            if (m_sCardType[i] == (-1))
            {
                if (i == dwChairID)
                {
                   ...
                   LOG(INFO)<<"---------------------------------open card"<<cardtype;
                    ...
                    // caiqing
                    LogCardData(i,m_cbCardData[i],SHOW_COUNT,"GAME_STATUS_OPEN 1:" + mGameIds);
                    LogCardData(i,m_cbOXCard[i],SHOW_COUNT,"GAME_STATUS_OPEN OXCard 1:" + mGameIds);
            ...
            else
            {
                ...
                // caiqing
                LogCardData(i,m_cbCardData[i],SHOW_COUNT,"GAME_STATUS_OPEN 2:" + mGameIds);
                LogCardData(i,m_cbOXCard[i],SHOW_COUNT,"GAME_STATUS_OPEN OXCard 2:" + mGameIds);
            }
        ...
        for (int i = 0; i < MAX_BANKER_MUL; ++i)
        {
            GameOpen.add_cbcallbankermultiple(m_cbCallable[chairId][i]);
        }
...
void CTableFrameSink::GameTimerReadyOver()
{
    ...
    if(wUserCount >= m_pGameRoomInfo->minPlayerNum)
    {
        ...
        m_wGameStatus = GAME_STATUS_START;
...
void CTableFrameSink::PersonalControl(bool steal)
{
    //扑克变量
    ...
    for (int j = 0; j < m_wPlayCount; j++)
    {
        ...
        for (int i = 0; i < GAME_PLAYER; i++)
        {
            ...
            //对比扑克
            if (m_GameLogic.CompareCard(cbUserCardData[i], m_cbCardData[wWinUser]) == steal)
            {
                wWinUser = i;
            }
        }
...
bool CTableFrameSink::ChangeCard(int chairid,int niu,int cardindex)
{
    if(chairid<0||chairid>GAME_PLAYER) return false;

    uint8_t  ardData[MAX_COUNT]={0};
    for(int i=0;i<MAX_COUNT;i++) ardData[i]=m_cbCardData[chairid][i];
    int indx=0;
    bool isStr=false;
    LOG(ERROR)<<"换牌前点数是: "<<(int)m_GameLogic.GetCardType(m_cbCardData[chairid], MAX_COUNT);
    for(int i=0;i<cvTempArray.size();i++)
    {
        ardData[cardindex]=cvTempArray[i];
        if(m_GameLogic.GetCardType(ardData, MAX_COUNT)==niu)
        {
            indx=i;
            isStr=true;
            break;
    ...
    if(isStr)
    {
        m_cbCardData[chairid][4]=cvTempArray[indx];
        cvTempArray.erase(cvTempArray.begin()+indx);
        LOG(ERROR)<<"换最后一张牌成功";
        LOG(ERROR)<<"换牌后点数是: "<<(int)m_GameLogic.GetCardType(m_cbCardData[chairid], MAX_COUNT);
...
void CTableFrameSink::OnGameStart()
{
    ...
    for(int i = 0; i < SHOW_COUNT; i++)
    {
        GameStart.add_cbhandcard(0);
    }
    for (int i = 0; i < MAX_BANKER_MUL; ++i)
    {
        GameStart.add_cbcallbankermultiple(0);
    }

    cbTempArray[MAX_CARD_TOTAL] = { 0 };
    m_GameLogic.RandCardData(cbTempArray, sizeof(cbTempArray));
    cvTempArray.clear();
    for(int i=0;i<MAX_CARD_TOTAL;i++)
    {
        cvTempArray.push_back(cbTempArray[i]);
    }
    LogCardData(INVALID_CHAIR,cbTempArray,MAX_CARD_TOTAL,"===========RandCardData:" + mGameIds);
    memset(m_cbCardData, 0, sizeof(m_cbCardData));
    memset(m_playerIsWin,0,sizeof(m_playerIsWin));

    int personCtrNum = 0;  //黑名单的数量
    int killOrLose = 0;
    float killOrLoseRatio = 0.0f;
    for (int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        //清除五个元素
        cvTempArray.erase(cvTempArray.begin(),cvTempArray.begin()+5);
        shared_ptr<CServerUserItem> user = m_pITableFrame->GetTableUserItem(i);
...
//        //黑名单点杀数据
//        tagBlacklistInfo& blackUser = user->GetBlacklistInfo();
//        tagPersonalProfit& personalCtr = user->GetPersonalPrott();
...
//            LOG(ERROR) << "-------------------agentRatio "<<personalCtr.agentRatio ;
//            LOG(ERROR) << "-------------------agentTendayProfit "<<personalCtr.agentTendayProfit ;
//            LOG(ERROR) << "-------------------playerALlbet "<<personalCtr.playerALlbet ;
//            LOG(ERROR) << "-------------------playerBaseValue "<<personalCtr.playerBaseValue ;
//            LOG(ERROR) << "-------------------playerBetLoseRatio "<<personalCtr.playerBetLoseRatio ;
//            LOG(ERROR) << "-------------------playerBetWinRatio "<<personalCtr.playerBetWinRatio ;
//            LOG(ERROR) << "-------------------playerHighLimit "<<personalCtr.playerHighLimit ;
//            LOG(ERROR) << "-------------------playerLowerLimit "<<personalCtr.playerLowerLimit ;
//            LOG(ERROR) << "-------------------playerAllProfit "<<personalCtr.playerAllProfit ;
//            LOG(ERROR) << "-------------------playerInterferenceRatio "<<personalCtr.playerInterferenceRatio;
...
//        }
    ...
    //
//    if(killOrLose == -1)//这个值等于-1是系统要放分
//    {

//        LOG(ERROR) << "-------------------个人库存决定放分" ;
//        PersonalControl(false);
//    }
//    else if(killOrLose == 1)//这个值等于1是系统要杀分
//    {
//        LOG(ERROR) << "-------------------个人库存决定杀分" ;
...
//    }
    ...
    for(int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        if(testCase->android&&user->IsAndroidUser())
        {
            testCase->CheckAdr("android",m_cbCardData[i]);
        }
    }
    ...
    for (int i = 0; i < GAME_PLAYER; ++i)
    {
        if (0 != m_cbPlayStatus[i])
        {
            ...
            GameStart.set_cbcallbankermultiple(0,m_cbBankerMultiple[0]);
            for (int j = 1; j < MAX_BANKER_MUL; ++j)
            {
                if(maxCallTimes >= m_cbBankerMultiple[j])
                {
                    m_cbCallable[i][j] = m_cbBankerMultiple[j];
                    GameStart.set_cbcallbankermultiple(j,m_cbBankerMultiple[j]);
                }else{
                    m_cbCallable[i][j] = -m_cbBankerMultiple[j];
                    GameStart.set_cbcallbankermultiple(j,-m_cbBankerMultiple[j]);
                }
...
//            std::cout << endl;
            ...
            {
                GameStart.set_cbhandcard(k,m_cbCardData[i][k]);
            }

            LogCardData(i,m_cbCardData[i],SHOW_COUNT,"CMD_S_GameStart:" + mGameIds);
            // caiqing
//            std::cout << endl;
        ...
        }else
        {
            ...
            //                m_cbCallable[i][j] = 0;
            //            }
                        memset(&m_cbCallable[i][0],0,MAX_BANKER_MUL);
            //            continue;
        }
...
int CTableFrameSink::OnUserCallBanker(int wChairID, int wCallFlag)
{
    ...
    for(int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        {
            callBanker.set_wcallbankeruser( wChairID);
            callBanker.set_cbcallmultiple(tmpHaveCall);
        }
        std::string data = callBanker.SerializeAsString();
        m_pITableFrame->SendTableData(i, NN_SUB_S_CALL_BANKER, (uint8_t*)data.c_str(), data.size());
    }
    ...
    if (wBankerUser != INVALID_CHAIR)
    {
        ...
        for (int i = 0; i < MAX_JETTON_MUL; ++i)
        {
            CallResult.add_cbjettonmultiple(0);
        }
        for(int j = 0; j < GAME_PLAYER; j++ )
        {
            if( m_cbPlayStatus[j] == 1 )
            {
                ...
                if(j != wBankerUser)
                {
                    ...
                    if( callMaxTimes >= 20)
                    {
                        for (int i = 0; i < MAX_JETTON_MUL; ++i)
                        {
                            ...
                            CallResult.set_cbjettonmultiple(i,m_cbJettonMultiple[i]);
                        }
                    }else if(callMaxTimes >= 4)
                    {
                        for (int i = 0; i < MAX_JETTON_MUL; ++i)
                        {
                            ...
                            CallResult.set_cbjettonmultiple(i,m_cbJettonable[j][i]);
                        }
                    }else
                    {
                        for (int i = 1; i <= MAX_JETTON_MUL; ++i)
                        {
                            ...
                            CallResult.set_cbjettonmultiple(i-1,m_cbJettonable[j][i-1]);
                        }
                    }
                }else
                {
                ...
                if(userItem->IsAndroidUser())
                {
                    CallResult.set_dwbankmuticalutical(m_sCallBanker[m_wBankerUser]);
                }

                CallResult.set_cbaddjettontime(TIME_ADD_SCORE);			//加注时间
                std::string data = CallResult.SerializeAsString();
                m_pITableFrame->SendTableData(j, NN_SUB_S_CALL_BANKER_RESULT, (uint8_t*)data.c_str(), data.size());
            }
...
int CTableFrameSink::OnUserAddScore(int wChairID, int wJettonIndex)
{
    ...
    if ((wTotalCount + 1) == m_wPlayCount)
    {
        ...
        for (int i = 0; i < GAME_PLAYER; ++i)
        {
            ...
            tagBlacklistInfo& blackUser = user->GetBlacklistInfo();
            tagPersonalProfit& personalCtr = user->GetPersonalPrott();
            if(!m_bBlacklist[i]&&user->GetBlacklistInfo().status == 1 && user->GetBlacklistInfo().weight >= rand()%1000)
            {
                m_iBlacklistNum++;
    ...
    for (int i = 0; i < GAME_PLAYER; ++i)
    {
        ...
        if (1 == cbIsSendCard)
        {
            ...
            if(m_cbPlayStatus[i] != 0 )
            {
                ...
                // caiqing
                LogCardData(i,m_cbCardData[i],MAX_COUNT,"SC_GAMESCENE_OPEN:" + mGameIds);
            }
...
void CTableFrameSink::AnalyseCardExAllControl()
{
...
void CTableFrameSink::AnalyseCardEx()
{
    ...
    if (m_llStorageControl > llHighLimitVal && rand() % 100 <= StrageCard*100)
        return ;
    else if (m_llStorageControl < llLowerLimitVal && rand() % 100 <= StrageCard*100)
        return ;
    if(bSteal)
    {
        LOG(ERROR) << "suan fa 吸分";
...
int CTableFrameSink::OnUserOpenCard(int wChairID)
{
    ...
    LogCardData(wChairID,m_cbCardData[wChairID],MAX_COUNT,"OpenCardResult:" + mGameIds);
    // caiqing
...
void CTableFrameSink::NormalGameEnd(int dwChairID)
{
	...
	//记录当局游戏详情binary
	KPQznn::CMD_S_GameRecordDetail details;
    ...
    ****/
    ...
    //叫庄倍数
    int64_t cbBankerMultiple = m_sCallBanker[m_wBankerUser];
    //庄家牌型倍数
...
//        }
        ...
        if(m_bPlayerMiPai[i])
        {
            scoreInfo.gameEvent="1";
            LOG(INFO)<<"scoreInfo.gameEvent==1";
        }
...
//        }
		...
		KPQznn::PlayerRecordDetail* detail = details.add_detail();
		//账号/昵称
		...
		//手牌/牌型
        KPQznn::HandCards* pcards = detail->mutable_handcards();
...
bool CTableFrameSink::OnGameMessage( uint32_t wChairID, uint8_t wSubCmdID, const uint8_t* pData, uint32_t wDataSize)
{
    ...
    switch (wSubCmdID)
    {
        ...
        case NN_SUB_C_MI_CARD:					//开牌消息
        {
            m_bPlayerMiPai[wChairID]=true;
            LOG(ERROR)<<"mi pai";
            return true;
        }
    }
