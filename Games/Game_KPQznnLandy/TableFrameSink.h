...
private:
    ...
    bool ChangeCard(int chairid,int niu,int cardindex=4);
...
private:
   ...
   std::string StringCards(uint8_t cbHandCardData[], uint8_t count);
    ...
    inline string converToHex(const uint8_t* hex, int size,int from)
    {
        string strValue;
        char buf[32]={0};
    ...
    // 写牌日志
    inline void LogCardData(int32_t id, uint8_t cbCardData[], int32_t len, string name)
    {
        stringstream ssStr;
        ssStr <<"chair:"<< id << " "<< name << "\n"; 
        ssStr <<  converToHex(cbCardData,len,0);
        LOG(INFO) << ssStr.str();
    }
private:
...
private:
    ...
    void PersonalControl(bool steal);
private:
    //游戏信息
...
//    uint8_t                 m_SitTableFrameStatus[GAME_PLAYER];				    //游戏中in play
    ...
    bool                    m_bPlayerMiPai[GAME_PLAYER];                  // palyer is Operation
    int                     m_iBlacklistNum;
...
private:
    ...
    uint32_t                  m_cbCallable[GAME_PLAYER][MAX_BANKER_MUL];//玩家可用的叫庄倍数
    uint32_t                 m_cbJettonable[GAME_PLAYER][MAX_JETTON_MUL];//玩家可用的下注倍数
private:
    static uint32_t			m_cbJettonMultiple[MAX_JETTON_MUL];					//下注倍数1, 5, 10, 15, 20
    static uint32_t			m_cbBankerMultiple[MAX_BANKER_MUL];					//叫庄倍数 1, 2, 4
