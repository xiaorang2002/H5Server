...
const char *m_Base64Char = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
using namespace std;

#define likely(x) __builtin_expect(!!(x), 1)
#define unlikely(x) __builtin_expect(!!(x), 0)

CryptoPublic::CryptoPublic()
{
}

char *CryptoPublic::Base64Encode(const unsigned char *bindata, char *base64, int binlength)
{
    int i, j;
    ...
    for (i = 0, j = 0; i < binlength; i += 3)
    {
        ...
        base64[j++] = m_Base64Char[(int)current];
...
int CryptoPublic::Base64Decode(const char *base64, unsigned char *bindata)
{
    int i, j;
    unsigned char k;
    ...
    for (i = 0, j = 0; base64[i] != '\0'; i += 4)
    {
        ...
        for (k = 0; k < 64; k++)
        {
            if (m_Base64Char[k] == base64[i])
                temp[0] = k;
        }
        for (k = 0; k < 64; k++)
        {
            if (m_Base64Char[k] == base64[i + 1])
                temp[1] = k;
        }
        for (k = 0; k < 64; k++)
        {
            if (m_Base64Char[k] == base64[i + 2])
                temp[2] = k;
        }
        for (k = 0; k < 64; k++)
        {
            if (m_Base64Char[k] == base64[i + 3])
                temp[3] = k;
        }
...
void CryptoPublic::CharToHex(const char *pSrc, unsigned char *pDest)
{
    int nSrcLen = 0;
    if (pSrc != 0)
    {
...
void CryptoPublic::ClearPadding(unsigned char *pSrc, int nSrcLen)
{
    int nEnd = nSrcLen;
...
void CryptoPublic::Padding(char *pSrc, int nSrcLen, int nPadLen)
{
...
void CryptoPublic::AES_ECB_Encrypt(string const &key, unsigned char *szIn, string &str)
{
...
string CryptoPublic::AES_ECB_Decrypt(string const &key, string const &szIn)
{
    //SetDecryptKey(0);
    AES_KEY aes;
    //设置加密密钥private_
...
//string CryptoPublic::ECBDecrypt(std::string const& data, std::string const& key)
string CryptoPublic::AES_ECB_Decrypt(string const &key, unsigned char const *szIn)
{
    //SetDecryptKey(0);
    AES_KEY aes;
    //设置加密密钥
...
// 产生公钥和私钥
void CryptoPublic::generateRSAKey(string &pubKey, string &priKey)
{
    size_t pri_len;
    size_t pub_len;
    char *pri_key = NULL;
...
//int CryptoPublic::rsaPublicNoEncrypt(const string &pubKey, const vector<unsigned char> &clearText, vector<unsigned char> &encryptedText)
//{
...
int CryptoPublic::rsaPublicEncrypt(const string &pubKey, const vector<unsigned char> &clearText, vector<unsigned char> &encryptedText)
{
    int ret = 0;
    try
    {
...
int CryptoPublic::rsaPrivateDecrypt(const std::string &priKey, const unsigned char *in, int inLen, vector<unsigned char> &out)
{
    int ret = 0;
...
//int CryptoPublic::rsaPrivateEncrypt(const string &priKey, const string &clearText, string &encryptedText)
//{
//    RSA *rsa = NULL;
...
//int CryptoPublic::rsaPublicDecrypt(const std::string &pubKey, const std::string &cipherText, string &decryptedText)
//{
//    RSA *rsa = RSA_new();
//    BIO *keybio;
...
string CryptoPublic::generateAESKey()
{
...
string CryptoPublic::generateUuid()
{
    boost::uuids::random_generator rgen;
...
int CryptoPublic::aesEncrypt(const string &key, const unsigned char *in, int inLen,
...
int CryptoPublic::aesEncrypt(const string &key, const vector<unsigned char> &in, vector<unsigned char> &out, internal_prev_header *internal_header)
{
    AES_KEY aes;
    //    unsigned char key[AES_BLOCK_SIZE];        // AES_BLOCK_SIZE = 16
    unsigned char iv[AES_BLOCK_SIZE]; // init vector
...
int CryptoPublic::aesEncrypt(const string &key, const vector<unsigned char> &in, vector<unsigned char> &out)
{
    AES_KEY aes;
    //    unsigned char key[AES_BLOCK_SIZE];        // AES_BLOCK_SIZE = 16
    unsigned char iv[AES_BLOCK_SIZE]; // init vector
...
int CryptoPublic::aesDecrypt(const string &key, const unsigned char *in, int inLen, vector<unsigned char> &out)
{
    AES_KEY aes;
    //    unsigned char key[AES_BLOCK_SIZE];        // AES_BLOCK_SIZE = 16
    unsigned char iv[AES_BLOCK_SIZE]; // init vector
...
int CryptoPublic::aesDecrypt(const string &key, const unsigned char *in, int inLen, unsigned char *out, int &outLen)
{
    AES_KEY aes;
    //    unsigned char key[AES_BLOCK_SIZE];        // AES_BLOCK_SIZE = 16
    unsigned char iv[AES_BLOCK_SIZE]; // init vector
...
string CryptoPublic::MD5HashToString(const char *data, int size)
{
    string hash = "";
    do
    {
...
string CryptoPublic::BufferToHexString(unsigned char *buf, int len)
{
    ostringstream oss;
...
void CryptoPublic::HexStringToBuffer(const string &source, unsigned char *dest, int &len)
{
...
bool CryptoPublic::base64Encode(const string &input, string &output)
{
...
bool CryptoPublic::base64Decode(const string &input, string &output)
{
...
int CryptoPublic::aesEncryptEx(const string &key, unsigned char *in, unsigned char *out)
{
    AES_KEY aes;
...
int CryptoPublic::aesDecryptEx(const string &key, const unsigned char *in, int inLen, unsigned char *out, int &outLen)
{
    AES_KEY aes;
    //    unsigned char key[AES_BLOCK_SIZE];        // AES_BLOCK_SIZE = 16
    unsigned char iv[AES_BLOCK_SIZE]; // init vector
...
bool CryptoPublic::DES_ECB_Encrypt(unsigned char *key, unsigned char *iv, string & strInput, string & strOutput)
{
...
bool CryptoPublic::DES_ECB_EncryptEx(unsigned char *key, unsigned char *iv, string & strInput, string & strOutput)//  char *strOutput)
{
    /*
    DES_cblock key;
    //随机密钥
...
int CryptoPublic::AES_ECB_Decrypt_EX(string const &key, unsigned char *szIn,int nEncryptDataLen,vector<unsigned char> &out)
{
    AES_KEY aes;
...
int CryptoPublic::AES_ECB_Encrypt_EX(string const &key, unsigned char *szIn, int nInLen,vector<unsigned char> &out) //nDataLen
{
    //设置加密密钥
    AES_KEY aes;
