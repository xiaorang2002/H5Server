...
class LotteryServer : public muduo::noncopyable
{
public:
...
public:
	LotteryServer(muduo::net::EventLoop* loop,
...
private:
	...
	/// {"companyCode":"IG","gametype":"102","score":"5002000"}
	// 公司帐目信息
	struct company_balance_t
	{
		int32_t retCode;
		int32_t gametype;
		int64_t balance;
	...
	std::map<string, company_balance_t> company_balance_;
	std::map<int64_t, std::map<string, company_balance_t>> player_plat_balance_;
	mutable boost::shared_mutex company_balance_mutex_;
private: 
	// 获取平台配置
	bool getCfgByCode(string platformcode,third_game_cfg & agent_info, int32_t & req_code,string & errmsg);
 	int32_t savePlayerBalance(int64_t uid,int64_t balance,int32_t gametype,string companyCode);
	bool initPlayerPlatBalance(int64_t uid);
	// 获取代理限红
	...
	// 检测并创建游戏帐号
 	bool getCreateAccountParams(ReqParamCfg & reqParam,third_game_cfg agent_info,int32_t & req_code,string & errmsg,cpr::Response & resp);
	// 查询订单状态
	bool orderInquiryParams(ReqParamCfg reqParam,third_game_cfg agent_info,string & billNo,int32_t & req_code,string & errmsg,cpr::Response & resp);
	// 获取参数
	string balanceInquiryParams(ReqParamCfg reqParam,third_game_cfg agent_info,bool bOnlineCheck = false);
	// 查询余额
	bool getBalanceIG(ReqParamCfg reqParam,third_game_cfg agent_info,int64_t & balance,int32_t & req_code,string & errmsg);
	// 游戏转账（上下分）参数
	string createTransferParams(ReqParamCfg reqParam,third_game_cfg agent_info, string billNo, string type,string creditStr);
	// 游戏转账（上下分）
	bool transferBalance(int64_t &newScore,ReqParamCfg reqParam,third_game_cfg agent_info, string billNo, string type,int64_t & _newbalance, int32_t &req_code, string &errmsg, cpr::Response &resp);
	// 进入游戏
	...
	// 统一请求参数
	cpr::Response getUrlRequest(string & parameContent,third_game_cfg agent_info);
	// Post
	cpr::Response PostUrlRequest(string & parameContent,string & reqHandle);//, cpr::Payload param);
	// 检查请求参数
	int32_t checkReqParams(string & reqStr,string & errmsg,ReqParamCfg &reqParam);
	int32_t checkReqParamsEx(string & reqStr,ReqParamCfg &reqParam, string & errmsg);
	// 解析返回结果
	bool getJsonPtreeEx( string & jsonStr, string &text,int64_t & logId,int32_t & code,string & msg,boost::property_tree::ptree & pt);
	// 解析返回结果
	bool getJsonPtree( string & jsonStr,int32_t & code,string & msg,boost::property_tree::ptree & pt);
	// 获取用户信息
	bool getUserNameById(ReqParamCfg &reqParam, int32_t & req_code,string & errmsg);
	// 创建订单
	...
	// 更新玩家状态
	void updatePalyingStatus(int64_t userid,int32_t gameid = 0);
	// 获取维护状态
	...
	// 
	bool TransferIG(ReqParamCfg param,third_game_cfg agentInfo,int64_t &newbalance, int32_t & req_code,string & req_msg);
	int32_t get_balance_all(ReqParamCfg param,string &rspdata,string &req_msg);
	int32_t get_recycle_all(ReqParamCfg param,string &rspdata,string &req_msg);
	int32_t createCompanyBalanceJson(int64_t uid,int32_t operatetype,std::string & rspdata,std::string & req_msg,int32_t req_code = 0);
	string writeJson4balance(int64_t score,int32_t req_code,string req_msg,string companyCode,int32_t gametype );

	bool updatePlayerPlatBalance(string msgStr);
	// 是否允许访问
	...
	// 获取查询连接参数
	bool getBalanceWG(ReqParamCfg reqParam,third_game_cfg agent_info, int64_t & balance,int32_t & req_code,string & errmsg,bool & status_online);
	// @bOnlineCheck是否获取在线状态，true,获取
	string balanceInquiryParamsWG(string &lgname, bool bOnlineCheck,third_game_cfg agent_info);
	// 缓存第三方参数
	bool cacheThirdPartGame();
	// 帐变
	bool AddScoreChangeRecordToDB(int64_t userId,string &account,int64_t addScore,int32_t agentId,string linecode,int64_t beforeScore,int32_t roomId,string reason);
	inline int32_t getTidByGameid(int32_t gameid)
    {
		//101010001
        return (gameid/10000)%1000;
    }
    inline int32_t getGameid(int32_t gameid)
    {
        return (gameid%1000);
    }
	//判断是否数字组成的字符串 ///
...
public:
    ...
    MutexLock m_sync_Lock;

	//代理信息map[agentId]=agent_info_
	// third_game_cfg agent_info_;
	std::map<string, third_game_cfg> agent_info_;
    ...
    std::map<int32_t,string> m_CompanyNoCodeMap;//
...
private:
	muduo::net::EventLoop * loopTimer_;
