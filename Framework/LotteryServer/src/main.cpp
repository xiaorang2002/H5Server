...
//#include "proto/LotteryServer.Message.pb.h"
...
LotteryServer *gLotteryServer = NULL;
...
int main(int argc, char* argv[])
{
    ...
    if(!boost::filesystem::exists("./log/LotteryServer/"))
    {
        boost::filesystem::create_directories("./log/LotteryServer/");
    }
    muduo::AsyncLogging log(::basename("LotteryServer"), kRoolSize);
    log.start();
    g_asyncLog = &log;

    LOG_INFO << "---LotteryServer pid = " << getpid();

    //=====config=====
    if(!boost::filesystem::exists("./conf/lottery.conf"))
    {
        LOG_INFO << "./conf/lottery.conf not exists";
        return 1;
    }
    ...
    //IP地址
    //strIpAddr = "LotteryServer.IP";
    //TCP监听相关
    std::string strTcpPort = "LotteryServer.tcpPort";
    std::string strTcpNumThreads = "LotteryServer.tcpNumThreads";
    std::string strTcpWorkerNumThreads = "LotteryServer.tcpWorkerNumThreads";
    //HTTP监听相关
    std::string strHttpPort = "LotteryServer.httpPort";
    std::string strHttpNumThreads = "LotteryServer.httpNumThreads";
    std::string strHttpWorkerNumThreads = "LotteryServer.httpWorkerNumThreads";
    ...
    LotteryServer server(&loop, listenAddr, listenAddrHttp, strIpAddr);
    // 
