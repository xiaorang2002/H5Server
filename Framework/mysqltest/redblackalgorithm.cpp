...
void RedBlackAlgorithm::Caculet()
{
    MogoNodes.clear();
    NodeMongo nodefa1(111,1000,0);
    MogoNodes.push_back(nodefa1);
    NodeMongo nodechl1(811,1000,111);
    MogoNodes.push_back(nodechl1);
    NodeMongo nodechl2(8111,2000,111);
    MogoNodes.push_back(nodechl2);
    NodeMongo nodechl3(9111,4000,111);
    MogoNodes.push_back(nodechl3);

    //NodeMongo nodefa2(222,10000,0);
    //MogoNodes.push_back(nodefa1);
    NodeMongo nodeshl1(822,10000,811);
    MogoNodes.push_back(nodeshl1);
    NodeMongo nodeshl2(8222,20000,811);
    MogoNodes.push_back(nodeshl2);
    NodeMongo nodeshl3(82222,40000,811);
    MogoNodes.push_back(nodeshl3);


    NodeMongo nodebhl1(833,10000,8111);
    MogoNodes.push_back(nodebhl1);
    NodeMongo nodebhl2(8333,20000,8111);
    MogoNodes.push_back(nodebhl2);
    NodeMongo nodebhl3(83333,40000,8111);
    MogoNodes.push_back(nodebhl3);



    vector<NodeMongo> text;
    text.clear();
    copy(MogoNodes.begin(),MogoNodes.end(),std::back_inserter(text));

    NodeRecursion(nodefa1,text);

    //NodeRecursion(nodefa1);
    LOG_INFO<<"第一个父节点的团队贡献值===="<<nodefa1.TeamContribution;


    text.clear();
    copy(MogoNodes.begin(),MogoNodes.end(),std::back_inserter(text));
    NodeRecursion(nodechl1,text);
    LOG_INFO<<"二级节点的团队贡献值===="<<nodechl1.TeamContribution;
}
void RedBlackAlgorithm::CallBackHandle()
{
    SetBetting(0 ,RandomInt(2000,2000));
    SetBetting(1 ,RandomInt(0,0));
    SetBetting(2 ,RandomInt(0,0));
    SetAlgorithemNum(5000000,0, profit,0.45, 5000);
    //测试算法的
    uint8_t aaa[2][3];
    GetOpenCard(aaa);
    CalculationResults();
