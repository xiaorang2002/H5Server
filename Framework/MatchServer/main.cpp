...
Landy::CMatchServer *gMatchServer = NULL;
...
int main(int argc, char* argv[])
{
    ...
    if(!boost::filesystem::exists("./log/MatchServer/"))
    {
        boost::filesystem::create_directories("./log/MatchServer/");
    }
    muduo::AsyncLogging log(::basename("MatchServer"), kRoolSize);
    log.start();
    g_asyncLog = &log;
    ...
    if(boost::filesystem::exists("./conf/game.conf"))
    {
		...
		LOG_INFO << __FUNCTION__ << " --- *** " << "MatchServer = " << strGameServerIP << ":" << gameServerPort;
        
		muduo::net::InetAddress serverAddr(gameServerPort);
        Landy::CMatchServer server(&loop, serverAddr, kIdleSeconds, netcardName);
		///////////////////////////////////////
           ...
           server.InitZookeeper(strZookeeperServerIP, gameId, roomId))
        {
            ...
            if(server.InitServer(gameId, roomId))
            {
                ...
                LOG_WARN << "---registerSignalHandler耗时(s)=======>[" << timdiff<<"]<=======";
				///////////////////////////////////////
				//worker线程，处理游戏业务逻辑 ///////////
