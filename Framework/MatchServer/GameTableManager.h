...
public:
    void InitAllTable(uint32_t tableCount, tagGameInfo * pGameInfo, tagGameRoomInfo* pRoomInfo, PFNCreateTableSink pfnCreate, shared_ptr<EventLoopThread> &loopThread, shared_ptr<EventLoopThread>& game_dbmysql_thread, ILogicThread*);
...
public:
    ...
    shared_ptr<ITableFrame> FindNormalSuitTable(uint32_t exceptTableId = INVALID_TABLE);
