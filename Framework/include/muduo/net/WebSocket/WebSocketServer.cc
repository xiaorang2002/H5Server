            ...
            case WCT_DISCONN:  //disconnect
                {
                    ...
                    int sendLen = WebSocketResponse::WebSocket_enPackage(&data[0], data.size(), sendData, false, WCT_DISCONN);
...
//                    conn->forceClose();
            ...
            case WCT_PING:
                {
                    vector<uint8_t> sendData;
                    int sendLen = WebSocketResponse::WebSocket_enPackage(&data[0], data.size(), sendData, false, WCT_PONG);
                    conn->send(&sendData[0], sendLen);
                }
                break;
            default:
                                  ...
                                  Timestamp receiveTime, string &ip)
{
    ...
    {
        LOG_ERROR <<" >>> onHandShake conn->shutdown(), receiveTime " <<receiveTime.toFormattedString();
...
//        response += "Sec-WebSocket-Extensions: deflate-stream\r\n";
        ...
        if(ip.empty()){
            ...
            LOG_WARN << " >>> onHandShake X-Forwarded-For,ip " << ip;
			//第一个IP为客户端真实IP，可伪装，第二个IP为一级代理IP，第三个IP为二级代理IP
            std::string::size_type spos = ip.find_first_of(',');
            if (spos == std::string::npos) {
                // LOG_WARN << " >>> X-Forwarded-For,canot find , char ip "<< ip;
			}
            else {
                ip = ip.substr(0, spos);
                LOG_WARN << " >>> substr For ip " << ip <<" spos "<<spos;
            }
        }
        conn->send(response);
//        if(m_WebSocketConnectedCallback)
