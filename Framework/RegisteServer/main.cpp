...
//#include "proto/RegisteServer.Message.pb.h"
...
RegisteServer *gLoginServer = NULL;
...
int main(int argc, char* argv[])
{
    ...
    if(!boost::filesystem::exists("./log/RegisteServer/"))
    {
        boost::filesystem::create_directories("./log/RegisteServer/");
    }
    muduo::AsyncLogging log(::basename("RegisteServer"), kRoolSize);
    log.start();
    g_asyncLog = &log;

    LOG_INFO << "---RegisteServer pid = " << getpid();

    //=====config=====
    if(!boost::filesystem::exists("./conf/registe.conf"))
    {
        LOG_INFO << "./conf/registe.conf not exists";
        return 1;
    }
    ...
    //IP地址
    //strIpAddr = "RegisteServer.IP";
    //TCP监听相关
    std::string strTcpPort = "RegisteServer.tcpPort";
    std::string strTcpNumThreads = "RegisteServer.tcpNumThreads";
    std::string strTcpWorkerNumThreads = "RegisteServer.tcpWorkerNumThreads";
    //HTTP监听相关
    std::string strHttpPort = "RegisteServer.httpPort";
    std::string strHttpNumThreads = "RegisteServer.httpNumThreads";
    std::string strHttpWorkerNumThreads = "RegisteServer.httpWorkerNumThreads";
    ...
    LOG_WARN << "---HttpPort "<< httpPort ;
 
    muduo::net::InetAddress listenAddr(tcpPort);
    muduo::net::InetAddress listenAddrHttp(httpPort);
    RegisteServer server(&loop, listenAddr, listenAddrHttp, strIpAddr);
