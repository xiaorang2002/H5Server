...
#ifndef __IAOCSREDXBLACK_HEADER__
#define __IAOCSREDXBLACK_HEADER__
...
typedef struct tagRedRemotePlusHard
{
	int32 sign;		// 数据有效性.
	int32 tabid;	// 加难桌台号.
	int32 hard;		// 是否有加难.
	int32 resv;		// 保留数据值. 
} RedRemotePlusHard,*PRedRemotePlusHard;

// 错误代码.
enum eRedXBlackErrCodes
{
    AICSREDERR_UNK	  = -1,		// 未知错误.
    AICSREDERR_OK	  =  0,		// 没有错误.
    AICSREDERR_NOKILL =  0,	    // 算法默认没打死鱼状态.
    AICSREDERR_KILLED =  0x90,	// 算法打死鱼返回值.
};
...
inline IAicsRedxBlackEngine* WINAPI GetAicsRedEngine()
{
	...
	strcat(buf,TEXT("/AicsRedXBlackEngine.dll"));
#else
	getcwd(buf,sizeof(buf));
	strcat(buf,TEXT("/libAicsRedXBlackEngine.so"));
#endif//_WIN32
            HINSTANCE hInst=LoadLibrary(buf);
            if (!hInst) {
				printf("load library libAicsRedXBlackEngine.so failed!");
                break;
            }
            ...
            if (!pfn) {
				printf("get function of fun0003 failed");
                FreeLibrary(hInst);
                break;
            }
...
#endif//__IAOCSREDXBLACK_HEADER__
